import org.openrs2.deob.annotation.OriginalClass;
import org.openrs2.deob.annotation.OriginalMember;

@OriginalClass("client!ml")
public class class296 extends class160 {

    @OriginalMember(owner = "client!ml", name = "U", descriptor = "I")
    private int field4678 = 6;

    @OriginalMember(owner = "client!ml", name = "S", descriptor = "Ljava/lang/String;")
    public static String field4676 = "green:";

    @OriginalMember(owner = "client!ml", name = "Y", descriptor = "[I")
    public static int[] field4682 = new int[50];

    @OriginalMember(owner = "client!ml", name = "V", descriptor = "I")
    public static int field4679 = 0;

    @OriginalMember(owner = "client!ml", name = "ab", descriptor = "Lam;")
    public static class180 field4684 = new class180();

    @OriginalMember(owner = "client!ml", name = "db", descriptor = "Ljava/lang/String;")
    public static String field4687 = "Loaded update list";

    @OriginalMember(owner = "client!ml", name = "T", descriptor = "I")
    public static int field4677;

    @OriginalMember(owner = "client!ml", name = "W", descriptor = "I")
    public static int field4680;

    @OriginalMember(owner = "client!ml", name = "X", descriptor = "I")
    public static int field4681;

    @OriginalMember(owner = "client!ml", name = "Z", descriptor = "I")
    public static int field4683;

    @OriginalMember(owner = "client!ml", name = "bb", descriptor = "I")
    public static int field4685;

    @OriginalMember(owner = "client!ml", name = "cb", descriptor = "I")
    public static int field4686;

    @OriginalMember(owner = "client!ml", name = "eb", descriptor = "I")
    public static int field4688;

    @OriginalMember(owner = "client!ml", name = "e", descriptor = "(I)V")
    public static void method1977(int arg0) {
        field4684 = null;
        field4687 = null;
        if (arg0 != 2) {
            method1979(-91, 93);
        }
        field4682 = null;
        field4676 = null;
    }

    @OriginalMember(owner = "client!ml", name = "<init>", descriptor = "()V")
    public class296() {
        super(2, false);
    }

    @OriginalMember(owner = "client!ml", name = "a", descriptor = "(IZ)[[I")
    public final int[][] method3(int arg0, boolean arg1) {
        ++field4683;
        if (!arg1) {
            field4676 = null;
        }
        int[][] var3 = super.field2476.method839(false, arg0);
        if (super.field2476.field1950) {
            int[][] var4 = this.method1079(arg0, 124, 0);
            int[][] var5 = this.method1079(arg0, 97, 1);
            int[] var6 = var3[0];
            int[] var7 = var3[2];
            int[] var8 = var4[0];
            int[] var9 = var4[1];
            int[] var10 = var5[0];
            int[] var11 = var3[1];
            int[] var12 = var4[2];
            int[] var13 = var5[1];
            int[] var14 = var5[2];
            int var15 = this.field4678;
            if (var15 != 1) {
                if (~var15 != -3) {
                    if (~var15 != -4) {
                        if (~var15 != -5) {
                            if (var15 != 5) {
                                if (~var15 != -7) {
                                    if (var15 != 7) {
                                        if (var15 != 8) {
                                            if (var15 != 9) {
                                                if (var15 != 10) {
                                                    if (var15 != 11) {
                                                        if (var15 == 12) {
                                                            for (int var16 = 0; ~var16 > ~class240.field3896; ++var16) {
                                                                int var17 = var8[var16];
                                                                int var18 = var10[var16];
                                                                int var19 = var14[var16];
                                                                int var20 = var12[var16];
                                                                int var21 = var9[var16];
                                                                int var22 = var13[var16];
                                                                var6[var16] = -(var17 * var18 >> 11) + var18 + var17;
                                                                var11[var16] = -(var21 * var22 >> 11) + var21 + var22;
                                                                var7[var16] = -(var19 * var20 >> 11) + var19 + var20;
                                                            }
                                                        }
                                                    } else {
                                                        for (int var23 = 0; ~class240.field3896 < ~var23; ++var23) {
                                                            int var24 = var14[var23];
                                                            int var25 = var9[var23];
                                                            int var26 = var8[var23];
                                                            int var27 = var12[var23];
                                                            int var28 = var10[var23];
                                                            int var29 = var13[var23];
                                                            var6[var23] = var26 > var28 ? -var28 + var26 : -var26 + var28;
                                                            var11[var23] = var25 > var29 ? -var29 + var25 : -var25 + var29;
                                                            var7[var23] = ~var27 < ~var24 ? -var24 + var27 : var24 - var27;
                                                        }
                                                    }
                                                } else {
                                                    for (int var30 = 0; ~var30 > ~class240.field3896; ++var30) {
                                                        int var31 = var8[var30];
                                                        int var32 = var13[var30];
                                                        int var33 = var9[var30];
                                                        int var34 = var12[var30];
                                                        int var35 = var14[var30];
                                                        int var36 = var10[var30];
                                                        var6[var30] = ~var31 >= ~var36 ? var36 : var31;
                                                        var11[var30] = ~var33 < ~var32 ? var33 : var32;
                                                        var7[var30] = ~var34 < ~var35 ? var34 : var35;
                                                    }
                                                }
                                            } else {
                                                for (int var37 = 0; ~class240.field3896 < ~var37; ++var37) {
                                                    int var38 = var8[var37];
                                                    int var39 = var14[var37];
                                                    int var40 = var9[var37];
                                                    int var41 = var13[var37];
                                                    int var42 = var10[var37];
                                                    int var43 = var12[var37];
                                                    var6[var37] = var42 <= var38 ? var42 : var38;
                                                    var11[var37] = var40 < var41 ? var40 : var41;
                                                    var7[var37] = ~var39 < ~var43 ? var43 : var39;
                                                }
                                            }
                                        } else {
                                            for (int var44 = 0; ~class240.field3896 < ~var44; ++var44) {
                                                int var45 = var12[var44];
                                                int var46 = var9[var44];
                                                int var47 = var8[var44];
                                                var6[var44] = var47 != 0 ? 4096 - (-var10[var44] + 4096 << 12) / var47 : 0;
                                                var11[var44] = ~var46 == -1 ? 0 : -((-var13[var44] + 4096 << 12) / var46) + 4096;
                                                var7[var44] = var45 == 0 ? 0 : -((4096 - var14[var44] << 12) / var45) + 4096;
                                            }
                                        }
                                    } else {
                                        for (int var48 = 0; var48 < class240.field3896; ++var48) {
                                            int var49 = var9[var48];
                                            int var50 = var8[var48];
                                            int var51 = var12[var48];
                                            var6[var48] = ~var50 == -4097 ? 4096 : (var10[var48] << 12) / (-var50 + 4096);
                                            var11[var48] = ~var49 != -4097 ? (var13[var48] << 12) / (-var49 + 4096) : 4096;
                                            var7[var48] = var51 == 4096 ? 4096 : (var14[var48] << 12) / (-var51 + 4096);
                                        }
                                    }
                                } else {
                                    for (int var52 = 0; class240.field3896 > var52; ++var52) {
                                        int var53 = var14[var52];
                                        int var54 = var13[var52];
                                        int var55 = var10[var52];
                                        var6[var52] = ~var55 > -2049 ? var8[var52] * var55 >> 11 : -((-var8[var52] + 4096) * (-var55 + 4096) >> 11) + 4096;
                                        var11[var52] = var54 >= 2048 ? -((4096 - var9[var52]) * (-var54 + 4096) >> 11) + 4096 : var9[var52] * var54 >> 11;
                                        var7[var52] = ~var53 > -2049 ? var12[var52] * var53 >> 11 : -((-var12[var52] + 4096) * (-var53 + 4096) >> 11) + 4096;
                                    }
                                }
                            } else {
                                for (int var56 = 0; ~var56 > ~class240.field3896; ++var56) {
                                    var6[var56] = -((4096 - var8[var56]) * (-var10[var56] + 4096) >> 12) + 4096;
                                    var11[var56] = 4096 - ((-var9[var56] + 4096) * (-var13[var56] + 4096) >> 12);
                                    var7[var56] = -((4096 - var14[var56]) * (-var12[var56] + 4096) >> 12) + 4096;
                                }
                            }
                        } else {
                            for (int var57 = 0; ~var57 > ~class240.field3896; ++var57) {
                                int var58 = var10[var57];
                                int var59 = var14[var57];
                                int var60 = var13[var57];
                                var6[var57] = var58 != 0 ? (var8[var57] << 12) / var58 : 4096;
                                var11[var57] = ~var60 != -1 ? (var9[var57] << 12) / var60 : 4096;
                                var7[var57] = ~var59 != -1 ? (var12[var57] << 12) / var59 : 4096;
                            }
                        }
                    } else {
                        for (int var61 = 0; class240.field3896 > var61; ++var61) {
                            var6[var61] = var8[var61] * var10[var61] >> 12;
                            var11[var61] = var9[var61] * var13[var61] >> 12;
                            var7[var61] = var12[var61] * var14[var61] >> 12;
                        }
                    }
                } else {
                    for (int var62 = 0; ~var62 > ~class240.field3896; ++var62) {
                        var6[var62] = var8[var62] + -var10[var62];
                        var11[var62] = var9[var62] + -var13[var62];
                        var7[var62] = var12[var62] + -var14[var62];
                    }
                }
            } else {
                for (int var63 = 0; ~var63 > ~class240.field3896; ++var63) {
                    var6[var63] = var8[var63] + var10[var63];
                    var11[var63] = var9[var63] - -var13[var63];
                    var7[var63] = var12[var63] + var14[var63];
                }
            }
        }
        return var3;
    }

    @OriginalMember(owner = "client!ml", name = "a", descriptor = "(IB)[I")
    public final int[] method1(int arg0, byte arg1) {
        ++field4680;
        if (arg1 != 57) {
            return null;
        } else {
            int[] var3 = super.field2488.method15((byte) 122, arg0);
            if (super.field2488.field42) {
                int[] var4 = this.method1085(0, arg0, (byte) 122);
                int[] var5 = this.method1085(1, arg0, (byte) 73);
                int var6 = this.field4678;
                if (var6 != 1) {
                    if (var6 != 2) {
                        if (var6 != 3) {
                            if (var6 != 4) {
                                if (var6 != 5) {
                                    if (var6 != 6) {
                                        if (var6 != 7) {
                                            if (~var6 != -9) {
                                                if (var6 != 9) {
                                                    if (var6 != 10) {
                                                        if (var6 != 11) {
                                                            if (~var6 == -13) {
                                                                for (int var7 = 0; ~class240.field3896 < ~var7; ++var7) {
                                                                    int var8 = var4[var7];
                                                                    int var9 = var5[var7];
                                                                    var3[var7] = var8 - (var8 * var9 >> 11) + var9;
                                                                }
                                                            }
                                                        } else {
                                                            for (int var10 = 0; ~class240.field3896 < ~var10; ++var10) {
                                                                int var11 = var5[var10];
                                                                int var12 = var4[var10];
                                                                var3[var10] = ~var11 <= ~var12 ? -var12 + var11 : -var11 + var12;
                                                            }
                                                        }
                                                    } else {
                                                        for (int var13 = 0; ~var13 > ~class240.field3896; ++var13) {
                                                            int var14 = var5[var13];
                                                            int var15 = var4[var13];
                                                            var3[var13] = var15 > var14 ? var15 : var14;
                                                        }
                                                    }
                                                } else {
                                                    for (int var16 = 0; class240.field3896 > var16; ++var16) {
                                                        int var17 = var5[var16];
                                                        int var18 = var4[var16];
                                                        var3[var16] = var18 >= var17 ? var17 : var18;
                                                    }
                                                }
                                            } else {
                                                for (int var19 = 0; ~class240.field3896 < ~var19; ++var19) {
                                                    int var20 = var4[var19];
                                                    var3[var19] = ~var20 == -1 ? 0 : 4096 - (4096 - var5[var19] << 12) / var20;
                                                }
                                            }
                                        } else {
                                            for (int var21 = 0; class240.field3896 > var21; ++var21) {
                                                int var22 = var4[var21];
                                                var3[var21] = var22 != 4096 ? (var5[var21] << 12) / (-var22 + 4096) : 4096;
                                            }
                                        }
                                    } else {
                                        for (int var23 = 0; class240.field3896 > var23; ++var23) {
                                            int var24 = var5[var23];
                                            var3[var23] = ~var24 <= -2049 ? -((-var4[var23] + 4096) * (-var24 + 4096) >> 11) + 4096 : var4[var23] * var24 >> 11;
                                        }
                                    }
                                } else {
                                    for (int var25 = 0; class240.field3896 > var25; ++var25) {
                                        var3[var25] = 4096 - ((4096 - var4[var25]) * (-var5[var25] + 4096) >> 12);
                                    }
                                }
                            } else {
                                for (int var26 = 0; ~class240.field3896 < ~var26; ++var26) {
                                    int var27 = var5[var26];
                                    var3[var26] = var27 == 0 ? 4096 : (var4[var26] << 12) / var27;
                                }
                            }
                        } else {
                            for (int var28 = 0; class240.field3896 > var28; ++var28) {
                                var3[var28] = var4[var28] * var5[var28] >> 12;
                            }
                        }
                    } else {
                        for (int var29 = 0; ~var29 > ~class240.field3896; ++var29) {
                            var3[var29] = var4[var29] + -var5[var29];
                        }
                    }
                } else {
                    for (int var30 = 0; ~class240.field3896 < ~var30; ++var30) {
                        var3[var30] = var4[var30] + var5[var30];
                    }
                }
            }
            return var3;
        }
    }

    @OriginalMember(owner = "client!ml", name = "a", descriptor = "(I[BZ[Lrh;IB)V")
    public static final void method1978(int arg0, byte[] arg1, boolean arg2, class47[] arg3, int arg4, byte arg5) {
        ++field4677;
        class88 var6 = new class88(arg1);
        int var7 = -1;
        if (arg5 != -26) {
            field4682 = null;
        }
        while (true) {
            int var8 = var6.method629(-113);
            if (var8 == 0) {
                return;
            }
            int var9 = 0;
            var7 += var8;
            while (true) {
                int var10 = var6.method666(false);
                if (~var10 == -1) {
                    break;
                }
                var9 += var10 - 1;
                int var11 = var9 & 63;
                int var12 = (var9 & 4068) >> 6;
                int var13 = var9 >> 12;
                int var14 = var6.method633(116);
                int var15 = var14 >> 2;
                int var16 = 3 & var14;
                int var17 = arg0 + var11;
                int var18 = arg4 + var12;
                if (~var18 < -1 && ~var17 < -1 && var18 < 103 && ~var17 > -104) {
                    class47 var19 = null;
                    if (!arg2) {
                        int var20 = var13;
                        if ((2 & class181.field2827[1][var18][var17]) == 2) {
                            var20 = var13 - 1;
                        }
                        if (~var20 <= -1) {
                            var19 = arg3[var20];
                        }
                    }
                    class82.method584(arg2, var7, var13, var18, var16, var17, true, !arg2, var13, var19, var15);
                }
            }
        }
    }

    @OriginalMember(owner = "client!ml", name = "a", descriptor = "(Lai;II)V")
    public final void method2(class88 arg0, int arg1, int arg2) {
        if (~arg2 != -1) {
            if (~arg2 == -2) {
                super.field2494 = ~arg0.method633(124) == -2;
            }
        } else {
            this.field4678 = arg0.method633(76);
        }
        ++field4681;
        if (arg1 <= 11) {
            method1979(-101, -6);
        }
    }

    @OriginalMember(owner = "client!ml", name = "d", descriptor = "(II)I")
    public static final int method1979(int arg0, int arg1) {
        ++field4686;
        if (arg0 < 21) {
            method1979(-114, -42);
        }
        return arg1 >>> 7;
    }
}
