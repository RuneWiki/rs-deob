import org.openrs2.deob.annotation.OriginalClass;
import org.openrs2.deob.annotation.OriginalMember;

@OriginalClass("client!x")
public class Entity extends DoublyLinkable {

    @OriginalMember(owner = "client!x", name = "j", descriptor = "I")
    public int field397 = 1000;

    @OriginalMember(owner = "client!x", name = "k", descriptor = "Z")
    public static boolean field398;

    @OriginalMember(owner = "client!x", name = "i", descriptor = "[Ln;")
    public VertexNormal[] field396;

    @OriginalMember(owner = "client!x", name = "a", descriptor = "(IIIIIIIII)V")
    public void method108(int arg0, int arg1, int arg2, int arg3, int arg4, int arg5, int arg6, int arg7, int arg8) {
        Model var10 = this.method109(-21173);
        if (var10 != null) {
            this.field397 = var10.field397;
            var10.method108(arg0, arg1, arg2, arg3, arg4, arg5, arg6, arg7, arg8);
        }
    }

    @OriginalMember(owner = "client!x", name = "a", descriptor = "(I)Leb;")
    public Model method109(int arg0) {
        if (arg0 != -21173) {
            throw new NullPointerException();
        }
        return null;
    }
}
