import java.io.ByteArrayInputStream;
import java.io.IOException;
import java.io.InputStream;
import java.io.OutputStream;
import java.net.Socket;
import java.util.zip.CRC32;
import java.util.zip.GZIPInputStream;
import org.openrs2.deob.annotation.OriginalClass;
import org.openrs2.deob.annotation.OriginalMember;
import sign.signlink;

@OriginalClass("client!ub")
public class OnDemand extends OnDemandProvider implements Runnable {

    @OriginalMember(owner = "client!ub", name = "b", descriptor = "I")
    private int field806 = 749;

    @OriginalMember(owner = "client!ub", name = "d", descriptor = "I")
    private int field808 = 633;

    @OriginalMember(owner = "client!ub", name = "e", descriptor = "Z")
    private boolean field809 = false;

    @OriginalMember(owner = "client!ub", name = "f", descriptor = "[[I")
    private int[][] field810 = new int[4][];

    @OriginalMember(owner = "client!ub", name = "g", descriptor = "[[I")
    private int[][] field811 = new int[4][];

    @OriginalMember(owner = "client!ub", name = "h", descriptor = "[[B")
    private byte[][] field812 = new byte[4][];

    @OriginalMember(owner = "client!ub", name = "q", descriptor = "Z")
    private boolean field821 = true;

    @OriginalMember(owner = "client!ub", name = "s", descriptor = "Ljava/util/zip/CRC32;")
    private CRC32 field823 = new CRC32();

    @OriginalMember(owner = "client!ub", name = "t", descriptor = "Z")
    private boolean field824 = false;

    @OriginalMember(owner = "client!ub", name = "w", descriptor = "Lpb;")
    private DoublyLinkList field827 = new DoublyLinkList(-5693);

    @OriginalMember(owner = "client!ub", name = "x", descriptor = "Lob;")
    private LinkList field828 = new LinkList(-794);

    @OriginalMember(owner = "client!ub", name = "y", descriptor = "Lob;")
    private LinkList field829 = new LinkList(-794);

    @OriginalMember(owner = "client!ub", name = "z", descriptor = "Lob;")
    private LinkList field830 = new LinkList(-794);

    @OriginalMember(owner = "client!ub", name = "A", descriptor = "Lob;")
    private LinkList field831 = new LinkList(-794);

    @OriginalMember(owner = "client!ub", name = "B", descriptor = "Lob;")
    private LinkList field832 = new LinkList(-794);

    @OriginalMember(owner = "client!ub", name = "C", descriptor = "Ljava/lang/String;")
    public String field833 = "";

    @OriginalMember(owner = "client!ub", name = "M", descriptor = "[B")
    private byte[] field843 = new byte[500];

    @OriginalMember(owner = "client!ub", name = "N", descriptor = "[B")
    private byte[] field844 = new byte[65000];

    @OriginalMember(owner = "client!ub", name = "c", descriptor = "I")
    private int field807;

    @OriginalMember(owner = "client!ub", name = "i", descriptor = "I")
    private int field813;

    @OriginalMember(owner = "client!ub", name = "u", descriptor = "I")
    private int field825;

    @OriginalMember(owner = "client!ub", name = "v", descriptor = "I")
    private int field826;

    @OriginalMember(owner = "client!ub", name = "D", descriptor = "I")
    private int field834;

    @OriginalMember(owner = "client!ub", name = "E", descriptor = "I")
    private int field835;

    @OriginalMember(owner = "client!ub", name = "I", descriptor = "I")
    public int field839;

    @OriginalMember(owner = "client!ub", name = "K", descriptor = "I")
    private int field841;

    @OriginalMember(owner = "client!ub", name = "L", descriptor = "I")
    private int field842;

    @OriginalMember(owner = "client!ub", name = "O", descriptor = "I")
    private int field845;

    @OriginalMember(owner = "client!ub", name = "P", descriptor = "I")
    private int field846;

    @OriginalMember(owner = "client!ub", name = "R", descriptor = "I")
    public int field848;

    @OriginalMember(owner = "client!ub", name = "Q", descriptor = "J")
    private long field847;

    @OriginalMember(owner = "client!ub", name = "J", descriptor = "Lmb;")
    private OnDemandRequest field840;

    @OriginalMember(owner = "client!ub", name = "r", descriptor = "Lclient;")
    private client field822;

    @OriginalMember(owner = "client!ub", name = "G", descriptor = "Ljava/io/InputStream;")
    private InputStream field837;

    @OriginalMember(owner = "client!ub", name = "H", descriptor = "Ljava/io/OutputStream;")
    private OutputStream field838;

    @OriginalMember(owner = "client!ub", name = "F", descriptor = "Ljava/net/Socket;")
    private Socket field836;

    @OriginalMember(owner = "client!ub", name = "a", descriptor = "Z")
    private static boolean field805;

    @OriginalMember(owner = "client!ub", name = "j", descriptor = "[B")
    private byte[] field814;

    @OriginalMember(owner = "client!ub", name = "k", descriptor = "[I")
    private int[] field815;

    @OriginalMember(owner = "client!ub", name = "l", descriptor = "[I")
    private int[] field816;

    @OriginalMember(owner = "client!ub", name = "m", descriptor = "[I")
    private int[] field817;

    @OriginalMember(owner = "client!ub", name = "n", descriptor = "[I")
    private int[] field818;

    @OriginalMember(owner = "client!ub", name = "o", descriptor = "[I")
    private int[] field819;

    @OriginalMember(owner = "client!ub", name = "p", descriptor = "[I")
    private int[] field820;

    @OriginalMember(owner = "client!ub", name = "a", descriptor = "(Lxb;Lclient;)V")
    public final void method270(Jagfile arg0, client arg1) {
        String[] var3 = new String[] { "model_version", "anim_version", "midi_version", "map_version" };
        for (int var4 = 0; var4 < 4; var4++) {
            byte[] var26 = arg0.method299(var3[var4], null);
            int var27 = var26.length / 2;
            Packet var28 = new Packet(var26, field805);
            this.field810[var4] = new int[var27];
            this.field812[var4] = new byte[var27];
            for (int var29 = 0; var29 < var27; var29++) {
                this.field810[var4][var29] = var28.method231();
            }
        }
        String[] var5 = new String[] { "model_crc", "anim_crc", "midi_crc", "map_crc" };
        for (int var6 = 0; var6 < 4; var6++) {
            byte[] var22 = arg0.method299(var5[var6], null);
            int var23 = var22.length / 4;
            Packet var24 = new Packet(var22, field805);
            this.field811[var6] = new int[var23];
            for (int var25 = 0; var25 < var23; var25++) {
                this.field811[var6][var25] = var24.method234();
            }
        }
        byte[] var7 = arg0.method299("model_index", null);
        int var8 = this.field810[0].length;
        this.field814 = new byte[var8];
        for (int var9 = 0; var9 < var8; var9++) {
            if (var9 < var7.length) {
                this.field814[var9] = var7[var9];
            } else {
                this.field814[var9] = 0;
            }
        }
        byte[] var10 = arg0.method299("map_index", null);
        Packet var11 = new Packet(var10, field805);
        int var12 = var10.length / 7;
        this.field815 = new int[var12];
        this.field816 = new int[var12];
        this.field817 = new int[var12];
        this.field818 = new int[var12];
        for (int var13 = 0; var13 < var12; var13++) {
            this.field815[var13] = var11.method231();
            this.field816[var13] = var11.method231();
            this.field817[var13] = var11.method231();
            this.field818[var13] = var11.method229();
        }
        byte[] var14 = arg0.method299("anim_index", null);
        Packet var15 = new Packet(var14, field805);
        int var16 = var14.length / 2;
        this.field819 = new int[var16];
        for (int var17 = 0; var17 < var16; var17++) {
            this.field819[var17] = var15.method231();
        }
        byte[] var18 = arg0.method299("midi_index", null);
        Packet var19 = new Packet(var18, field805);
        int var20 = var18.length;
        this.field820 = new int[var20];
        for (int var21 = 0; var21 < var20; var21++) {
            this.field820[var21] = var19.method229();
        }
        this.field822 = arg1;
        this.field821 = true;
        this.field822.method12(this, 2);
    }

    @OriginalMember(owner = "client!ub", name = "a", descriptor = "()V")
    public final void method271() {
        this.field821 = false;
    }

    @OriginalMember(owner = "client!ub", name = "a", descriptor = "(II)I")
    public final int method272(int arg0, int arg1) {
        return arg0 == 2136 ? this.field810[arg1].length : 2;
    }

    @OriginalMember(owner = "client!ub", name = "b", descriptor = "(I)I")
    public final int method273(int arg0) {
        if (arg0 != 0) {
            field805 = !field805;
        }
        return this.field819.length;
    }

    @OriginalMember(owner = "client!ub", name = "a", descriptor = "(IIII)I")
    public final int method274(int arg0, int arg1, int arg2, int arg3) {
        int var5 = (arg3 << 8) + arg1;
        if (arg2 != -28157) {
            return 2;
        }
        for (int var6 = 0; var6 < this.field815.length; var6++) {
            if (this.field815[var6] == var5) {
                if (arg0 == 0) {
                    return this.field816[var6];
                }
                return this.field817[var6];
            }
        }
        return -1;
    }

    @OriginalMember(owner = "client!ub", name = "a", descriptor = "(IZ)V")
    public final void method275(int arg0, boolean arg1) {
        if (arg0 != 0) {
            for (int var3 = 1; var3 > 0; var3++) {
            }
        }
        int var4 = this.field815.length;
        for (int var5 = 0; var5 < var4; var5++) {
            if (arg1 || this.field818[var5] != 0) {
                this.method282(349, (byte) 2, 3, this.field817[var5]);
                this.method282(349, (byte) 2, 3, this.field816[var5]);
            }
        }
    }

    @OriginalMember(owner = "client!ub", name = "a", descriptor = "(BI)Z")
    public final boolean method276(byte arg0, int arg1) {
        for (int var3 = 0; var3 < this.field815.length; var3++) {
            if (this.field817[var3] == arg1) {
                return true;
            }
        }
        if (arg0 != 9) {
            this.field807 = 310;
        }
        return false;
    }

    @OriginalMember(owner = "client!ub", name = "b", descriptor = "(II)I")
    public final int method277(int arg0, int arg1) {
        return arg1 == 3 ? this.field814[arg0] & 0xFF : this.field806;
    }

    @OriginalMember(owner = "client!ub", name = "a", descriptor = "(ZI)Z")
    public final boolean method278(boolean arg0, int arg1) {
        if (arg0) {
            for (int var3 = 1; var3 > 0; var3++) {
            }
        }
        return this.field820[arg1] == 1;
    }

    @OriginalMember(owner = "client!ub", name = "a", descriptor = "(I)V")
    public final void method269(int arg0) {
        this.method279(0, arg0);
    }

    @OriginalMember(owner = "client!ub", name = "c", descriptor = "(II)V")
    public final void method279(int arg0, int arg1) {
        if (arg0 < 0 || arg0 > this.field810.length || arg1 < 0 || arg1 > this.field810[arg0].length || this.field810[arg0][arg1] == 0) {
            return;
        }
        DoublyLinkList var3 = this.field827;
        synchronized (this.field827) {
            for (OnDemandRequest var4 = (OnDemandRequest) this.field827.method255(); var4 != null; var4 = (OnDemandRequest) this.field827.method256(true)) {
                if (var4.field725 == arg0 && var4.field726 == arg1) {
                    return;
                }
            }
            OnDemandRequest var5 = new OnDemandRequest();
            var5.field725 = arg0;
            var5.field726 = arg1;
            var5.field729 = true;
            LinkList var6 = this.field828;
            synchronized (this.field828) {
                this.field828.method245(var5);
            }
            this.field827.method253(var5);
        }
    }

    @OriginalMember(owner = "client!ub", name = "b", descriptor = "()I")
    public final int method280() {
        DoublyLinkList var1 = this.field827;
        synchronized (this.field827) {
            return this.field827.method257();
        }
    }

    @OriginalMember(owner = "client!ub", name = "c", descriptor = "()Lmb;")
    public final OnDemandRequest method281() {
        LinkList var1 = this.field831;
        OnDemandRequest var2;
        synchronized (this.field831) {
            var2 = (OnDemandRequest) this.field831.method247();
        }
        if (var2 == null) {
            return null;
        }
        DoublyLinkList var3 = this.field827;
        synchronized (this.field827) {
            var2.method107();
        }
        if (var2.field727 == null) {
            return var2;
        }
        int var4 = 0;
        try {
            GZIPInputStream var5 = new GZIPInputStream(new ByteArrayInputStream(var2.field727));
            while (true) {
                if (this.field844.length == var4) {
                    throw new RuntimeException("buffer overflow!");
                }
                int var6 = var5.read(this.field844, var4, this.field844.length - var4);
                if (var6 == -1) {
                    break;
                }
                var4 += var6;
            }
        } catch (IOException var10) {
            throw new RuntimeException("error unzipping");
        }
        var2.field727 = new byte[var4];
        for (int var7 = 0; var7 < var4; var7++) {
            var2.field727[var7] = this.field844[var7];
        }
        return var2;
    }

    @OriginalMember(owner = "client!ub", name = "a", descriptor = "(IBII)V")
    public final void method282(int arg0, byte arg1, int arg2, int arg3) {
        if (this.field822.field1305[0] == null || this.field810[arg2][arg3] == 0) {
            return;
        }
        byte[] var5 = this.field822.field1305[arg2 + 1].method291(false, arg3);
        int var6 = 83 / arg0;
        if (this.method289((byte) 3, this.field811[arg2][arg3], var5, this.field810[arg2][arg3])) {
            return;
        }
        this.field812[arg2][arg3] = arg1;
        if (arg1 > this.field813) {
            this.field813 = arg1;
        }
        this.field835++;
    }

    @OriginalMember(owner = "client!ub", name = "a", descriptor = "(Z)V")
    public final void method283(boolean arg0) {
        if (arg0) {
            for (int var2 = 1; var2 > 0; var2++) {
            }
        }
        LinkList var3 = this.field832;
        synchronized (this.field832) {
            this.field832.method252();
        }
    }

    @OriginalMember(owner = "client!ub", name = "a", descriptor = "(III)V")
    public final void method284(int arg0, int arg1, int arg2) {
        if (this.field822.field1305[0] == null || (this.field810[arg2][arg1] == 0 || (this.field812[arg2][arg1] == 0 || this.field813 == 0))) {
            return;
        }
        OnDemandRequest var4 = new OnDemandRequest();
        var4.field725 = arg2;
        var4.field726 = arg1;
        var4.field729 = false;
        LinkList var5 = this.field832;
        synchronized (this.field832) {
            this.field832.method245(var4);
        }
        if (this.field808 == arg0) {
            ;
        }
    }

    @OriginalMember(owner = "client!ub", name = "run", descriptor = "()V")
    public final void run() {
        try {
            while (this.field821) {
                this.field848++;
                byte var1 = 20;
                if (this.field813 == 0 && this.field822.field1305[0] != null) {
                    var1 = 50;
                }
                try {
                    Thread.sleep((long) var1);
                } catch (Exception var9) {
                }
                this.field824 = true;
                for (int var2 = 0; var2 < 100 && this.field824; var2++) {
                    this.field824 = false;
                    this.method285(-225);
                    this.method286((byte) 41);
                    if (this.field825 == 0 && var2 >= 5) {
                        break;
                    }
                    this.method287(-715);
                    if (this.field837 != null) {
                        this.method288(-21071);
                    }
                }
                boolean var3 = false;
                for (OnDemandRequest var4 = (OnDemandRequest) this.field830.method248(); var4 != null; var4 = (OnDemandRequest) this.field830.method250(true)) {
                    if (var4.field729) {
                        var3 = true;
                        var4.field728++;
                        if (var4.field728 > 50) {
                            var4.field728 = 0;
                            this.method290(var4, this.field809);
                        }
                    }
                }
                if (!var3) {
                    for (OnDemandRequest var5 = (OnDemandRequest) this.field830.method248(); var5 != null; var5 = (OnDemandRequest) this.field830.method250(true)) {
                        var3 = true;
                        var5.field728++;
                        if (var5.field728 > 50) {
                            var5.field728 = 0;
                            this.method290(var5, this.field809);
                        }
                    }
                }
                if (var3) {
                    this.field845++;
                    if (this.field845 > 750) {
                        try {
                            this.field836.close();
                        } catch (Exception var8) {
                        }
                        this.field836 = null;
                        this.field837 = null;
                        this.field838 = null;
                        this.field842 = 0;
                    }
                } else {
                    this.field845 = 0;
                    this.field833 = "";
                }
                if (this.field822.field1277 && this.field836 != null && this.field838 != null && (this.field813 > 0 || this.field822.field1305[0] == null)) {
                    this.field846++;
                    if (this.field846 > 500) {
                        this.field846 = 0;
                        this.field843[0] = 0;
                        this.field843[1] = 0;
                        this.field843[2] = 0;
                        this.field843[3] = 10;
                        try {
                            this.field838.write(this.field843, 0, 4);
                        } catch (IOException var7) {
                            this.field845 = 5000;
                        }
                    }
                }
            }
        } catch (Exception var10) {
            signlink.reporterror("od_ex " + var10.getMessage());
        }
    }

    @OriginalMember(owner = "client!ub", name = "c", descriptor = "(I)V")
    private final void method285(int arg0) {
        while (arg0 >= 0) {
            for (int var2 = 1; var2 > 0; var2++) {
            }
        }
        LinkList var3 = this.field828;
        OnDemandRequest var4;
        synchronized (this.field828) {
            var4 = (OnDemandRequest) this.field828.method247();
        }
        while (var4 != null) {
            this.field824 = true;
            byte[] var5 = null;
            if (this.field822.field1305[0] != null) {
                var5 = this.field822.field1305[var4.field725 + 1].method291(false, var4.field726);
            }
            if (!this.method289((byte) 3, this.field811[var4.field725][var4.field726], var5, this.field810[var4.field725][var4.field726])) {
                var5 = null;
            }
            LinkList var6 = this.field828;
            synchronized (this.field828) {
                if (var5 == null) {
                    this.field829.method245(var4);
                } else {
                    var4.field727 = var5;
                    LinkList var7 = this.field831;
                    synchronized (this.field831) {
                        this.field831.method245(var4);
                    }
                }
                var4 = (OnDemandRequest) this.field828.method247();
            }
        }
    }

    @OriginalMember(owner = "client!ub", name = "a", descriptor = "(B)V")
    private final void method286(byte arg0) {
        this.field825 = 0;
        this.field826 = 0;
        OnDemandRequest var2 = (OnDemandRequest) this.field830.method248();
        if (arg0 != 41) {
            return;
        }
        while (var2 != null) {
            if (var2.field729) {
                this.field825++;
            } else {
                this.field826++;
            }
            var2 = (OnDemandRequest) this.field830.method250(true);
        }
        while (this.field825 < 10) {
            OnDemandRequest var3 = (OnDemandRequest) this.field829.method247();
            if (var3 == null) {
                break;
            }
            if (this.field812[var3.field725][var3.field726] != 0) {
                this.field834++;
            }
            this.field812[var3.field725][var3.field726] = 0;
            this.field830.method245(var3);
            this.field825++;
            this.method290(var3, this.field809);
            this.field824 = true;
        }
    }

    @OriginalMember(owner = "client!ub", name = "d", descriptor = "(I)V")
    private final void method287(int arg0) {
        if (arg0 >= 0) {
            this.field809 = !this.field809;
        }
        while (this.field825 == 0) {
            if (this.field826 >= 10 || this.field813 == 0) {
                return;
            }
            LinkList var2 = this.field832;
            OnDemandRequest var3;
            synchronized (this.field832) {
                var3 = (OnDemandRequest) this.field832.method247();
            }
            while (var3 != null) {
                if (this.field812[var3.field725][var3.field726] != 0) {
                    this.field812[var3.field725][var3.field726] = 0;
                    this.field830.method245(var3);
                    this.method290(var3, this.field809);
                    this.field824 = true;
                    if (this.field834 < this.field835) {
                        this.field834++;
                    }
                    this.field833 = "Loading extra files - " + this.field834 * 100 / this.field835 + "%";
                    this.field826++;
                    if (this.field826 == 10) {
                        return;
                    }
                }
                LinkList var4 = this.field832;
                synchronized (this.field832) {
                    var3 = (OnDemandRequest) this.field832.method247();
                }
            }
            for (int var5 = 0; var5 < 4; var5++) {
                byte[] var6 = this.field812[var5];
                int var7 = var6.length;
                for (int var8 = 0; var8 < var7; var8++) {
                    if (var6[var8] == this.field813) {
                        var6[var8] = 0;
                        OnDemandRequest var9 = new OnDemandRequest();
                        var9.field725 = var5;
                        var9.field726 = var8;
                        var9.field729 = false;
                        this.field830.method245(var9);
                        this.method290(var9, this.field809);
                        this.field824 = true;
                        if (this.field834 < this.field835) {
                            this.field834++;
                        }
                        this.field833 = "Loading extra files - " + this.field834 * 100 / this.field835 + "%";
                        this.field826++;
                        if (this.field826 == 10) {
                            return;
                        }
                    }
                }
            }
            this.field813--;
        }
    }

    @OriginalMember(owner = "client!ub", name = "e", descriptor = "(I)V")
    private final void method288(int arg0) {
        if (arg0 != -21071) {
            return;
        }
        try {
            int var2 = this.field837.available();
            if (this.field842 == 0 && var2 >= 6) {
                this.field824 = true;
                for (int var3 = 0; var3 < 6; var3 += this.field837.read(this.field843, var3, 6 - var3)) {
                }
                int var4 = this.field843[0] & 0xFF;
                int var5 = ((this.field843[1] & 0xFF) << 8) + (this.field843[2] & 0xFF);
                int var6 = ((this.field843[3] & 0xFF) << 8) + (this.field843[4] & 0xFF);
                int var7 = this.field843[5] & 0xFF;
                this.field840 = null;
                for (OnDemandRequest var8 = (OnDemandRequest) this.field830.method248(); var8 != null; var8 = (OnDemandRequest) this.field830.method250(true)) {
                    if (var8.field725 == var4 && var8.field726 == var5) {
                        this.field840 = var8;
                    }
                    if (this.field840 != null) {
                        var8.field728 = 0;
                    }
                }
                if (this.field840 != null) {
                    this.field845 = 0;
                    if (var6 == 0) {
                        signlink.reporterror("Rej: " + var4 + "," + var5);
                        this.field840.field727 = null;
                        if (this.field840.field729) {
                            LinkList var9 = this.field831;
                            synchronized (this.field831) {
                                this.field831.method245(this.field840);
                            }
                        } else {
                            this.field840.method106();
                        }
                        this.field840 = null;
                    } else {
                        if (this.field840.field727 == null && var7 == 0) {
                            this.field840.field727 = new byte[var6];
                        }
                        if (this.field840.field727 == null && var7 != 0) {
                            throw new IOException("missing start of file");
                        }
                    }
                }
                this.field841 = var7 * 500;
                this.field842 = 500;
                if (this.field842 > var6 - var7 * 500) {
                    this.field842 = var6 - var7 * 500;
                }
            }
            if (this.field842 > 0 && var2 >= this.field842) {
                this.field824 = true;
                byte[] var10 = this.field843;
                int var11 = 0;
                if (this.field840 != null) {
                    var10 = this.field840.field727;
                    var11 = this.field841;
                }
                for (int var12 = 0; var12 < this.field842; var12 += this.field837.read(var10, var11 + var12, this.field842 - var12)) {
                }
                if (this.field842 + this.field841 >= var10.length && this.field840 != null) {
                    if (this.field822.field1305[0] != null) {
                        this.field822.field1305[this.field840.field725 + 1].method292(var10.length, this.field840.field726, 0, var10);
                    }
                    if (!this.field840.field729 && this.field840.field725 == 3) {
                        this.field840.field729 = true;
                        this.field840.field725 = 93;
                    }
                    if (this.field840.field729) {
                        LinkList var13 = this.field831;
                        synchronized (this.field831) {
                            this.field831.method245(this.field840);
                        }
                    } else {
                        this.field840.method106();
                    }
                }
                this.field842 = 0;
            }
        } catch (IOException var18) {
            try {
                this.field836.close();
            } catch (Exception var15) {
            }
            this.field836 = null;
            this.field837 = null;
            this.field838 = null;
            this.field842 = 0;
        }
    }

    @OriginalMember(owner = "client!ub", name = "a", descriptor = "(BI[BI)Z")
    private final boolean method289(byte arg0, int arg1, byte[] arg2, int arg3) {
        if (arg2 == null || arg2.length < 2) {
            return false;
        }
        int var5 = arg2.length - 2;
        int var6 = ((arg2[var5] & 0xFF) << 8) + (arg2[var5 + 1] & 0xFF);
        this.field823.reset();
        this.field823.update(arg2, 0, var5);
        int var7 = (int) this.field823.getValue();
        if (arg0 == 3) {
            boolean var8 = false;
        } else {
            for (int var9 = 1; var9 > 0; var9++) {
            }
        }
        if (arg3 == var6) {
            return arg1 == var7;
        } else {
            return false;
        }
    }

    @OriginalMember(owner = "client!ub", name = "a", descriptor = "(Lmb;Z)V")
    private final void method290(OnDemandRequest arg0, boolean arg1) {
        if (arg1) {
            for (int var3 = 1; var3 > 0; var3++) {
            }
        }
        try {
            if (this.field836 == null) {
                long var4 = System.currentTimeMillis();
                if (var4 - this.field847 < 4000L) {
                    return;
                }
                this.field847 = var4;
                this.field836 = this.field822.method485(client.field1390 + 43594);
                this.field837 = this.field836.getInputStream();
                this.field838 = this.field836.getOutputStream();
                this.field838.write(15);
                for (int var6 = 0; var6 < 8; var6++) {
                    this.field837.read();
                }
                this.field845 = 0;
            }
            this.field843[0] = (byte) arg0.field725;
            this.field843[1] = (byte) (arg0.field726 >> 8);
            this.field843[2] = (byte) arg0.field726;
            if (arg0.field729) {
                this.field843[3] = 2;
            } else if (this.field822.field1277) {
                this.field843[3] = 0;
            } else {
                this.field843[3] = 1;
            }
            this.field838.write(this.field843, 0, 4);
            this.field846 = 0;
            this.field839 = -10000;
        } catch (IOException var9) {
            try {
                this.field836.close();
            } catch (Exception var8) {
            }
            this.field836 = null;
            this.field837 = null;
            this.field838 = null;
            this.field842 = 0;
            this.field839++;
        }
    }
}
