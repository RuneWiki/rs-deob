import org.openrs2.deob.annotation.OriginalClass;
import org.openrs2.deob.annotation.OriginalMember;

@OriginalClass("client!hm")
public class class188 {

    @OriginalMember(owner = "client!hm", name = "d", descriptor = "I")
    public int field2319;

    @OriginalMember(owner = "client!hm", name = "a", descriptor = "I")
    public int field2316;

    @OriginalMember(owner = "client!hm", name = "c", descriptor = "I")
    public int field2318;

    @OriginalMember(owner = "client!hm", name = "b", descriptor = "I")
    public int field2317;

    @OriginalMember(owner = "client!hm", name = "<init>", descriptor = "()V", line = 6)
    public class188() {
    }

    @OriginalMember(owner = "client!hm", name = "<init>", descriptor = "(Lhm;)V", line = 8)
    public class188(class188 arg0) {
        this.field2319 = arg0.field2319;
        this.field2316 = arg0.field2316;
        this.field2318 = arg0.field2318;
        this.field2317 = arg0.field2317;
    }
}
