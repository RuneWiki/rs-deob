import org.openrs2.deob.annotation.OriginalClass;
import org.openrs2.deob.annotation.OriginalMember;

@OriginalClass("client!al")
public abstract class class109 {

    @OriginalMember(owner = "client!al", name = "a", descriptor = "Lwv;")
    public static class37 field1519 = new class37();

    @OriginalMember(owner = "client!al", name = "d", descriptor = "I")
    public static int field1522 = 0;

    @OriginalMember(owner = "client!al", name = "c", descriptor = "Lg;")
    public static class166 field1521 = new class166(4);

    @OriginalMember(owner = "client!al", name = "b", descriptor = "I")
    public static int field1520;

    @OriginalMember(owner = "client!al", name = "d", descriptor = "(I)V", line = 22)
    public static void method819(int arg0) {
        field1521 = null;
        if (arg0 >= 56) {
            field1519 = null;
        }
    }

    @OriginalMember(owner = "client!al", name = "a", descriptor = "([I)V")
    public abstract void method809(int[] arg0);

    @OriginalMember(owner = "client!al", name = "a", descriptor = "(I)V")
    public abstract void method810(int arg0);

    @OriginalMember(owner = "client!al", name = "b", descriptor = "(I)V")
    public abstract void method811(int arg0);

    @OriginalMember(owner = "client!al", name = "a", descriptor = "(III[I)V")
    public abstract void method812(int arg0, int arg1, int arg2, int[] arg3);

    @OriginalMember(owner = "client!al", name = "a", descriptor = "()Lal;")
    public abstract class109 method813();

    @OriginalMember(owner = "client!al", name = "a", descriptor = "(III)V")
    public abstract void method814(int arg0, int arg1, int arg2);

    @OriginalMember(owner = "client!al", name = "b", descriptor = "(III[I)V")
    public abstract void method815(int arg0, int arg1, int arg2, int[] arg3);

    @OriginalMember(owner = "client!al", name = "a", descriptor = "(Lal;)V")
    public abstract void method816(class109 arg0);

    @OriginalMember(owner = "client!al", name = "c", descriptor = "(I)V")
    public abstract void method817(int arg0);

    @OriginalMember(owner = "client!al", name = "b", descriptor = "(III)V")
    public abstract void method818(int arg0, int arg1, int arg2);

    @OriginalMember(owner = "client!al", name = "c", descriptor = "(III[I)V")
    public abstract void method820(int arg0, int arg1, int arg2, int[] arg3);

    @OriginalMember(owner = "client!al", name = "e", descriptor = "(I)V")
    public abstract void method821(int arg0);

    @OriginalMember(owner = "client!al", name = "b", descriptor = "()V")
    public abstract void method822();

    @OriginalMember(owner = "client!al", name = "a", descriptor = "(IIIIII)V")
    public abstract void method823(int arg0, int arg1, int arg2, int arg3, int arg4, int arg5);

    @OriginalMember(owner = "client!al", name = "f", descriptor = "(I)V")
    public abstract void method824(int arg0);

    @OriginalMember(owner = "client!al", name = "g", descriptor = "(I)V")
    public abstract void method825(int arg0);
}
