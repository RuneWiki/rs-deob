import org.openrs2.deob.annotation.OriginalClass;
import org.openrs2.deob.annotation.OriginalMember;

@OriginalClass("client!gp")
public class class36 {

    @OriginalMember(owner = "client!gp", name = "b", descriptor = "Llg;")
    public static class284 field502 = new class284(11, 2);

    @OriginalMember(owner = "client!gp", name = "d", descriptor = "Lwo;")
    public static class522 field504 = new class522();

    @OriginalMember(owner = "client!gp", name = "e", descriptor = "Luf;")
    public static class310 field505 = new class310(71, -2);

    @OriginalMember(owner = "client!gp", name = "a", descriptor = "I")
    public static int field501;

    @OriginalMember(owner = "client!gp", name = "f", descriptor = "Lvh;")
    public static class240 field506;

    @OriginalMember(owner = "client!gp", name = "c", descriptor = "Ljava/lang/String;")
    public static String field503;

    @OriginalMember(owner = "client!gp", name = "a", descriptor = "(I)V")
    public static final void method236(int arg0) {
        if (arg0 != 73) {
            field504 = null;
        }
        field501++;
        if (class397.field5520.toLowerCase().indexOf("microsoft") != -1) {
            class195.field2784[221] = 43;
            class195.field2784[220] = 74;
            class195.field2784[187] = 27;
            class195.field2784[192] = 58;
            class195.field2784[219] = 42;
            class195.field2784[222] = 59;
            class195.field2784[186] = 57;
            class195.field2784[190] = 72;
            class195.field2784[191] = 73;
            class195.field2784[223] = 28;
            class195.field2784[189] = 26;
            class195.field2784[188] = 71;
            return;
        }
        class195.field2784[44] = 71;
        class195.field2784[92] = 74;
        class195.field2784[47] = 73;
        class195.field2784[91] = 42;
        class195.field2784[46] = 72;
        class195.field2784[45] = 26;
        if (class397.field5536 == null) {
            class195.field2784[222] = 59;
            class195.field2784[192] = 58;
        } else {
            class195.field2784[222] = 58;
            class195.field2784[520] = 59;
            class195.field2784[192] = 28;
        }
        class195.field2784[93] = 43;
        class195.field2784[61] = 27;
        class195.field2784[59] = 57;
    }

    @OriginalMember(owner = "client!gp", name = "a", descriptor = "(B)V")
    public static void method237(byte arg0) {
        field506 = null;
        field504 = null;
        field503 = null;
        field505 = null;
        field502 = null;
        if (arg0 < 86) {
            method237((byte) -52);
        }
    }
}
