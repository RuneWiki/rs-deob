import org.openrs2.deob.annotation.OriginalClass;
import org.openrs2.deob.annotation.OriginalMember;

@OriginalClass("client!efa")
public class class168 extends class66 {

    @OriginalMember(owner = "client!efa", name = "C", descriptor = "I")
    public static int field2828 = 0;

    @OriginalMember(owner = "client!efa", name = "q", descriptor = "I")
    public int field2816;

    @OriginalMember(owner = "client!efa", name = "r", descriptor = "I")
    public int field2817;

    @OriginalMember(owner = "client!efa", name = "s", descriptor = "I")
    public int field2818;

    @OriginalMember(owner = "client!efa", name = "t", descriptor = "I")
    public static int field2819;

    @OriginalMember(owner = "client!efa", name = "u", descriptor = "I")
    public static int field2820;

    @OriginalMember(owner = "client!efa", name = "v", descriptor = "I")
    public int field2821;

    @OriginalMember(owner = "client!efa", name = "w", descriptor = "I")
    public int field2822;

    @OriginalMember(owner = "client!efa", name = "x", descriptor = "I")
    public int field2823;

    @OriginalMember(owner = "client!efa", name = "y", descriptor = "I")
    public int field2824;

    @OriginalMember(owner = "client!efa", name = "A", descriptor = "I")
    public int field2826;

    @OriginalMember(owner = "client!efa", name = "B", descriptor = "I")
    public int field2827;

    @OriginalMember(owner = "client!efa", name = "D", descriptor = "I")
    public int field2829;

    @OriginalMember(owner = "client!efa", name = "E", descriptor = "I")
    public int field2830;

    @OriginalMember(owner = "client!efa", name = "G", descriptor = "I")
    public static int field2832;

    @OriginalMember(owner = "client!efa", name = "H", descriptor = "I")
    public int field2833;

    @OriginalMember(owner = "client!efa", name = "I", descriptor = "I")
    public int field2834;

    @OriginalMember(owner = "client!efa", name = "K", descriptor = "I")
    public int field2836;

    @OriginalMember(owner = "client!efa", name = "L", descriptor = "I")
    public int field2837;

    @OriginalMember(owner = "client!efa", name = "M", descriptor = "I")
    public static int field2838;

    @OriginalMember(owner = "client!efa", name = "O", descriptor = "I")
    public int field2840;

    @OriginalMember(owner = "client!efa", name = "P", descriptor = "I")
    public int field2841;

    @OriginalMember(owner = "client!efa", name = "N", descriptor = "Lqga;")
    public class169 field2839;

    @OriginalMember(owner = "client!efa", name = "z", descriptor = "Lse;")
    public class235 field2825;

    @OriginalMember(owner = "client!efa", name = "F", descriptor = "Lqr;")
    public class59 field2831;

    @OriginalMember(owner = "client!efa", name = "J", descriptor = "Lfm;")
    public class683 field2835;

    @OriginalMember(owner = "client!efa", name = "a", descriptor = "(I)V", line = 3)
    public final void method1356(int arg0) {
        this.field2839 = null;
        field2819++;
        this.field2835 = null;
        this.field2825 = null;
        this.field2831 = null;
        if (arg0 != 8) {
            method1359(-0.53703934F, -112, null, -92, -99, -70, -52, 21, (byte) 99, 80);
        }
    }

    @OriginalMember(owner = "client!efa", name = "b", descriptor = "(I)V", line = 22)
    public static final void method1357(int arg0) {
        int var1 = 37 / ((34 - arg0) / 38);
        class472.field6984.method749(false);
        field2838++;
        class61.field789.method17((byte) 43);
        class385.field5985.method2460(3);
        class213.field3537.method100(-122);
        class538.field7860.method3227((byte) -124);
        class91.field1083.method2848(-18043);
        class636.field8942.method2875((byte) 119);
        class65.field826.method3687(-78);
        class585.field8317.method718(45);
        class566.field8134.method3118(-50);
        class472.field6988.method2678((byte) 29);
        class584.field8299.method1887((byte) 67);
        class498.field7303.method3284(-1);
        class690.field9681.method1038(1);
        class610.field8680.method3569(64);
        class275.field4412.method1405((byte) 97);
        class254.field4046.method3588(-101);
        class357.field5693.method2625(true);
        class489.field7154.method1810(-100);
        class243.field3854.method2535(1001);
        class318.method2166(2048);
        class233.method1692((byte) -114);
        class115.method1039(-2135);
        class273.method1924((byte) 18);
        class511.method3101(false);
        class462.field6900.method563(0);
        class206.field3450.method563(0);
        class10.field173.method563(0);
        class300.field4688.method563(0);
        class142.field2397.method563(0);
    }

    @OriginalMember(owner = "client!efa", name = "a", descriptor = "(B)V", line = 64)
    public static final void method1358(byte arg0) {
        field2820++;
        if (class433.method2753(-124)) {
            if (class589.field8381 == null) {
                class700.method3953(46);
            }
            class385.field5986 = 0;
            class191.field3165 = true;
        }
        if (arg0 >= -27) {
            field2828 = -35;
        }
    }

    @OriginalMember(owner = "client!efa", name = "a", descriptor = "(FI[FIIIIIBI)V", line = 105)
    public static final void method1359(float arg0, int arg1, float[] arg2, int arg3, int arg4, int arg5, int arg6, int arg7, byte arg8, int arg9) {
        field2832++;
        int var10 = arg4 - arg9;
        int var11 = arg3 - arg1;
        if (arg8 <= 105) {
            field2828 = 99;
        }
        int var12 = arg5 - arg7;
        float var13 = arg2[2] * (float) var11 + arg2[1] * (float) var12 + arg2[0] * (float) var10;
        float var14 = arg2[5] * (float) var11 + arg2[3] * (float) var10 + arg2[4] * (float) var12;
        float var15 = arg2[8] * (float) var11 + arg2[6] * (float) var10 + arg2[7] * (float) var12;
        float var16 = (float) Math.sqrt((double) (var15 * var15 + var13 * var13 + var14 * var14));
        float var17 = (float) Math.atan2((double) var13, (double) var15) / 6.2831855F + 0.5F;
        float var18 = arg0 + (float) Math.asin((double) (var14 / var16)) / 3.1415927F + 0.5F;
        if (arg6 == 1) {
            float var19 = var17;
            var17 = -var18;
            var18 = var19;
        } else if (arg6 == 2) {
            var17 = -var17;
            var18 = -var18;
        } else if (arg6 == 3) {
            float var20 = var17;
            var17 = var18;
            var18 = -var20;
        }
        class78.field994 = var18;
        class439.field6689 = var17;
    }
}
