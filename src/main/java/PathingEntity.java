import org.openrs2.deob.annotation.OriginalClass;
import org.openrs2.deob.annotation.OriginalMember;

@OriginalClass("client!y")
public class PathingEntity extends Entity {

    @OriginalMember(owner = "client!y", name = "l", descriptor = "Z")
    private boolean field392 = true;

    @OriginalMember(owner = "client!y", name = "m", descriptor = "Z")
    private boolean field393 = false;

    @OriginalMember(owner = "client!y", name = "q", descriptor = "Z")
    public boolean field397 = false;

    @OriginalMember(owner = "client!y", name = "r", descriptor = "I")
    public int field398 = 1;

    @OriginalMember(owner = "client!y", name = "s", descriptor = "I")
    public int field399 = -1;

    @OriginalMember(owner = "client!y", name = "t", descriptor = "I")
    public int field400 = -1;

    @OriginalMember(owner = "client!y", name = "u", descriptor = "I")
    public int field401 = -1;

    @OriginalMember(owner = "client!y", name = "v", descriptor = "I")
    public int field402 = -1;

    @OriginalMember(owner = "client!y", name = "w", descriptor = "I")
    public int field403 = -1;

    @OriginalMember(owner = "client!y", name = "x", descriptor = "I")
    public int field404 = -1;

    @OriginalMember(owner = "client!y", name = "y", descriptor = "I")
    public int field405 = -1;

    @OriginalMember(owner = "client!y", name = "A", descriptor = "I")
    public int field407 = 100;

    @OriginalMember(owner = "client!y", name = "D", descriptor = "[I")
    public int[] field410 = new int[4];

    @OriginalMember(owner = "client!y", name = "E", descriptor = "[I")
    public int[] field411 = new int[4];

    @OriginalMember(owner = "client!y", name = "F", descriptor = "[I")
    public int[] field412 = new int[4];

    @OriginalMember(owner = "client!y", name = "G", descriptor = "I")
    public int field413 = -1000;

    @OriginalMember(owner = "client!y", name = "J", descriptor = "I")
    public int field416 = -1;

    @OriginalMember(owner = "client!y", name = "M", descriptor = "I")
    public int field419 = -1;

    @OriginalMember(owner = "client!y", name = "P", descriptor = "I")
    public int field422 = -1;

    @OriginalMember(owner = "client!y", name = "U", descriptor = "I")
    public int field427 = -1;

    @OriginalMember(owner = "client!y", name = "hb", descriptor = "I")
    public int field440 = 200;

    @OriginalMember(owner = "client!y", name = "jb", descriptor = "I")
    public int field442 = 32;

    @OriginalMember(owner = "client!y", name = "lb", descriptor = "[I")
    public int[] field444 = new int[10];

    @OriginalMember(owner = "client!y", name = "mb", descriptor = "[I")
    public int[] field445 = new int[10];

    @OriginalMember(owner = "client!y", name = "nb", descriptor = "[Z")
    public boolean[] field446 = new boolean[10];

    @OriginalMember(owner = "client!y", name = "n", descriptor = "I")
    public int field394;

    @OriginalMember(owner = "client!y", name = "o", descriptor = "I")
    public int field395;

    @OriginalMember(owner = "client!y", name = "p", descriptor = "I")
    public int field396;

    @OriginalMember(owner = "client!y", name = "B", descriptor = "I")
    public int field408;

    @OriginalMember(owner = "client!y", name = "C", descriptor = "I")
    public int field409;

    @OriginalMember(owner = "client!y", name = "H", descriptor = "I")
    public int field414;

    @OriginalMember(owner = "client!y", name = "I", descriptor = "I")
    public int field415;

    @OriginalMember(owner = "client!y", name = "K", descriptor = "I")
    public int field417;

    @OriginalMember(owner = "client!y", name = "L", descriptor = "I")
    public int field418;

    @OriginalMember(owner = "client!y", name = "N", descriptor = "I")
    public int field420;

    @OriginalMember(owner = "client!y", name = "O", descriptor = "I")
    public int field421;

    @OriginalMember(owner = "client!y", name = "Q", descriptor = "I")
    public int field423;

    @OriginalMember(owner = "client!y", name = "R", descriptor = "I")
    public int field424;

    @OriginalMember(owner = "client!y", name = "S", descriptor = "I")
    public int field425;

    @OriginalMember(owner = "client!y", name = "T", descriptor = "I")
    public int field426;

    @OriginalMember(owner = "client!y", name = "V", descriptor = "I")
    public int field428;

    @OriginalMember(owner = "client!y", name = "W", descriptor = "I")
    public int field429;

    @OriginalMember(owner = "client!y", name = "X", descriptor = "I")
    public int field430;

    @OriginalMember(owner = "client!y", name = "Y", descriptor = "I")
    public int field431;

    @OriginalMember(owner = "client!y", name = "Z", descriptor = "I")
    public int field432;

    @OriginalMember(owner = "client!y", name = "ab", descriptor = "I")
    public int field433;

    @OriginalMember(owner = "client!y", name = "bb", descriptor = "I")
    public int field434;

    @OriginalMember(owner = "client!y", name = "cb", descriptor = "I")
    public int field435;

    @OriginalMember(owner = "client!y", name = "db", descriptor = "I")
    public int field436;

    @OriginalMember(owner = "client!y", name = "eb", descriptor = "I")
    public int field437;

    @OriginalMember(owner = "client!y", name = "fb", descriptor = "I")
    public int field438;

    @OriginalMember(owner = "client!y", name = "gb", descriptor = "I")
    public int field439;

    @OriginalMember(owner = "client!y", name = "ib", descriptor = "I")
    public int field441;

    @OriginalMember(owner = "client!y", name = "kb", descriptor = "I")
    public int field443;

    @OriginalMember(owner = "client!y", name = "ob", descriptor = "I")
    public int field447;

    @OriginalMember(owner = "client!y", name = "pb", descriptor = "I")
    public int field448;

    @OriginalMember(owner = "client!y", name = "z", descriptor = "Ljava/lang/String;")
    public String field406;

    @OriginalMember(owner = "client!y", name = "a", descriptor = "(IZZI)V")
    public final void method110(int arg0, boolean arg1, boolean arg2, int arg3) {
        if (this.field422 != -1 && SeqType.field1143[this.field422].field1156 == 1) {
            this.field422 = -1;
        }
        if (!arg2) {
            int var5 = arg0 - this.field444[0];
            int var6 = arg3 - this.field445[0];
            if (var5 >= -8 && var5 <= 8 && var6 >= -8 && var6 <= 8) {
                if (this.field443 < 9) {
                    this.field443++;
                }
                for (int var7 = this.field443; var7 > 0; var7--) {
                    this.field444[var7] = this.field444[var7 - 1];
                    this.field445[var7] = this.field445[var7 - 1];
                    this.field446[var7] = this.field446[var7 - 1];
                }
                this.field444[0] = arg0;
                this.field445[0] = arg3;
                this.field446[0] = false;
                return;
            }
        }
        this.field443 = 0;
        this.field448 = 0;
        this.field447 = 0;
        this.field444[0] = arg0;
        this.field445[0] = arg3;
        this.field394 = this.field444[0] * 128 + this.field398 * 64;
        if (arg1) {
            this.field392 = !this.field392;
        }
        this.field395 = this.field445[0] * 128 + this.field398 * 64;
    }

    @OriginalMember(owner = "client!y", name = "a", descriptor = "(IIZ)V")
    public final void method111(int arg0, int arg1, boolean arg2) {
        int var4 = this.field444[0];
        int var5 = this.field445[0];
        if (arg1 != 2) {
            return;
        }
        if (arg0 == 0) {
            var4--;
            var5++;
        }
        if (arg0 == 1) {
            var5++;
        }
        if (arg0 == 2) {
            var4++;
            var5++;
        }
        if (arg0 == 3) {
            var4--;
        }
        if (arg0 == 4) {
            var4++;
        }
        if (arg0 == 5) {
            var4--;
            var5--;
        }
        if (arg0 == 6) {
            var5--;
        }
        if (arg0 == 7) {
            var4++;
            var5--;
        }
        if (this.field422 != -1 && SeqType.field1143[this.field422].field1156 == 1) {
            this.field422 = -1;
        }
        if (this.field443 < 9) {
            this.field443++;
        }
        for (int var6 = this.field443; var6 > 0; var6--) {
            this.field444[var6] = this.field444[var6 - 1];
            this.field445[var6] = this.field445[var6 - 1];
            this.field446[var6] = this.field446[var6 - 1];
        }
        this.field444[0] = var4;
        this.field445[0] = var5;
        this.field446[0] = arg2;
    }

    @OriginalMember(owner = "client!y", name = "a", descriptor = "(I)V")
    public final void method112(int arg0) {
        this.field443 = 0;
        if (arg0 <= 0) {
            for (int var2 = 1; var2 > 0; var2++) {
            }
        }
        this.field448 = 0;
    }

    @OriginalMember(owner = "client!y", name = "b", descriptor = "(I)Z")
    public boolean method113(int arg0) {
        if (arg0 != 8) {
            throw new NullPointerException();
        }
        return false;
    }

    @OriginalMember(owner = "client!y", name = "a", descriptor = "(IIII)V")
    public final void method114(int arg0, int arg1, int arg2, int arg3) {
        for (int var5 = 0; var5 < 4; var5++) {
            if (this.field412[var5] <= arg1) {
                this.field410[var5] = arg0;
                this.field411[var5] = arg3;
                this.field412[var5] = arg1 + 70;
                return;
            }
        }
        if (arg2 < 0) {
            ;
        }
    }
}
