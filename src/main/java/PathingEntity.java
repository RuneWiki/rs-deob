import org.openrs2.deob.annotation.OriginalClass;
import org.openrs2.deob.annotation.OriginalMember;

@OriginalClass("client!y")
public class PathingEntity extends Entity {

    @OriginalMember(owner = "client!y", name = "m", descriptor = "Z")
    private boolean field394 = false;

    @OriginalMember(owner = "client!y", name = "n", descriptor = "Z")
    private boolean field395 = false;

    @OriginalMember(owner = "client!y", name = "r", descriptor = "Z")
    public boolean field399 = false;

    @OriginalMember(owner = "client!y", name = "s", descriptor = "I")
    public int field400 = 1;

    @OriginalMember(owner = "client!y", name = "t", descriptor = "I")
    public int field401 = -1;

    @OriginalMember(owner = "client!y", name = "u", descriptor = "I")
    public int field402 = -1;

    @OriginalMember(owner = "client!y", name = "v", descriptor = "I")
    public int field403 = -1;

    @OriginalMember(owner = "client!y", name = "w", descriptor = "I")
    public int field404 = -1;

    @OriginalMember(owner = "client!y", name = "x", descriptor = "I")
    public int field405 = -1;

    @OriginalMember(owner = "client!y", name = "y", descriptor = "I")
    public int field406 = -1;

    @OriginalMember(owner = "client!y", name = "z", descriptor = "I")
    public int field407 = -1;

    @OriginalMember(owner = "client!y", name = "B", descriptor = "I")
    public int field409 = 100;

    @OriginalMember(owner = "client!y", name = "E", descriptor = "[I")
    public int[] field412 = new int[4];

    @OriginalMember(owner = "client!y", name = "F", descriptor = "[I")
    public int[] field413 = new int[4];

    @OriginalMember(owner = "client!y", name = "G", descriptor = "[I")
    public int[] field414 = new int[4];

    @OriginalMember(owner = "client!y", name = "H", descriptor = "I")
    public int field415 = -1000;

    @OriginalMember(owner = "client!y", name = "K", descriptor = "I")
    public int field418 = -1;

    @OriginalMember(owner = "client!y", name = "N", descriptor = "I")
    public int field421 = -1;

    @OriginalMember(owner = "client!y", name = "Q", descriptor = "I")
    public int field424 = -1;

    @OriginalMember(owner = "client!y", name = "V", descriptor = "I")
    public int field429 = -1;

    @OriginalMember(owner = "client!y", name = "ib", descriptor = "I")
    public int field442 = 200;

    @OriginalMember(owner = "client!y", name = "kb", descriptor = "I")
    public int field444 = 32;

    @OriginalMember(owner = "client!y", name = "mb", descriptor = "[I")
    public int[] field446 = new int[10];

    @OriginalMember(owner = "client!y", name = "nb", descriptor = "[I")
    public int[] field447 = new int[10];

    @OriginalMember(owner = "client!y", name = "ob", descriptor = "[Z")
    public boolean[] field448 = new boolean[10];

    @OriginalMember(owner = "client!y", name = "o", descriptor = "I")
    public int field396;

    @OriginalMember(owner = "client!y", name = "p", descriptor = "I")
    public int field397;

    @OriginalMember(owner = "client!y", name = "q", descriptor = "I")
    public int field398;

    @OriginalMember(owner = "client!y", name = "C", descriptor = "I")
    public int field410;

    @OriginalMember(owner = "client!y", name = "D", descriptor = "I")
    public int field411;

    @OriginalMember(owner = "client!y", name = "I", descriptor = "I")
    public int field416;

    @OriginalMember(owner = "client!y", name = "J", descriptor = "I")
    public int field417;

    @OriginalMember(owner = "client!y", name = "L", descriptor = "I")
    public int field419;

    @OriginalMember(owner = "client!y", name = "M", descriptor = "I")
    public int field420;

    @OriginalMember(owner = "client!y", name = "O", descriptor = "I")
    public int field422;

    @OriginalMember(owner = "client!y", name = "P", descriptor = "I")
    public int field423;

    @OriginalMember(owner = "client!y", name = "R", descriptor = "I")
    public int field425;

    @OriginalMember(owner = "client!y", name = "S", descriptor = "I")
    public int field426;

    @OriginalMember(owner = "client!y", name = "T", descriptor = "I")
    public int field427;

    @OriginalMember(owner = "client!y", name = "U", descriptor = "I")
    public int field428;

    @OriginalMember(owner = "client!y", name = "W", descriptor = "I")
    public int field430;

    @OriginalMember(owner = "client!y", name = "X", descriptor = "I")
    public int field431;

    @OriginalMember(owner = "client!y", name = "Y", descriptor = "I")
    public int field432;

    @OriginalMember(owner = "client!y", name = "Z", descriptor = "I")
    public int field433;

    @OriginalMember(owner = "client!y", name = "ab", descriptor = "I")
    public int field434;

    @OriginalMember(owner = "client!y", name = "bb", descriptor = "I")
    public int field435;

    @OriginalMember(owner = "client!y", name = "cb", descriptor = "I")
    public int field436;

    @OriginalMember(owner = "client!y", name = "db", descriptor = "I")
    public int field437;

    @OriginalMember(owner = "client!y", name = "eb", descriptor = "I")
    public int field438;

    @OriginalMember(owner = "client!y", name = "fb", descriptor = "I")
    public int field439;

    @OriginalMember(owner = "client!y", name = "gb", descriptor = "I")
    public int field440;

    @OriginalMember(owner = "client!y", name = "hb", descriptor = "I")
    public int field441;

    @OriginalMember(owner = "client!y", name = "jb", descriptor = "I")
    public int field443;

    @OriginalMember(owner = "client!y", name = "lb", descriptor = "I")
    public int field445;

    @OriginalMember(owner = "client!y", name = "pb", descriptor = "I")
    public int field449;

    @OriginalMember(owner = "client!y", name = "qb", descriptor = "I")
    public int field450;

    @OriginalMember(owner = "client!y", name = "A", descriptor = "Ljava/lang/String;")
    public String field408;

    @OriginalMember(owner = "client!y", name = "a", descriptor = "(IZIB)V")
    public final void method110(int arg0, boolean arg1, int arg2, byte arg3) {
        if (this.field424 != -1 && SeqType.field1150[this.field424].field1163 == 1) {
            this.field424 = -1;
        }
        if (!arg1) {
            int var5 = arg0 - this.field446[0];
            int var6 = arg2 - this.field447[0];
            if (var5 >= -8 && var5 <= 8 && var6 >= -8 && var6 <= 8) {
                if (this.field445 < 9) {
                    this.field445++;
                }
                for (int var7 = this.field445; var7 > 0; var7--) {
                    this.field446[var7] = this.field446[var7 - 1];
                    this.field447[var7] = this.field447[var7 - 1];
                    this.field448[var7] = this.field448[var7 - 1];
                }
                this.field446[0] = arg0;
                this.field447[0] = arg2;
                this.field448[0] = false;
                return;
            }
        }
        this.field445 = 0;
        this.field450 = 0;
        this.field449 = 0;
        this.field446[0] = arg0;
        this.field447[0] = arg2;
        this.field396 = this.field446[0] * 128 + this.field400 * 64;
        if (arg3 == 1) {
            boolean var8 = false;
        } else {
            this.field394 = !this.field394;
        }
        this.field397 = this.field447[0] * 128 + this.field400 * 64;
    }

    @OriginalMember(owner = "client!y", name = "a", descriptor = "(ZII)V")
    public final void method111(boolean arg0, int arg1, int arg2) {
        int var4 = this.field446[0];
        int var5 = this.field447[0];
        if (arg1 == 0) {
            var4--;
            var5++;
        }
        if (arg1 == 1) {
            var5++;
        }
        if (arg1 == 2) {
            var4++;
            var5++;
        }
        if (arg1 == 3) {
            var4--;
        }
        if (arg1 == 4) {
            var4++;
        }
        if (arg1 == 5) {
            var4--;
            var5--;
        }
        if (arg1 == 6) {
            var5--;
        }
        if (arg1 == 7) {
            var4++;
            var5--;
        }
        if (this.field424 != -1 && SeqType.field1150[this.field424].field1163 == 1) {
            this.field424 = -1;
        }
        if (this.field445 < 9) {
            this.field445++;
        }
        for (int var6 = this.field445; var6 > 0; var6--) {
            this.field446[var6] = this.field446[var6 - 1];
            this.field447[var6] = this.field447[var6 - 1];
            this.field448[var6] = this.field448[var6 - 1];
        }
        this.field446[0] = var4;
        this.field447[0] = var5;
        this.field448[0] = arg0;
        if (arg2 == -6002) {
            ;
        }
    }

    @OriginalMember(owner = "client!y", name = "a", descriptor = "(Z)V")
    public final void method112(boolean arg0) {
        if (arg0) {
            this.field395 = !this.field395;
        }
        this.field445 = 0;
        this.field450 = 0;
    }

    @OriginalMember(owner = "client!y", name = "b", descriptor = "(Z)Z")
    public boolean method113(boolean arg0) {
        if (!arg0) {
            throw new NullPointerException();
        }
        return false;
    }

    @OriginalMember(owner = "client!y", name = "a", descriptor = "(IIIZ)V")
    public final void method114(int arg0, int arg1, int arg2, boolean arg3) {
        if (arg3) {
            return;
        }
        for (int var5 = 0; var5 < 4; var5++) {
            if (this.field414[var5] <= arg1) {
                this.field412[var5] = arg2;
                this.field413[var5] = arg0;
                this.field414[var5] = arg1 + 70;
                return;
            }
        }
    }
}
