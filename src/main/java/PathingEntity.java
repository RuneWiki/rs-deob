import org.openrs2.deob.annotation.OriginalClass;
import org.openrs2.deob.annotation.OriginalMember;

@OriginalClass("client!y")
public class PathingEntity extends Entity {

    @OriginalMember(owner = "client!y", name = "l", descriptor = "Z")
    private boolean field396 = false;

    @OriginalMember(owner = "client!y", name = "m", descriptor = "I")
    private int field397 = 3;

    @OriginalMember(owner = "client!y", name = "n", descriptor = "B")
    private byte field398 = -84;

    @OriginalMember(owner = "client!y", name = "r", descriptor = "Z")
    public boolean field402 = false;

    @OriginalMember(owner = "client!y", name = "s", descriptor = "I")
    public int field403 = 1;

    @OriginalMember(owner = "client!y", name = "t", descriptor = "I")
    public int field404 = -1;

    @OriginalMember(owner = "client!y", name = "u", descriptor = "I")
    public int field405 = -1;

    @OriginalMember(owner = "client!y", name = "v", descriptor = "I")
    public int field406 = -1;

    @OriginalMember(owner = "client!y", name = "w", descriptor = "I")
    public int field407 = -1;

    @OriginalMember(owner = "client!y", name = "x", descriptor = "I")
    public int field408 = -1;

    @OriginalMember(owner = "client!y", name = "y", descriptor = "I")
    public int field409 = -1;

    @OriginalMember(owner = "client!y", name = "z", descriptor = "I")
    public int field410 = -1;

    @OriginalMember(owner = "client!y", name = "B", descriptor = "I")
    public int field412 = 100;

    @OriginalMember(owner = "client!y", name = "E", descriptor = "[I")
    public int[] field415 = new int[4];

    @OriginalMember(owner = "client!y", name = "F", descriptor = "[I")
    public int[] field416 = new int[4];

    @OriginalMember(owner = "client!y", name = "G", descriptor = "[I")
    public int[] field417 = new int[4];

    @OriginalMember(owner = "client!y", name = "H", descriptor = "I")
    public int field418 = -1000;

    @OriginalMember(owner = "client!y", name = "K", descriptor = "I")
    public int field421 = -1;

    @OriginalMember(owner = "client!y", name = "N", descriptor = "I")
    public int field424 = -1;

    @OriginalMember(owner = "client!y", name = "Q", descriptor = "I")
    public int field427 = -1;

    @OriginalMember(owner = "client!y", name = "V", descriptor = "I")
    public int field432 = -1;

    @OriginalMember(owner = "client!y", name = "ib", descriptor = "I")
    public int field445 = 200;

    @OriginalMember(owner = "client!y", name = "kb", descriptor = "I")
    public int field447 = 32;

    @OriginalMember(owner = "client!y", name = "mb", descriptor = "[I")
    public int[] field449 = new int[10];

    @OriginalMember(owner = "client!y", name = "nb", descriptor = "[I")
    public int[] field450 = new int[10];

    @OriginalMember(owner = "client!y", name = "ob", descriptor = "[Z")
    public boolean[] field451 = new boolean[10];

    @OriginalMember(owner = "client!y", name = "o", descriptor = "I")
    public int field399;

    @OriginalMember(owner = "client!y", name = "p", descriptor = "I")
    public int field400;

    @OriginalMember(owner = "client!y", name = "q", descriptor = "I")
    public int field401;

    @OriginalMember(owner = "client!y", name = "C", descriptor = "I")
    public int field413;

    @OriginalMember(owner = "client!y", name = "D", descriptor = "I")
    public int field414;

    @OriginalMember(owner = "client!y", name = "I", descriptor = "I")
    public int field419;

    @OriginalMember(owner = "client!y", name = "J", descriptor = "I")
    public int field420;

    @OriginalMember(owner = "client!y", name = "L", descriptor = "I")
    public int field422;

    @OriginalMember(owner = "client!y", name = "M", descriptor = "I")
    public int field423;

    @OriginalMember(owner = "client!y", name = "O", descriptor = "I")
    public int field425;

    @OriginalMember(owner = "client!y", name = "P", descriptor = "I")
    public int field426;

    @OriginalMember(owner = "client!y", name = "R", descriptor = "I")
    public int field428;

    @OriginalMember(owner = "client!y", name = "S", descriptor = "I")
    public int field429;

    @OriginalMember(owner = "client!y", name = "T", descriptor = "I")
    public int field430;

    @OriginalMember(owner = "client!y", name = "U", descriptor = "I")
    public int field431;

    @OriginalMember(owner = "client!y", name = "W", descriptor = "I")
    public int field433;

    @OriginalMember(owner = "client!y", name = "X", descriptor = "I")
    public int field434;

    @OriginalMember(owner = "client!y", name = "Y", descriptor = "I")
    public int field435;

    @OriginalMember(owner = "client!y", name = "Z", descriptor = "I")
    public int field436;

    @OriginalMember(owner = "client!y", name = "ab", descriptor = "I")
    public int field437;

    @OriginalMember(owner = "client!y", name = "bb", descriptor = "I")
    public int field438;

    @OriginalMember(owner = "client!y", name = "cb", descriptor = "I")
    public int field439;

    @OriginalMember(owner = "client!y", name = "db", descriptor = "I")
    public int field440;

    @OriginalMember(owner = "client!y", name = "eb", descriptor = "I")
    public int field441;

    @OriginalMember(owner = "client!y", name = "fb", descriptor = "I")
    public int field442;

    @OriginalMember(owner = "client!y", name = "gb", descriptor = "I")
    public int field443;

    @OriginalMember(owner = "client!y", name = "hb", descriptor = "I")
    public int field444;

    @OriginalMember(owner = "client!y", name = "jb", descriptor = "I")
    public int field446;

    @OriginalMember(owner = "client!y", name = "lb", descriptor = "I")
    public int field448;

    @OriginalMember(owner = "client!y", name = "pb", descriptor = "I")
    public int field452;

    @OriginalMember(owner = "client!y", name = "qb", descriptor = "I")
    public int field453;

    @OriginalMember(owner = "client!y", name = "A", descriptor = "Ljava/lang/String;")
    public String field411;

    @OriginalMember(owner = "client!y", name = "a", descriptor = "(IBZI)V")
    public final void method110(int arg0, byte arg1, boolean arg2, int arg3) {
        if (this.field427 != -1 && SeqType.field1149[this.field427].field1162 == 1) {
            this.field427 = -1;
        }
        if (!arg2) {
            int var5 = arg3 - this.field449[0];
            int var6 = arg0 - this.field450[0];
            if (var5 >= -8 && var5 <= 8 && var6 >= -8 && var6 <= 8) {
                if (this.field448 < 9) {
                    this.field448++;
                }
                for (int var7 = this.field448; var7 > 0; var7--) {
                    this.field449[var7] = this.field449[var7 - 1];
                    this.field450[var7] = this.field450[var7 - 1];
                    this.field451[var7] = this.field451[var7 - 1];
                }
                this.field449[0] = arg3;
                this.field450[0] = arg0;
                this.field451[0] = false;
                return;
            }
        }
        this.field448 = 0;
        this.field453 = 0;
        this.field452 = 0;
        if (arg1 != 7) {
            this.field396 = !this.field396;
        }
        this.field449[0] = arg3;
        this.field450[0] = arg0;
        this.field399 = this.field449[0] * 128 + this.field403 * 64;
        this.field400 = this.field450[0] * 128 + this.field403 * 64;
    }

    @OriginalMember(owner = "client!y", name = "a", descriptor = "(ZII)V")
    public final void method111(boolean arg0, int arg1, int arg2) {
        int var4 = this.field449[0];
        int var5 = this.field450[0];
        if (arg2 == 0) {
            var4--;
            var5++;
        }
        if (arg2 == 1) {
            var5++;
        }
        if (arg2 == 2) {
            var4++;
            var5++;
        }
        if (arg2 == 3) {
            var4--;
        }
        if (arg2 == 4) {
            var4++;
        }
        if (arg2 == 5) {
            var4--;
            var5--;
        }
        if (arg2 == 6) {
            var5--;
        }
        if (arg2 == 7) {
            var4++;
            var5--;
        }
        if (this.field427 != -1 && SeqType.field1149[this.field427].field1162 == 1) {
            this.field427 = -1;
        }
        if (this.field448 < 9) {
            this.field448++;
        }
        for (int var6 = this.field448; var6 > 0; var6--) {
            this.field449[var6] = this.field449[var6 - 1];
            this.field450[var6] = this.field450[var6 - 1];
            this.field451[var6] = this.field451[var6 - 1];
        }
        this.field449[0] = var4;
        this.field450[0] = var5;
        this.field451[0] = arg0;
        int var7 = 78 / arg1;
    }

    @OriginalMember(owner = "client!y", name = "b", descriptor = "(I)V")
    public final void method112(int arg0) {
        if (arg0 == 0) {
            this.field448 = 0;
            this.field453 = 0;
        }
    }

    @OriginalMember(owner = "client!y", name = "a", descriptor = "(B)Z")
    public boolean method113(byte arg0) {
        if (this.field398 != arg0) {
            this.field397 = 373;
        }
        return false;
    }

    @OriginalMember(owner = "client!y", name = "a", descriptor = "(ZIII)V")
    public final void method114(boolean arg0, int arg1, int arg2, int arg3) {
        if (!arg0) {
            this.field396 = !this.field396;
        }
        for (int var5 = 0; var5 < 4; var5++) {
            if (this.field417[var5] <= arg2) {
                this.field415[var5] = arg3;
                this.field416[var5] = arg1;
                this.field417[var5] = arg2 + 70;
                return;
            }
        }
    }
}
