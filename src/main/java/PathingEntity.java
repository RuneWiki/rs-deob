import org.openrs2.deob.annotation.OriginalClass;
import org.openrs2.deob.annotation.OriginalMember;

@OriginalClass("client!y")
public class PathingEntity extends Entity {

    @OriginalMember(owner = "client!y", name = "l", descriptor = "Z")
    private boolean field400 = true;

    @OriginalMember(owner = "client!y", name = "m", descriptor = "I")
    private int field401 = 31409;

    @OriginalMember(owner = "client!y", name = "q", descriptor = "Z")
    public boolean field405 = false;

    @OriginalMember(owner = "client!y", name = "r", descriptor = "I")
    public int field406 = 1;

    @OriginalMember(owner = "client!y", name = "s", descriptor = "I")
    public int field407 = -1;

    @OriginalMember(owner = "client!y", name = "t", descriptor = "I")
    public int field408 = -1;

    @OriginalMember(owner = "client!y", name = "u", descriptor = "I")
    public int field409 = -1;

    @OriginalMember(owner = "client!y", name = "v", descriptor = "I")
    public int field410 = -1;

    @OriginalMember(owner = "client!y", name = "w", descriptor = "I")
    public int field411 = -1;

    @OriginalMember(owner = "client!y", name = "x", descriptor = "I")
    public int field412 = -1;

    @OriginalMember(owner = "client!y", name = "y", descriptor = "I")
    public int field413 = -1;

    @OriginalMember(owner = "client!y", name = "A", descriptor = "I")
    public int field415 = 100;

    @OriginalMember(owner = "client!y", name = "D", descriptor = "[I")
    public int[] field418 = new int[4];

    @OriginalMember(owner = "client!y", name = "E", descriptor = "[I")
    public int[] field419 = new int[4];

    @OriginalMember(owner = "client!y", name = "F", descriptor = "[I")
    public int[] field420 = new int[4];

    @OriginalMember(owner = "client!y", name = "G", descriptor = "I")
    public int field421 = -1000;

    @OriginalMember(owner = "client!y", name = "J", descriptor = "I")
    public int field424 = -1;

    @OriginalMember(owner = "client!y", name = "M", descriptor = "I")
    public int field427 = -1;

    @OriginalMember(owner = "client!y", name = "P", descriptor = "I")
    public int field430 = -1;

    @OriginalMember(owner = "client!y", name = "U", descriptor = "I")
    public int field435 = -1;

    @OriginalMember(owner = "client!y", name = "hb", descriptor = "I")
    public int field448 = 200;

    @OriginalMember(owner = "client!y", name = "jb", descriptor = "I")
    public int field450 = 32;

    @OriginalMember(owner = "client!y", name = "lb", descriptor = "[I")
    public int[] field452 = new int[10];

    @OriginalMember(owner = "client!y", name = "mb", descriptor = "[I")
    public int[] field453 = new int[10];

    @OriginalMember(owner = "client!y", name = "nb", descriptor = "[Z")
    public boolean[] field454 = new boolean[10];

    @OriginalMember(owner = "client!y", name = "n", descriptor = "I")
    public int field402;

    @OriginalMember(owner = "client!y", name = "o", descriptor = "I")
    public int field403;

    @OriginalMember(owner = "client!y", name = "p", descriptor = "I")
    public int field404;

    @OriginalMember(owner = "client!y", name = "B", descriptor = "I")
    public int field416;

    @OriginalMember(owner = "client!y", name = "C", descriptor = "I")
    public int field417;

    @OriginalMember(owner = "client!y", name = "H", descriptor = "I")
    public int field422;

    @OriginalMember(owner = "client!y", name = "I", descriptor = "I")
    public int field423;

    @OriginalMember(owner = "client!y", name = "K", descriptor = "I")
    public int field425;

    @OriginalMember(owner = "client!y", name = "L", descriptor = "I")
    public int field426;

    @OriginalMember(owner = "client!y", name = "N", descriptor = "I")
    public int field428;

    @OriginalMember(owner = "client!y", name = "O", descriptor = "I")
    public int field429;

    @OriginalMember(owner = "client!y", name = "Q", descriptor = "I")
    public int field431;

    @OriginalMember(owner = "client!y", name = "R", descriptor = "I")
    public int field432;

    @OriginalMember(owner = "client!y", name = "S", descriptor = "I")
    public int field433;

    @OriginalMember(owner = "client!y", name = "T", descriptor = "I")
    public int field434;

    @OriginalMember(owner = "client!y", name = "V", descriptor = "I")
    public int field436;

    @OriginalMember(owner = "client!y", name = "W", descriptor = "I")
    public int field437;

    @OriginalMember(owner = "client!y", name = "X", descriptor = "I")
    public int field438;

    @OriginalMember(owner = "client!y", name = "Y", descriptor = "I")
    public int field439;

    @OriginalMember(owner = "client!y", name = "Z", descriptor = "I")
    public int field440;

    @OriginalMember(owner = "client!y", name = "ab", descriptor = "I")
    public int field441;

    @OriginalMember(owner = "client!y", name = "bb", descriptor = "I")
    public int field442;

    @OriginalMember(owner = "client!y", name = "cb", descriptor = "I")
    public int field443;

    @OriginalMember(owner = "client!y", name = "db", descriptor = "I")
    public int field444;

    @OriginalMember(owner = "client!y", name = "eb", descriptor = "I")
    public int field445;

    @OriginalMember(owner = "client!y", name = "fb", descriptor = "I")
    public int field446;

    @OriginalMember(owner = "client!y", name = "gb", descriptor = "I")
    public int field447;

    @OriginalMember(owner = "client!y", name = "ib", descriptor = "I")
    public int field449;

    @OriginalMember(owner = "client!y", name = "kb", descriptor = "I")
    public int field451;

    @OriginalMember(owner = "client!y", name = "ob", descriptor = "I")
    public int field455;

    @OriginalMember(owner = "client!y", name = "pb", descriptor = "I")
    public int field456;

    @OriginalMember(owner = "client!y", name = "z", descriptor = "Ljava/lang/String;")
    public String field414;

    @OriginalMember(owner = "client!y", name = "a", descriptor = "(IZII)V")
    public final void method110(int arg0, boolean arg1, int arg2, int arg3) {
        if (this.field430 != -1 && SeqType.field1144[this.field430].field1157 == 1) {
            this.field430 = -1;
        }
        if (!arg1) {
            int var5 = arg0 - this.field452[0];
            int var6 = arg3 - this.field453[0];
            if (var5 >= -8 && var5 <= 8 && var6 >= -8 && var6 <= 8) {
                if (this.field451 < 9) {
                    this.field451++;
                }
                for (int var7 = this.field451; var7 > 0; var7--) {
                    this.field452[var7] = this.field452[var7 - 1];
                    this.field453[var7] = this.field453[var7 - 1];
                    this.field454[var7] = this.field454[var7 - 1];
                }
                this.field452[0] = arg0;
                this.field453[0] = arg3;
                this.field454[0] = false;
                return;
            }
        }
        this.field451 = 0;
        this.field456 = 0;
        this.field455 = 0;
        this.field452[0] = arg0;
        if (arg2 >= 1 && arg2 <= 1) {
            this.field453[0] = arg3;
            this.field402 = this.field452[0] * 128 + this.field406 * 64;
            this.field403 = this.field453[0] * 128 + this.field406 * 64;
        }
    }

    @OriginalMember(owner = "client!y", name = "a", descriptor = "(IZI)V")
    public final void method111(int arg0, boolean arg1, int arg2) {
        if (this.field401 != arg2) {
            return;
        }
        int var4 = this.field452[0];
        int var5 = this.field453[0];
        if (arg0 == 0) {
            var4--;
            var5++;
        }
        if (arg0 == 1) {
            var5++;
        }
        if (arg0 == 2) {
            var4++;
            var5++;
        }
        if (arg0 == 3) {
            var4--;
        }
        if (arg0 == 4) {
            var4++;
        }
        if (arg0 == 5) {
            var4--;
            var5--;
        }
        if (arg0 == 6) {
            var5--;
        }
        if (arg0 == 7) {
            var4++;
            var5--;
        }
        if (this.field430 != -1 && SeqType.field1144[this.field430].field1157 == 1) {
            this.field430 = -1;
        }
        if (this.field451 < 9) {
            this.field451++;
        }
        for (int var6 = this.field451; var6 > 0; var6--) {
            this.field452[var6] = this.field452[var6 - 1];
            this.field453[var6] = this.field453[var6 - 1];
            this.field454[var6] = this.field454[var6 - 1];
        }
        this.field452[0] = var4;
        this.field453[0] = var5;
        this.field454[0] = arg1;
    }

    @OriginalMember(owner = "client!y", name = "a", descriptor = "(I)V")
    public final void method112(int arg0) {
        this.field451 = 0;
        if (arg0 != 0) {
            for (int var2 = 1; var2 > 0; var2++) {
            }
        }
        this.field456 = 0;
    }

    @OriginalMember(owner = "client!y", name = "b", descriptor = "(Z)Z")
    public boolean method113(boolean arg0) {
        if (arg0) {
            this.field400 = !this.field400;
        }
        return false;
    }

    @OriginalMember(owner = "client!y", name = "a", descriptor = "(IIII)V")
    public final void method114(int arg0, int arg1, int arg2, int arg3) {
        for (int var5 = 0; var5 < 4; var5++) {
            if (this.field420[var5] <= arg3) {
                this.field418[var5] = arg1;
                this.field419[var5] = arg2;
                this.field420[var5] = arg3 + 70;
                return;
            }
        }
        if (arg0 == 16040) {
            ;
        }
    }
}
