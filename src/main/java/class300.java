import org.openrs2.deob.annotation.OriginalClass;
import org.openrs2.deob.annotation.OriginalMember;

@OriginalClass("client!g")
public class class300 extends class198 {

    @OriginalMember(owner = "client!g", name = "M", descriptor = "I")
    private int field4922 = 6;

    @OriginalMember(owner = "client!g", name = "K", descriptor = "[S")
    public static short[] field4920 = new short[256];

    @OriginalMember(owner = "client!g", name = "P", descriptor = "[Lcl;")
    public static class208[] field4925 = new class208[2048];

    @OriginalMember(owner = "client!g", name = "S", descriptor = "[I")
    public static int[] field4928 = new int[] { 2047, 16383, 65535 };

    @OriginalMember(owner = "client!g", name = "X", descriptor = "[Ljava/lang/String;")
    public static String[] field4933 = new String[100];

    @OriginalMember(owner = "client!g", name = "V", descriptor = "I")
    public static int field4931 = -16 + (int) (Math.random() * 33.0D);

    @OriginalMember(owner = "client!g", name = "J", descriptor = "Z")
    public static boolean field4919 = false;

    @OriginalMember(owner = "client!g", name = "U", descriptor = "Ljava/lang/String;")
    public static String field4930 = "Loading config - ";

    @OriginalMember(owner = "client!g", name = "I", descriptor = "I")
    public static int field4918;

    @OriginalMember(owner = "client!g", name = "L", descriptor = "I")
    public static int field4921;

    @OriginalMember(owner = "client!g", name = "N", descriptor = "I")
    public static int field4923;

    @OriginalMember(owner = "client!g", name = "O", descriptor = "I")
    public static int field4924;

    @OriginalMember(owner = "client!g", name = "Q", descriptor = "I")
    public static int field4926;

    @OriginalMember(owner = "client!g", name = "R", descriptor = "I")
    public static int field4927;

    @OriginalMember(owner = "client!g", name = "W", descriptor = "I")
    public static int field4932;

    @OriginalMember(owner = "client!g", name = "T", descriptor = "Ljava/lang/String;")
    public static String field4929;

    @OriginalMember(owner = "client!g", name = "<init>", descriptor = "()V")
    public class300() {
        super(2, false);
    }

    @OriginalMember(owner = "client!g", name = "a", descriptor = "(ZI)[I")
    public final int[] method32(boolean arg0, int arg1) {
        ++field4926;
        int[] var3 = super.field3134.method36(arg1, 30672);
        if (super.field3134.field69) {
            int[] var4 = this.method1367((byte) -79, 0, arg1);
            int[] var5 = this.method1367((byte) -79, 1, arg1);
            int var6 = this.field4922;
            if (var6 != 1) {
                if (~var6 != -3) {
                    if (~var6 != -4) {
                        if (~var6 != -5) {
                            if (var6 != 5) {
                                if (var6 != 6) {
                                    if (~var6 != -8) {
                                        if (~var6 != -9) {
                                            if (var6 != 9) {
                                                if (var6 != 10) {
                                                    if (var6 != 11) {
                                                        if (~var6 == -13) {
                                                            for (int var7 = 0; ~var7 > ~class42.field590; ++var7) {
                                                                int var8 = var5[var7];
                                                                int var9 = var4[var7];
                                                                var3[var7] = var9 - ((var8 * var9 >> 11) + -var8);
                                                            }
                                                        }
                                                    } else {
                                                        for (int var10 = 0; class42.field590 > var10; ++var10) {
                                                            int var11 = var4[var10];
                                                            int var12 = var5[var10];
                                                            var3[var10] = ~var12 > ~var11 ? var11 - var12 : -var11 + var12;
                                                        }
                                                    }
                                                } else {
                                                    for (int var13 = 0; ~var13 > ~class42.field590; ++var13) {
                                                        int var14 = var4[var13];
                                                        int var15 = var5[var13];
                                                        var3[var13] = ~var14 < ~var15 ? var14 : var15;
                                                    }
                                                }
                                            } else {
                                                for (int var16 = 0; class42.field590 > var16; ++var16) {
                                                    int var17 = var4[var16];
                                                    int var18 = var5[var16];
                                                    var3[var16] = var18 > var17 ? var17 : var18;
                                                }
                                            }
                                        } else {
                                            for (int var19 = 0; ~class42.field590 < ~var19; ++var19) {
                                                int var20 = var4[var19];
                                                var3[var19] = var20 != 0 ? -((-var5[var19] + 4096 << 12) / var20) + 4096 : 0;
                                            }
                                        }
                                    } else {
                                        for (int var21 = 0; var21 < class42.field590; ++var21) {
                                            int var22 = var4[var21];
                                            var3[var21] = var22 == 4096 ? 4096 : (var5[var21] << 12) / (-var22 + 4096);
                                        }
                                    }
                                } else {
                                    for (int var23 = 0; ~class42.field590 < ~var23; ++var23) {
                                        int var24 = var5[var23];
                                        var3[var23] = ~var24 > -2049 ? var4[var23] * var24 >> 11 : -((4096 - var24) * (-var4[var23] + 4096) >> 11) + 4096;
                                    }
                                }
                            } else {
                                for (int var25 = 0; var25 < class42.field590; ++var25) {
                                    var3[var25] = 4096 - ((4096 - var5[var25]) * (-var4[var25] + 4096) >> 12);
                                }
                            }
                        } else {
                            for (int var26 = 0; ~class42.field590 < ~var26; ++var26) {
                                int var27 = var5[var26];
                                var3[var26] = var27 != 0 ? (var4[var26] << 12) / var27 : 4096;
                            }
                        }
                    } else {
                        for (int var28 = 0; ~var28 > ~class42.field590; ++var28) {
                            var3[var28] = var4[var28] * var5[var28] >> 12;
                        }
                    }
                } else {
                    for (int var29 = 0; var29 < class42.field590; ++var29) {
                        var3[var29] = var4[var29] + -var5[var29];
                    }
                }
            } else {
                for (int var30 = 0; ~var30 > ~class42.field590; ++var30) {
                    var3[var30] = var4[var30] + var5[var30];
                }
            }
        }
        return !arg0 ? null : var3;
    }

    @OriginalMember(owner = "client!g", name = "b", descriptor = "(Z)V")
    public static void method2074(boolean arg0) {
        field4930 = null;
        if (arg0) {
            field4919 = false;
        }
        field4929 = null;
        field4933 = null;
        field4925 = null;
        field4920 = null;
        field4928 = null;
    }

    @OriginalMember(owner = "client!g", name = "b", descriptor = "(II)I")
    public static final int method2075(int arg0, int arg1) {
        ++field4923;
        if (~arg0 > -1) {
            return 0;
        } else {
            class120 var2 = (class120) class42.field550.method872((byte) -121, (long) arg0);
            if (var2 == null) {
                return class252.method1744(arg0, 0).field4360;
            } else {
                int var3 = 0;
                if (arg1 != 1) {
                    field4925 = null;
                }
                for (int var4 = 0; ~var2.field1842.length < ~var4; ++var4) {
                    if (var2.field1842[var4] == -1) {
                        ++var3;
                    }
                }
                return var3 + class252.method1744(arg0, 0).field4360 + -var2.field1842.length;
            }
        }
    }

    @OriginalMember(owner = "client!g", name = "a", descriptor = "(ILma;I)V")
    public final void method52(int arg0, class202 arg1, int arg2) {
        if (arg2 != 0) {
            if (arg2 == 1) {
                super.field3139 = ~arg1.method1420((byte) 0) == -2;
            }
        } else {
            this.field4922 = arg1.method1420((byte) 0);
        }
        ++field4927;
        int var5 = 53 / ((arg0 - 60) / 61);
    }

    @OriginalMember(owner = "client!g", name = "a", descriptor = "(BI)I")
    public static final int method2076(byte arg0, int arg1) {
        ++field4921;
        if (arg0 <= 94) {
            method2077(-18, -34);
        }
        return arg1 == 16711935 ? -1 : class33.method192(arg1, false);
    }

    @OriginalMember(owner = "client!g", name = "a", descriptor = "(II)[[I")
    public final int[][] method200(int arg0, int arg1) {
        if (arg1 != -2) {
            method2075(22, -122);
        }
        int[][] var3 = super.field3150.method642(arg0, 1);
        if (super.field3150.field1498) {
            int[][] var4 = this.method1372(0, arg0, (byte) -46);
            int[][] var5 = this.method1372(1, arg0, (byte) 92);
            int[] var6 = var3[1];
            int[] var7 = var3[0];
            int[] var8 = var3[2];
            int[] var9 = var4[0];
            int[] var10 = var4[1];
            int[] var11 = var4[2];
            int[] var12 = var5[1];
            int[] var13 = var5[0];
            int[] var14 = var5[2];
            int var15 = this.field4922;
            if (~var15 != -2) {
                if (var15 != 2) {
                    if (~var15 != -4) {
                        if (var15 != 4) {
                            if (var15 != 5) {
                                if (var15 != 6) {
                                    if (~var15 != -8) {
                                        if (var15 != 8) {
                                            if (~var15 != -10) {
                                                if (~var15 != -11) {
                                                    if (~var15 != -12) {
                                                        if (~var15 == -13) {
                                                            for (int var16 = 0; ~var16 > ~class42.field590; ++var16) {
                                                                int var17 = var10[var16];
                                                                int var18 = var14[var16];
                                                                int var19 = var12[var16];
                                                                int var20 = var11[var16];
                                                                int var21 = var13[var16];
                                                                int var22 = var9[var16];
                                                                var7[var16] = var21 + var22 + -(var21 * var22 >> 11);
                                                                var6[var16] = -(var17 * var19 >> 11) + var17 + var19;
                                                                var8[var16] = -(var18 * var20 >> 11) + var18 + var20;
                                                            }
                                                        }
                                                    } else {
                                                        for (int var23 = 0; ~class42.field590 < ~var23; ++var23) {
                                                            int var24 = var10[var23];
                                                            int var25 = var13[var23];
                                                            int var26 = var11[var23];
                                                            int var27 = var9[var23];
                                                            int var28 = var14[var23];
                                                            int var29 = var12[var23];
                                                            var7[var23] = var25 < var27 ? -var25 + var27 : -var27 + var25;
                                                            var6[var23] = var24 <= var29 ? -var24 + var29 : -var29 + var24;
                                                            var8[var23] = ~var26 < ~var28 ? -var28 + var26 : -var26 + var28;
                                                        }
                                                    }
                                                } else {
                                                    for (int var30 = 0; var30 < class42.field590; ++var30) {
                                                        int var31 = var9[var30];
                                                        int var32 = var10[var30];
                                                        int var33 = var14[var30];
                                                        int var34 = var11[var30];
                                                        int var35 = var13[var30];
                                                        int var36 = var12[var30];
                                                        var7[var30] = var31 > var35 ? var31 : var35;
                                                        var6[var30] = var36 < var32 ? var32 : var36;
                                                        var8[var30] = var34 > var33 ? var34 : var33;
                                                    }
                                                }
                                            } else {
                                                for (int var37 = 0; var37 < class42.field590; ++var37) {
                                                    int var38 = var12[var37];
                                                    int var39 = var10[var37];
                                                    int var40 = var9[var37];
                                                    int var41 = var13[var37];
                                                    int var42 = var11[var37];
                                                    int var43 = var14[var37];
                                                    var7[var37] = var41 <= var40 ? var41 : var40;
                                                    var6[var37] = ~var39 > ~var38 ? var39 : var38;
                                                    var8[var37] = var43 <= var42 ? var43 : var42;
                                                }
                                            }
                                        } else {
                                            for (int var44 = 0; class42.field590 > var44; ++var44) {
                                                int var45 = var11[var44];
                                                int var46 = var10[var44];
                                                int var47 = var9[var44];
                                                var7[var44] = ~var47 == -1 ? 0 : 4096 - (4096 - var13[var44] << 12) / var47;
                                                var6[var44] = var46 == 0 ? 0 : -((-var12[var44] + 4096 << 12) / var46) + 4096;
                                                var8[var44] = var45 != 0 ? -((-var14[var44] + 4096 << 12) / var45) + 4096 : 0;
                                            }
                                        }
                                    } else {
                                        for (int var48 = 0; ~var48 > ~class42.field590; ++var48) {
                                            int var49 = var10[var48];
                                            int var50 = var11[var48];
                                            int var51 = var9[var48];
                                            var7[var48] = ~var51 == -4097 ? 4096 : (var13[var48] << 12) / (-var51 + 4096);
                                            var6[var48] = ~var49 != -4097 ? (var12[var48] << 12) / (-var49 + 4096) : 4096;
                                            var8[var48] = ~var50 != -4097 ? (var14[var48] << 12) / (-var50 + 4096) : 4096;
                                        }
                                    }
                                } else {
                                    for (int var52 = 0; ~var52 > ~class42.field590; ++var52) {
                                        int var53 = var12[var52];
                                        int var54 = var14[var52];
                                        int var55 = var13[var52];
                                        var7[var52] = var55 >= 2048 ? 4096 - ((-var9[var52] + 4096) * (-var55 + 4096) >> 11) : var9[var52] * var55 >> 11;
                                        var6[var52] = ~var53 > -2049 ? var10[var52] * var53 >> 11 : -((-var10[var52] + 4096) * (-var53 + 4096) >> 11) + 4096;
                                        var8[var52] = ~var54 > -2049 ? var11[var52] * var54 >> 11 : -((-var11[var52] + 4096) * (-var54 + 4096) >> 11) + 4096;
                                    }
                                }
                            } else {
                                for (int var56 = 0; var56 < class42.field590; ++var56) {
                                    var7[var56] = 4096 - ((4096 - var13[var56]) * (-var9[var56] + 4096) >> 12);
                                    var6[var56] = -((-var10[var56] + 4096) * (-var12[var56] + 4096) >> 12) + 4096;
                                    var8[var56] = 4096 - ((-var11[var56] + 4096) * (-var14[var56] + 4096) >> 12);
                                }
                            }
                        } else {
                            for (int var57 = 0; var57 < class42.field590; ++var57) {
                                int var58 = var12[var57];
                                int var59 = var13[var57];
                                int var60 = var14[var57];
                                var7[var57] = ~var59 == -1 ? 4096 : (var9[var57] << 12) / var59;
                                var6[var57] = var58 == 0 ? 4096 : (var10[var57] << 12) / var58;
                                var8[var57] = var60 == 0 ? 4096 : (var11[var57] << 12) / var60;
                            }
                        }
                    } else {
                        for (int var61 = 0; var61 < class42.field590; ++var61) {
                            var7[var61] = var9[var61] * var13[var61] >> 12;
                            var6[var61] = var10[var61] * var12[var61] >> 12;
                            var8[var61] = var11[var61] * var14[var61] >> 12;
                        }
                    }
                } else {
                    for (int var62 = 0; class42.field590 > var62; ++var62) {
                        var7[var62] = var9[var62] + -var13[var62];
                        var6[var62] = var10[var62] - var12[var62];
                        var8[var62] = var11[var62] + -var14[var62];
                    }
                }
            } else {
                for (int var63 = 0; ~var63 > ~class42.field590; ++var63) {
                    var7[var63] = var9[var63] + var13[var63];
                    var6[var63] = var10[var63] + var12[var63];
                    var8[var63] = var11[var63] - -var14[var63];
                }
            }
        }
        ++field4918;
        return var3;
    }

    @OriginalMember(owner = "client!g", name = "c", descriptor = "(II)V")
    public static final void method2077(int arg0, int arg1) {
        class124.field1909 = -1;
        class278.field4621 = false;
        class202.field3238 = 1;
        ++field4924;
        class180.field2793 = arg1;
        class231.field3705 = 0;
        class169.field2637 = arg0;
        class79.field1257 = null;
    }
}
