import org.openrs2.deob.annotation.OriginalClass;
import org.openrs2.deob.annotation.OriginalMember;

@OriginalClass("client!ar")
public class class684 extends class38 {

    @OriginalMember(owner = "client!ar", name = "F", descriptor = "I")
    private int field9463 = -1;

    @OriginalMember(owner = "client!ar", name = "H", descriptor = "I")
    public static int field9465 = 0;

    @OriginalMember(owner = "client!ar", name = "P", descriptor = "Lfa;")
    public static class235 field9473 = new class235(4, 1, 1, 1);

    @OriginalMember(owner = "client!ar", name = "Q", descriptor = "[B")
    public static byte[] field9474 = new byte[] { 31, -117, 8, 0, 0, 0, 0, 0, 0, 0, -5, 127, -29, -1, 109, 6, 103, 6, 14, 54, 54, 118, 54, 86, 14, 118, 118, 118, 78, 78, 14, 46, 30, 17, 94, 30, 110, 110, 30, 73, 33, 97, 126, 17, 89, 41, 121, 57, 89, 41, 25, 25, 5, 21, 61, 117, 5, 37, 29, 101, 25, 25, 13, 115, 77, 29, 3, 67, 19, 19, 19, 121, 117, 75, 91, 11, 35, 27, 61, 99, 19, 35, -112, 33, -116, -100, -100, -100, 60, -36, 60, 18, -68, -68, 18, 70, -118, 50, -118, 70, 36, -125, -1, 7, 24, 68, 56, 24, 24, 25, 24, 89, 24, -107, 24, -104, 4, 25, -103, 5, 25, 89, -108, 24, -2, 31, 97, -112, 103, 96, 96, 100, 101, 4, 3, 6, 40, 96, 100, 98, 102, 97, 101, 99, -25, -32, -28, -30, 6, 42, -40, 42, -64, -64, -60, -56, -52, -52, -60, -62, -52, -54, -54, -62, 2, -108, -83, 5, -54, 51, -80, 8, -78, 10, 41, 26, 58, -78, 9, 7, 38, -78, 43, 21, -118, 24, 53, 78, 92, -56, -95, -20, -76, -15, -96, 104, -48, -59, 15, 42, -58, 73, 69, 77, -100, 92, 98, -30, 18, -110, 82, -86, 106, -22, 26, -102, 90, 38, -90, 102, -26, 22, -106, 86, -50, 46, -82, 110, -18, 30, -98, 94, -63, 33, -95, 97, -31, 17, -111, 81, -55, 41, -87, 105, -23, 25, -103, 89, -59, 37, -91, 101, -27, 21, -107, 85, -51, 45, -83, 109, -19, 29, -99, 93, -109, 38, 79, -103, 58, 109, -6, -116, -103, -77, 22, 45, 94, -78, 116, -39, -14, 21, 43, 87, 109, -38, -68, 101, -21, -74, -19, 59, 118, -18, 58, 116, -8, -56, -47, 99, -57, 79, -100, 60, 117, -23, -14, -107, -85, -41, -82, -33, -72, 121, -21, -31, -93, -57, 79, -98, 62, 123, -2, -30, -27, -85, -113, -97, 62, 127, -7, -6, -19, -5, -113, -97, -65, 64, -2, 98, 100, 96, 102, -124, 1, -84, -2, 18, 4, -6, -117, -119, -123, -123, -103, -123, 29, -28, 47, 70, -90, 114, -112, 2, 65, 22, 86, 69, 67, 54, 33, -57, 64, -10, -60, 66, 97, 37, -93, 70, 14, 17, -89, -119, 11, 55, 30, -28, 84, 54, 14, -6, 32, -102, 84, 116, -111, 75, 76, -59, -28, -95, -22, 71, -112, -41, -64, 62, 35, -50, 99, 77, 100, -7, 12, -18, 49, -124, -65, 110, 49, -16, -79, 48, 2, -29, 15, -24, 78, 6, 6, 123, -122, -97, -1, -70, 52, 22, -16, -49, 127, -79, -88, -21, -1, 77, 0, 38, 27, -28, 10, 110, 2, 0, 0 };

    @OriginalMember(owner = "client!ar", name = "D", descriptor = "I")
    public static int field9462;

    @OriginalMember(owner = "client!ar", name = "I", descriptor = "I")
    private int field9466;

    @OriginalMember(owner = "client!ar", name = "J", descriptor = "I")
    public static int field9467;

    @OriginalMember(owner = "client!ar", name = "K", descriptor = "I")
    public static int field9468;

    @OriginalMember(owner = "client!ar", name = "L", descriptor = "I")
    public static int field9469;

    @OriginalMember(owner = "client!ar", name = "M", descriptor = "I")
    private int field9470;

    @OriginalMember(owner = "client!ar", name = "N", descriptor = "I")
    public static int field9471;

    @OriginalMember(owner = "client!ar", name = "G", descriptor = "Lvea;")
    public static class275 field9464;

    @OriginalMember(owner = "client!ar", name = "O", descriptor = "[I")
    private int[] field9472;

    @OriginalMember(owner = "client!ar", name = "a", descriptor = "(I)V", line = 4)
    public final void method198(int arg0) {
        super.method198(arg0);
        ++field9467;
        this.field9472 = null;
    }

    @OriginalMember(owner = "client!ar", name = "a", descriptor = "(II)[[I", line = 13)
    public final int[][] method196(int arg0, int arg1) {
        if (arg0 != -18210) {
            this.method198(121);
        }
        ++field9468;
        int[][] var3 = super.field387.method2809(arg1, (byte) -48);
        if (super.field387.field6792) {
            int var4 = this.field9466 * (~class632.field8852 != ~this.field9470 ? this.field9470 * arg1 / class632.field8852 : arg1);
            int[] var5 = var3[0];
            int[] var6 = var3[1];
            int[] var7 = var3[2];
            if (~class729.field10216 != ~this.field9466) {
                for (int var8 = 0; class729.field10216 > var8; ++var8) {
                    int var9 = this.field9466 * var8 / class729.field10216;
                    int var10 = this.field9472[var4 + var9];
                    var7[var8] = class493.method2943(var10, 255) << 4;
                    var6[var8] = class493.method2943(65280, var10) >> 4;
                    var5[var8] = class493.method2943(var10 >> 12, 4080);
                }
            } else {
                for (int var11 = 0; class729.field10216 > var11; ++var11) {
                    int var12 = this.field9472[var4++];
                    var7[var11] = class493.method2943(4080, var12 << 4);
                    var6[var11] = class493.method2943(var12, 65280) >> 4;
                    var5[var11] = class493.method2943(4080, var12 >> 12);
                }
            }
        }
        return var3;
    }

    @OriginalMember(owner = "client!ar", name = "a", descriptor = "(III)V", line = 79)
    public final void method194(int arg0, int arg1, int arg2) {
        super.method194(arg0, arg1, arg2);
        ++field9462;
        if (~this.field9463 <= -1 && class470.field6863 != null) {
            int var4 = !class470.field6863.method1124((byte) 89, this.field9463).field4174 ? 128 : 64;
            this.field9472 = class470.field6863.method1123((byte) 110, 1.0F, false, this.field9463, var4, var4);
            this.field9470 = var4;
            this.field9466 = var4;
        }
    }

    @OriginalMember(owner = "client!ar", name = "<init>", descriptor = "()V", line = 148)
    public class684() {
        super(0, false);
    }

    @OriginalMember(owner = "client!ar", name = "f", descriptor = "(B)V", line = 106)
    public static void method3905(byte arg0) {
        field9473 = null;
        field9464 = null;
        field9474 = null;
        int var1 = 21 % ((arg0 - 65) / 48);
    }

    @OriginalMember(owner = "client!ar", name = "e", descriptor = "(B)I", line = 117)
    public final int method197(byte arg0) {
        ++field9469;
        int var2 = 125 % ((-26 - arg0) / 62);
        return this.field9463;
    }

    @OriginalMember(owner = "client!ar", name = "a", descriptor = "(IILrg;)V", line = 132)
    public final void method190(int arg0, int arg1, class645 arg2) {
        if (~arg1 == -1) {
            this.field9463 = arg2.method3712((byte) 1);
        }
        if (arg0 <= 34) {
            field9474 = null;
        }
        ++field9471;
    }
}
