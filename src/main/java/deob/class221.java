package deob;

import java.util.ArrayList;
import java.util.Collections;
import java.util.Comparator;
import java.util.List;

@ObfuscatedName("hu")
public class class221 {

    @ObfuscatedName("hu.o")
    public final List field3164;

    @ObfuscatedName("hu.e")
    public static Comparator field3162 = new class218();

    @ObfuscatedName("hu.u")
    public static Comparator field3161;

    @ObfuscatedName("hu.b")
    public static Comparator field3165;

    @ObfuscatedName("hu.p")
    public static Comparator field3163;

    static {
        new class219();
        field3161 = new class217();
        field3165 = new class213();
        field3163 = new class216();
    }

    public class221(class119 arg0, boolean arg1) {
        int var3 = arg0.method2339();
        boolean var4 = arg0.method2427() == 1;
        byte var5;
        if (var4) {
            var5 = 1;
        } else {
            var5 = 0;
        }
        int var6 = arg0.method2339();
        this.field3164 = new ArrayList(var6);
        for (int var7 = 0; var7 < var6; var7++) {
            this.field3164.add(new class214(arg0, var5, var3));
        }
    }

    @ObfuscatedName("hu.o(Ljava/util/Comparator;ZI)V")
    public void method3751(Comparator arg0, boolean arg1) {
        if (arg1) {
            Collections.sort(this.field3164, arg0);
        } else {
            Collections.sort(this.field3164, Collections.reverseOrder(arg0));
        }
    }
}
