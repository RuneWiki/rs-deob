package deob;

import java.util.ArrayList;
import java.util.Collections;
import java.util.Comparator;
import java.util.List;

@ObfuscatedName("hl")
public class class221 {

    @ObfuscatedName("hl.m")
    public final List field3152;

    @ObfuscatedName("hl.l")
    public static Comparator field3153 = new class218();

    @ObfuscatedName("hl.y")
    public static Comparator field3151;

    @ObfuscatedName("hl.u")
    public static Comparator field3150;

    @ObfuscatedName("hl.k")
    public static Comparator field3154;

    static {
        new class219();
        field3151 = new class217();
        field3150 = new class213();
        field3154 = new class216();
    }

    public class221(class119 arg0, boolean arg1) {
        int var3 = arg0.method2541();
        boolean var4 = arg0.method2562() == 1;
        byte var5;
        if (var4) {
            var5 = 1;
        } else {
            var5 = 0;
        }
        int var6 = arg0.method2541();
        this.field3152 = new ArrayList(var6);
        for (int var7 = 0; var7 < var6; var7++) {
            this.field3152.add(new class214(arg0, var5, var3));
        }
    }

    @ObfuscatedName("hl.m(Ljava/util/Comparator;ZI)V")
    public void method3806(Comparator arg0, boolean arg1) {
        if (arg1) {
            Collections.sort(this.field3152, arg0);
        } else {
            Collections.sort(this.field3152, Collections.reverseOrder(arg0));
        }
    }
}
