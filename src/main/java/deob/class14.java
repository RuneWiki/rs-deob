package deob;

import java.util.ArrayList;
import java.util.Collections;
import java.util.Comparator;
import java.util.List;

@ObfuscatedName("u")
public class class14 {

    @ObfuscatedName("u.d")
    public final List field285;

    @ObfuscatedName("u.k")
    public static Comparator field282 = new class20();

    @ObfuscatedName("u.e")
    public static Comparator field281;

    @ObfuscatedName("u.p")
    public static Comparator field284;

    @ObfuscatedName("u.q")
    public static Comparator field283;

    static {
        new class13();
        field281 = new class18();
        field284 = new class19();
        field283 = new class16();
    }

    public class14(class174 arg0, boolean arg1) {
        int var3 = arg0.method3035();
        boolean var4 = arg0.method2955() == 1;
        byte var5;
        if (var4) {
            var5 = 1;
        } else {
            var5 = 0;
        }
        int var6 = arg0.method3035();
        this.field285 = new ArrayList(var6);
        for (int var7 = 0; var7 < var6; var7++) {
            this.field285.add(new class15(arg0, var5, var3));
        }
    }

    @ObfuscatedName("u.d(Ljava/util/Comparator;ZI)V")
    public void method67(Comparator arg0, boolean arg1) {
        if (arg1) {
            Collections.sort(this.field285, arg0);
        } else {
            Collections.sort(this.field285, Collections.reverseOrder(arg0));
        }
    }
}
