package deob;

@ObfuscatedName("ex")
public class class147 extends class133 {

    @ObfuscatedName("ex.v")
    public long field2227 = System.nanoTime();

    @ObfuscatedName("ex.v(I)V")
    public void method2687() {
        this.field2227 = System.nanoTime();
    }

    @ObfuscatedName("ex.f(IIS)I")
    public int method2688(int arg0, int arg1) {
        long var3 = (long) arg1 * 1000000L;
        long var5 = this.field2227 - System.nanoTime();
        if (var5 < var3) {
            var5 = var3;
        }
        class127.method2134(var5 / 1000000L);
        long var7 = System.nanoTime();
        int var9 = 0;
        while (var9 < 10 && (var9 < 1 || this.field2227 < var7)) {
            var9++;
            this.field2227 += (long) arg0 * 1000000L;
        }
        if (this.field2227 < var7) {
            this.field2227 = var7;
        }
        return var9;
    }
}
