package deob;

@ObfuscatedName("ef")
public class class147 extends class153 {

    @ObfuscatedName("ef.f")
    public int field1644;

    @ObfuscatedName("ef.o")
    public int field1641;

    @ObfuscatedName("ef.u")
    public int field1640;

    @ObfuscatedName("ef.p")
    public int field1643;

    @ObfuscatedName("ef.f(Lnu;I)V")
    public void method2660(class382 arg0) {
        int var2 = arg0.method5856();
        if (class159.field1745.field1740 != var2) {
            throw new IllegalStateException("");
        }
        this.field1694 = arg0.method5856();
        this.field1695 = arg0.method5856();
        this.field1700 = arg0.method5858();
        this.field1705 = arg0.method5858();
        this.field1644 = arg0.method5856();
        this.field1641 = arg0.method5856();
        this.field1692 = arg0.method5858();
        this.field1693 = arg0.method5858();
        this.field1640 = arg0.method5856();
        this.field1643 = arg0.method5856();
        this.field1696 = arg0.method5873();
        this.field1697 = arg0.method5873();
    }

    @ObfuscatedName("ef.o(Lnu;I)V")
    public void method2347(class382 arg0) {
        this.field1695 = Math.min(this.field1695, 4);
        this.field1698 = new short[1][64][64];
        this.field1699 = new short[this.field1695][64][64];
        this.field1691 = new byte[this.field1695][64][64];
        this.field1701 = new byte[this.field1695][64][64];
        this.field1702 = new class155[this.field1695][64][64][];
        int var2 = arg0.method5856();
        if (class158.field1734.field1736 != var2) {
            throw new IllegalStateException("");
        }
        int var3 = arg0.method5856();
        int var4 = arg0.method5856();
        int var5 = arg0.method5856();
        int var6 = arg0.method5856();
        if (this.field1692 != var3 || this.field1693 != var4 || this.field1640 != var5 || this.field1643 != var6) {
            throw new IllegalStateException("");
        }
        for (int var7 = 0; var7 < 8; var7++) {
            for (int var8 = 0; var8 < 8; var8++) {
                this.method2693(this.field1640 * 8 + var7, this.field1643 * 8 + var8, arg0);
            }
        }
    }

    public boolean equals(Object arg0) {
        if (!(arg0 instanceof class147)) {
            return false;
        }
        class147 var2 = (class147) arg0;
        if (this.field1692 == var2.field1692 && this.field1693 == var2.field1693) {
            return this.field1640 == var2.field1640 && this.field1643 == var2.field1643;
        } else {
            return false;
        }
    }

    public int hashCode() {
        return this.field1692 | this.field1693 << 8 | this.field1640 << 16 | this.field1643 << 24;
    }

    @ObfuscatedName("ef.u(B)I")
    public int method2658() {
        return this.field1644;
    }

    @ObfuscatedName("ef.p(I)I")
    public int method2659() {
        return this.field1641;
    }

    @ObfuscatedName("ef.k(B)I")
    public int method2672() {
        return this.field1640;
    }

    @ObfuscatedName("ef.g(B)I")
    public int method2661() {
        return this.field1643;
    }
}
