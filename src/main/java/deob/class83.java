package deob;

import java.awt.Component;
import java.awt.Graphics;
import java.awt.Image;
import java.awt.Shape;
import java.awt.image.ColorModel;
import java.awt.image.DirectColorModel;
import java.awt.image.ImageConsumer;
import java.awt.image.ImageObserver;
import java.awt.image.ImageProducer;
import java.util.Hashtable;

@ObfuscatedName("ca")
public final class class83 extends class85 implements ImageProducer, ImageObserver {

    @ObfuscatedName("ca.p")
    public ColorModel field1449;

    @ObfuscatedName("ca.e")
    public ImageConsumer field1447;

    @ObfuscatedName("ca.p(IILjava/awt/Component;I)V")
    public final void method1624(int arg0, int arg1, Component arg2) {
        this.field1462 = arg0;
        this.field1464 = arg1;
        this.field1474 = new int[arg0 * arg1 + 1];
        this.field1449 = new DirectColorModel(32, 16711680, 65280, 255);
        this.field1465 = arg2.createImage(this);
        this.method1623();
        arg2.prepareImage(this.field1465, this);
        this.method1623();
        arg2.prepareImage(this.field1465, this);
        this.method1623();
        arg2.prepareImage(this.field1465, this);
        this.method1699();
    }

    @ObfuscatedName("ca.e(Ljava/awt/Graphics;III)V")
    public final void method1625(Graphics arg0, int arg1, int arg2) {
        this.method1623();
        arg0.drawImage(this.field1465, arg1, arg2, this);
    }

    @ObfuscatedName("ca.a(Ljava/awt/Graphics;IIIII)V")
    public final void method1626(Graphics arg0, int arg1, int arg2, int arg3, int arg4) {
        this.method1627(arg1, arg2, arg3, arg4);
        Shape var6 = arg0.getClip();
        arg0.clipRect(arg1, arg2, arg3, arg4);
        arg0.drawImage(this.field1465, 0, 0, this);
        arg0.setClip(var6);
    }

    public synchronized void addConsumer(ImageConsumer arg0) {
        this.field1447 = arg0;
        arg0.setDimensions(this.field1462, this.field1464);
        arg0.setProperties((Hashtable) null);
        arg0.setColorModel(this.field1449);
        arg0.setHints(14);
    }

    public synchronized boolean isConsumer(ImageConsumer arg0) {
        return this.field1447 == arg0;
    }

    public synchronized void removeConsumer(ImageConsumer arg0) {
        if (this.field1447 == arg0) {
            this.field1447 = null;
        }
    }

    public void startProduction(ImageConsumer arg0) {
        this.addConsumer(arg0);
    }

    public void requestTopDownLeftRightResend(ImageConsumer arg0) {
    }

    @ObfuscatedName("ca.h(B)V")
    public synchronized void method1623() {
        if (this.field1447 != null) {
            this.field1447.setPixels(0, 0, this.field1462, this.field1464, this.field1449, this.field1474, 0, this.field1462);
            this.field1447.imageComplete(2);
        }
    }

    @ObfuscatedName("ca.y(IIIII)V")
    public synchronized void method1627(int arg0, int arg1, int arg2, int arg3) {
        if (this.field1447 != null) {
            this.field1447.setPixels(arg0, arg1, arg2, arg3, this.field1449, this.field1474, this.field1462 * arg1 + arg0, this.field1462);
            this.field1447.imageComplete(2);
        }
    }

    public boolean imageUpdate(Image arg0, int arg1, int arg2, int arg3, int arg4, int arg5) {
        return true;
    }
}
