package deob;

@ObfuscatedName("co")
public class class88 {

    @ObfuscatedName("co.o")
    public static int[] field1518 = new int[500];

    @ObfuscatedName("co.e")
    public static int[] field1516 = new int[500];

    @ObfuscatedName("co.u")
    public static int[] field1517 = new int[500];

    @ObfuscatedName("co.b")
    public static int[] field1523 = new int[500];

    @ObfuscatedName("co.p")
    public class102 field1515 = null;

    @ObfuscatedName("co.s")
    public int field1520 = -1;

    @ObfuscatedName("co.y")
    public int[] field1525;

    @ObfuscatedName("co.t")
    public int[] field1522;

    @ObfuscatedName("co.w")
    public int[] field1521;

    @ObfuscatedName("co.h")
    public int[] field1524;

    @ObfuscatedName("co.d")
    public boolean field1519 = false;

    public class88(byte[] arg0, class102 arg1) {
        this.field1515 = arg1;
        class119 var3 = new class119(arg0);
        class119 var4 = new class119(arg0);
        var3.field1974 = 2;
        int var5 = var3.method2427();
        int var6 = -1;
        int var7 = 0;
        var4.field1974 = var3.field1974 + var5;
        for (int var8 = 0; var8 < var5; var8++) {
            int var9 = var3.method2427();
            if (var9 > 0) {
                if (this.field1515.field1752[var8] != 0) {
                    for (int var10 = var8 - 1; var10 > var6; var10--) {
                        if (this.field1515.field1752[var10] == 0) {
                            field1518[var7] = var10;
                            field1516[var7] = 0;
                            field1517[var7] = 0;
                            field1523[var7] = 0;
                            var7++;
                            break;
                        }
                    }
                }
                field1518[var7] = var8;
                short var11 = 0;
                if (this.field1515.field1752[var8] == 3) {
                    var11 = 128;
                }
                if ((var9 & 0x1) == 0) {
                    field1516[var7] = var11;
                } else {
                    field1516[var7] = var4.method2349();
                }
                if ((var9 & 0x2) == 0) {
                    field1517[var7] = var11;
                } else {
                    field1517[var7] = var4.method2349();
                }
                if ((var9 & 0x4) == 0) {
                    field1523[var7] = var11;
                } else {
                    field1523[var7] = var4.method2349();
                }
                var6 = var8;
                var7++;
                if (this.field1515.field1752[var8] == 5) {
                    this.field1519 = true;
                }
            }
        }
        if (var4.field1974 != arg0.length) {
            throw new RuntimeException();
        }
        this.field1520 = var7;
        this.field1525 = new int[var7];
        this.field1522 = new int[var7];
        this.field1521 = new int[var7];
        this.field1524 = new int[var7];
        for (int var12 = 0; var12 < var7; var12++) {
            this.field1525[var12] = field1518[var12];
            this.field1522[var12] = field1516[var12];
            this.field1521[var12] = field1517[var12];
            this.field1524[var12] = field1523[var12];
        }
    }
}
