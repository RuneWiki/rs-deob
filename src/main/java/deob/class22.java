package deob;

import java.io.BufferedReader;
import java.io.IOException;
import java.io.InputStream;
import java.io.InputStreamReader;
import java.net.HttpURLConnection;

@ObfuscatedName("q")
public class class22 {

    @ObfuscatedName("q.c")
    public final int field116;

    @ObfuscatedName("q.b")
    public final String field115;

    public class22(HttpURLConnection arg0) throws IOException {
        this.field116 = arg0.getResponseCode();
        arg0.getResponseMessage();
        arg0.getHeaderFields();
        StringBuilder var2 = new StringBuilder();
        InputStream var3 = this.field116 >= 300 ? arg0.getErrorStream() : arg0.getInputStream();
        if (var3 != null) {
            InputStreamReader var4 = new InputStreamReader(var3);
            BufferedReader var5 = new BufferedReader(var4);
            String var6;
            while ((var6 = var5.readLine()) != null) {
                var2.append(var6);
            }
            var3.close();
        }
        this.field115 = var2.toString();
    }

    public class22(String arg0) {
        this.field116 = 400;
        this.field115 = "";
    }

    @ObfuscatedName("q.c(I)I")
    public int method330() {
        return this.field116;
    }

    @ObfuscatedName("q.b(S)Ljava/lang/String;")
    public String method331() {
        return this.field115;
    }
}
