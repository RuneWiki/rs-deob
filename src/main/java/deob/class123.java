package deob;

import java.awt.EventQueue;
import java.awt.Toolkit;
import java.io.DataInputStream;
import java.net.InetAddress;
import java.net.Socket;
import java.net.URL;

@ObfuscatedName("dk")
public class class123 implements Runnable {

    @ObfuscatedName("dk.y")
    public class47 field1935;

    @ObfuscatedName("dk.b")
    public class133 field1936 = null;

    @ObfuscatedName("dk.w")
    public class133 field1942 = null;

    @ObfuscatedName("dk.r")
    public Thread field1938;

    @ObfuscatedName("dk.l")
    public boolean field1939 = false;

    @ObfuscatedName("dk.s")
    public EventQueue field1945;

    public class123() {
        Statics.field1940 = "Unknown";
        Statics.field1934 = "1.1";
        try {
            Statics.field1940 = System.getProperty("java.vendor");
            Statics.field1934 = System.getProperty("java.version");
        } catch (Exception var4) {
        }
        try {
            this.field1945 = Toolkit.getDefaultToolkit().getSystemEventQueue();
        } catch (Throwable var3) {
        }
        this.field1939 = false;
        this.field1938 = new Thread(this);
        this.field1938.setPriority(10);
        this.field1938.setDaemon(true);
        this.field1938.start();
    }

    @ObfuscatedName("dk.o(I)V")
    public final void method2411() {
        synchronized (this) {
            this.field1939 = true;
            this.notifyAll();
        }
        try {
            this.field1938.join();
        } catch (InterruptedException var4) {
        }
    }

    public final void run() {
        while (true) {
            class133 var2;
            synchronized (this) {
                while (true) {
                    if (this.field1939) {
                        return;
                    }
                    if (this.field1936 != null) {
                        var2 = this.field1936;
                        this.field1936 = this.field1936.field2059;
                        if (this.field1936 == null) {
                            this.field1942 = null;
                        }
                        break;
                    }
                    try {
                        this.wait();
                    } catch (InterruptedException var12) {
                    }
                }
            }
            try {
                int var5 = var2.field2060;
                if (var5 == 1) {
                    var2.field2067 = new Socket(InetAddress.getByName((String) var2.field2066), var2.field2065);
                } else if (var5 == 2) {
                    Thread var6 = new Thread((Runnable) var2.field2066);
                    var6.setDaemon(true);
                    var6.start();
                    var6.setPriority(var2.field2065);
                    var2.field2067 = var6;
                } else if (var5 == 4) {
                    var2.field2067 = new DataInputStream(((URL) var2.field2066).openStream());
                } else if (var5 == 3) {
                    String var7 = (var2.field2065 >> 24 & 0xFF) + "." + (var2.field2065 >> 16 & 0xFF) + "." + (var2.field2065 >> 8 & 0xFF) + "." + (var2.field2065 & 0xFF);
                    var2.field2067 = InetAddress.getByName(var7).getHostName();
                }
                var2.field2064 = 1;
            } catch (ThreadDeath var10) {
                throw var10;
            } catch (Throwable var11) {
                var2.field2064 = 2;
            }
        }
    }

    @ObfuscatedName("dk.y(IIILjava/lang/Object;I)Lev;")
    public final class133 method2412(int arg0, int arg1, int arg2, Object arg3) {
        class133 var5 = new class133();
        var5.field2060 = arg0;
        var5.field2065 = arg1;
        var5.field2066 = arg3;
        synchronized (this) {
            if (this.field1942 == null) {
                this.field1942 = this.field1936 = var5;
            } else {
                this.field1942.field2059 = var5;
                this.field1942 = var5;
            }
            this.notify();
            return var5;
        }
    }

    @ObfuscatedName("dk.b(Ljava/lang/String;II)Lev;")
    public final class133 method2413(String arg0, int arg1) {
        return this.method2412(1, arg1, 0, arg0);
    }

    @ObfuscatedName("dk.w(Ljava/lang/Runnable;IB)Lev;")
    public final class133 method2414(Runnable arg0, int arg1) {
        return this.method2412(2, arg1, 0, arg0);
    }

    @ObfuscatedName("dk.r(II)Lev;")
    public final class133 method2429(int arg0) {
        return this.method2412(3, arg0, 0, (Object) null);
    }

    @ObfuscatedName("dk.l(Ljava/net/URL;B)Lev;")
    public final class133 method2416(URL arg0) {
        return this.method2412(4, 0, 0, arg0);
    }

    @ObfuscatedName("dk.s(B)Laz;")
    public final class47 method2417() {
        return this.field1935;
    }
}
