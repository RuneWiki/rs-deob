package deob;

@ObfuscatedName("df")
public abstract class class120 {

    @ObfuscatedName("df.y")
    public static boolean field1934 = false;

    @ObfuscatedName("bk.f([BZI)Ljava/lang/Object;")
    public static Object method1574(byte[] arg0, boolean arg1) {
        if (arg0 == null) {
            return null;
        }
        if (arg0.length > 136 && !field1934) {
            try {
                class113 var2 = new class113();
                var2.method2304(arg0);
                return var2;
            } catch (Throwable var7) {
                field1934 = true;
            }
        }
        if (arg1) {
            int var4 = arg0.length;
            byte[] var5 = new byte[var4];
            System.arraycopy(arg0, 0, var5, 0, var4);
            return var5;
        } else {
            return arg0;
        }
    }

    @ObfuscatedName("du.i(Ljava/lang/Object;ZI)[B")
    public static byte[] method2605(Object arg0, boolean arg1) {
        if (arg0 == null) {
            return null;
        } else if (arg0 instanceof byte[]) {
            byte[] var2 = (byte[]) ((byte[]) arg0);
            if (arg1) {
                int var3 = var2.length;
                byte[] var4 = new byte[var3];
                System.arraycopy(var2, 0, var4, 0, var3);
                return var4;
            } else {
                return var2;
            }
        } else if (arg0 instanceof class120) {
            class120 var6 = (class120) arg0;
            return var6.method2300();
        } else {
            throw new IllegalArgumentException();
        }
    }

    @ObfuscatedName("df.j(I)[B")
    public abstract byte[] method2300();

    @ObfuscatedName("df.y([BB)V")
    public abstract void method2304(byte[] arg0);
}
