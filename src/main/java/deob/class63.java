package deob;

import java.awt.Component;
import javax.sound.sampled.AudioFormat;
import javax.sound.sampled.AudioSystem;
import javax.sound.sampled.LineUnavailableException;
import javax.sound.sampled.SourceDataLine;
import javax.sound.sampled.DataLine.Info;

@ObfuscatedName("be")
public class class63 extends class50 {

    @ObfuscatedName("be.e")
    public AudioFormat field1189;

    @ObfuscatedName("be.o")
    public SourceDataLine field1188;

    @ObfuscatedName("be.af")
    public int field1190;

    @ObfuscatedName("be.ao")
    public byte[] field1191;

    @ObfuscatedName("be.e(Ljava/awt/Component;)V")
    public void method908(Component arg0) {
        this.field1189 = new AudioFormat((float) Statics.field1095, 16, Statics.field1761 ? 2 : 1, true, false);
        this.field1191 = new byte[0x100 << (Statics.field1761 ? 2 : 1)];
    }

    @ObfuscatedName("be.o(I)V")
    public void method909(int arg0) throws LineUnavailableException {
        try {
            Info var2 = new Info(SourceDataLine.class, this.field1189, arg0 << (Statics.field1761 ? 2 : 1));
            this.field1188 = (SourceDataLine) AudioSystem.getLine(var2);
            this.field1188.open();
            this.field1188.start();
            this.field1190 = arg0;
        } catch (LineUnavailableException var18) {
            int var5 = (arg0 >>> 1 & 0x55555555) + (arg0 & 0x55555555);
            int var6 = (var5 >>> 2 & 0x33333333) + (var5 & 0x33333333);
            int var7 = (var6 >>> 4) + var6 & 0xF0F0F0F;
            int var8 = (var7 >>> 8) + var7;
            int var9 = (var8 >>> 16) + var8;
            int var10 = var9 & 0xFF;
            if (var10 == 1) {
                this.field1188 = null;
                throw var18;
            }
            int var11 = arg0 - 1;
            int var12 = var11 | var11 >>> 1;
            int var13 = var12 | var12 >>> 2;
            int var14 = var13 | var13 >>> 4;
            int var15 = var14 | var14 >>> 8;
            int var16 = var15 | var15 >>> 16;
            int var17 = var16 + 1;
            this.method909(var17);
        }
    }

    @ObfuscatedName("be.y()I")
    public int method910() {
        return this.field1190 - (this.field1188.available() >> (Statics.field1761 ? 2 : 1));
    }

    @ObfuscatedName("be.b()V")
    public void method916() {
        int var1 = 256;
        if (Statics.field1761) {
            var1 <<= 0x1;
        }
        for (int var2 = 0; var2 < var1; var2++) {
            int var3 = this.field1085[var2];
            if ((var3 + 8388608 & 0xFF000000) != 0) {
                var3 = 0x7FFFFF ^ var3 >> 31;
            }
            this.field1191[var2 * 2] = (byte) (var3 >> 8);
            this.field1191[var2 * 2 + 1] = (byte) (var3 >> 16);
        }
        this.field1188.write(this.field1191, 0, var1 << 1);
    }

    @ObfuscatedName("be.w()V")
    public void method912() {
        if (this.field1188 != null) {
            this.field1188.close();
            this.field1188 = null;
        }
    }

    @ObfuscatedName("be.r()V")
    public void method907() {
        this.field1188.flush();
    }
}
