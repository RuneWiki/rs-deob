package deob;

import java.awt.Component;
import javax.sound.sampled.AudioFormat;
import javax.sound.sampled.AudioSystem;
import javax.sound.sampled.LineUnavailableException;
import javax.sound.sampled.SourceDataLine;
import javax.sound.sampled.DataLine.Info;

@ObfuscatedName("bu")
public class class60 extends class58 {

    @ObfuscatedName("bu.ag")
    public AudioFormat field1070;

    @ObfuscatedName("bu.al")
    public SourceDataLine field1069;

    @ObfuscatedName("bu.an")
    public int field1071;

    @ObfuscatedName("bu.af")
    public byte[] field1072;

    @ObfuscatedName("bu.v(Ljava/awt/Component;I)V")
    public void method1011(Component arg0) {
        this.field1070 = new AudioFormat((float) Statics.field66, 16, Statics.field1056 ? 2 : 1, true, false);
        this.field1072 = new byte[0x100 << (Statics.field1056 ? 2 : 1)];
    }

    @ObfuscatedName("bu.j(II)V")
    public void method1002(int arg0) throws LineUnavailableException {
        try {
            Info var2 = new Info(SourceDataLine.class, this.field1070, arg0 << (Statics.field1056 ? 2 : 1));
            this.field1069 = (SourceDataLine) AudioSystem.getLine(var2);
            this.field1069.open();
            this.field1069.start();
            this.field1071 = arg0;
        } catch (LineUnavailableException var11) {
            if (class158.method3018(arg0) == 1) {
                this.field1069 = null;
                throw var11;
            }
            int var4 = arg0 - 1;
            int var5 = var4 | var4 >>> 1;
            int var6 = var5 | var5 >>> 2;
            int var7 = var6 | var6 >>> 4;
            int var8 = var7 | var7 >>> 8;
            int var9 = var8 | var8 >>> 16;
            int var10 = var9 + 1;
            this.method1002(var10);
        }
    }

    @ObfuscatedName("bu.p(I)I")
    public int method1014() {
        return this.field1071 - (this.field1069.available() >> (Statics.field1056 ? 2 : 1));
    }

    @ObfuscatedName("bu.k()V")
    public void method1013() {
        int var1 = 256;
        if (Statics.field1056) {
            var1 <<= 0x1;
        }
        for (int var2 = 0; var2 < var1; var2++) {
            int var3 = this.field1036[var2];
            if ((var3 + 8388608 & 0xFF000000) != 0) {
                var3 = 0x7FFFFF ^ var3 >> 31;
            }
            this.field1072[var2 * 2] = (byte) (var3 >> 8);
            this.field1072[var2 * 2 + 1] = (byte) (var3 >> 16);
        }
        this.field1069.write(this.field1072, 0, var1 << 1);
    }

    @ObfuscatedName("bu.r(I)V")
    public void method1030() {
        if (this.field1069 != null) {
            this.field1069.close();
            this.field1069 = null;
        }
    }

    @ObfuscatedName("bu.y(I)V")
    public void method1016() {
        this.field1069.flush();
    }
}
