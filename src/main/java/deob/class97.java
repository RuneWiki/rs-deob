package deob;

@ObfuscatedName("cs")
public class class97 {

    @ObfuscatedName("cs.j")
    public static int[] field1645 = new int[500];

    @ObfuscatedName("cs.f")
    public static int[] field1637 = new int[500];

    @ObfuscatedName("cs.o")
    public static int[] field1638 = new int[500];

    @ObfuscatedName("cs.h")
    public static int[] field1636 = new int[500];

    @ObfuscatedName("cs.u")
    public class105 field1640 = null;

    @ObfuscatedName("cs.a")
    public int field1641 = -1;

    @ObfuscatedName("cs.q")
    public int[] field1642;

    @ObfuscatedName("cs.t")
    public int[] field1643;

    @ObfuscatedName("cs.n")
    public int[] field1639;

    @ObfuscatedName("cs.i")
    public int[] field1646;

    @ObfuscatedName("cs.c")
    public boolean field1644 = false;

    public class97(byte[] arg0, class105 arg1) {
        this.field1640 = arg1;
        class127 var3 = new class127(arg0);
        class127 var4 = new class127(arg0);
        var3.field2035 = 2;
        int var5 = var3.method2464();
        int var6 = -1;
        int var7 = 0;
        var4.field2035 = var3.field2035 + var5;
        for (int var8 = 0; var8 < var5; var8++) {
            int var9 = var3.method2464();
            if (var9 > 0) {
                if (this.field1640.field1794[var8] != 0) {
                    for (int var10 = var8 - 1; var10 > var6; var10--) {
                        if (this.field1640.field1794[var10] == 0) {
                            field1645[var7] = var10;
                            field1637[var7] = 0;
                            field1638[var7] = 0;
                            field1636[var7] = 0;
                            var7++;
                            break;
                        }
                    }
                }
                field1645[var7] = var8;
                short var11 = 0;
                if (this.field1640.field1794[var8] == 3) {
                    var11 = 128;
                }
                if ((var9 & 0x1) == 0) {
                    field1637[var7] = var11;
                } else {
                    field1637[var7] = var4.method2476();
                }
                if ((var9 & 0x2) == 0) {
                    field1638[var7] = var11;
                } else {
                    field1638[var7] = var4.method2476();
                }
                if ((var9 & 0x4) == 0) {
                    field1636[var7] = var11;
                } else {
                    field1636[var7] = var4.method2476();
                }
                var6 = var8;
                var7++;
                if (this.field1640.field1794[var8] == 5) {
                    this.field1644 = true;
                }
            }
        }
        if (var4.field2035 != arg0.length) {
            throw new RuntimeException();
        }
        this.field1641 = var7;
        this.field1642 = new int[var7];
        this.field1643 = new int[var7];
        this.field1639 = new int[var7];
        this.field1646 = new int[var7];
        for (int var12 = 0; var12 < var7; var12++) {
            this.field1642[var12] = field1645[var12];
            this.field1643[var12] = field1637[var12];
            this.field1639[var12] = field1638[var12];
            this.field1646[var12] = field1636[var12];
        }
    }
}
