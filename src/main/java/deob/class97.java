package deob;

@ObfuscatedName("cu")
public class class97 {

    @ObfuscatedName("cu.i")
    public static int[] field1638 = new int[500];

    @ObfuscatedName("cu.p")
    public static int[] field1634 = new int[500];

    @ObfuscatedName("cu.a")
    public static int[] field1635 = new int[500];

    @ObfuscatedName("cu.l")
    public static int[] field1636 = new int[500];

    @ObfuscatedName("cu.q")
    public class105 field1637 = null;

    @ObfuscatedName("cu.b")
    public int field1642 = -1;

    @ObfuscatedName("cu.u")
    public int[] field1639;

    @ObfuscatedName("cu.d")
    public int[] field1633;

    @ObfuscatedName("cu.m")
    public int[] field1641;

    @ObfuscatedName("cu.v")
    public int[] field1640;

    @ObfuscatedName("cu.j")
    public boolean field1643 = false;

    public class97(byte[] arg0, class105 arg1) {
        this.field1637 = arg1;
        class127 var3 = new class127(arg0);
        class127 var4 = new class127(arg0);
        var3.field2037 = 2;
        int var5 = var3.method2411();
        int var6 = -1;
        int var7 = 0;
        var4.field2037 = var3.field2037 + var5;
        for (int var8 = 0; var8 < var5; var8++) {
            int var9 = var3.method2411();
            if (var9 > 0) {
                if (this.field1637.field1797[var8] != 0) {
                    for (int var10 = var8 - 1; var10 > var6; var10--) {
                        if (this.field1637.field1797[var10] == 0) {
                            field1638[var7] = var10;
                            field1634[var7] = 0;
                            field1635[var7] = 0;
                            field1636[var7] = 0;
                            var7++;
                            break;
                        }
                    }
                }
                field1638[var7] = var8;
                short var11 = 0;
                if (this.field1637.field1797[var8] == 3) {
                    var11 = 128;
                }
                if ((var9 & 0x1) == 0) {
                    field1634[var7] = var11;
                } else {
                    field1634[var7] = var4.method2423();
                }
                if ((var9 & 0x2) == 0) {
                    field1635[var7] = var11;
                } else {
                    field1635[var7] = var4.method2423();
                }
                if ((var9 & 0x4) == 0) {
                    field1636[var7] = var11;
                } else {
                    field1636[var7] = var4.method2423();
                }
                var6 = var8;
                var7++;
                if (this.field1637.field1797[var8] == 5) {
                    this.field1643 = true;
                }
            }
        }
        if (var4.field2037 != arg0.length) {
            throw new RuntimeException();
        }
        this.field1642 = var7;
        this.field1639 = new int[var7];
        this.field1633 = new int[var7];
        this.field1641 = new int[var7];
        this.field1640 = new int[var7];
        for (int var12 = 0; var12 < var7; var12++) {
            this.field1639[var12] = field1638[var12];
            this.field1633[var12] = field1634[var12];
            this.field1641[var12] = field1635[var12];
            this.field1640[var12] = field1636[var12];
        }
    }
}
