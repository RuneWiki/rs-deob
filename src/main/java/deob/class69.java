package deob;

import java.awt.Component;
import java.awt.Graphics;
import java.awt.Image;
import java.awt.Shape;
import java.awt.image.ColorModel;
import java.awt.image.DirectColorModel;
import java.awt.image.ImageConsumer;
import java.awt.image.ImageObserver;
import java.awt.image.ImageProducer;
import java.util.Hashtable;

@ObfuscatedName("bl")
public final class class69 extends class71 implements ImageProducer, ImageObserver {

    @ObfuscatedName("bl.j")
    public ColorModel field1279;

    @ObfuscatedName("bl.z")
    public ImageConsumer field1283;

    @ObfuscatedName("bl.j(IILjava/awt/Component;I)V")
    public final void method1364(int arg0, int arg1, Component arg2) {
        this.field1294 = arg0;
        this.field1295 = arg1;
        this.field1299 = new int[arg0 * arg1 + 1];
        this.field1279 = new DirectColorModel(32, 16711680, 65280, 255);
        this.field1296 = arg2.createImage(this);
        this.method1342();
        arg2.prepareImage(this.field1296, this);
        this.method1342();
        arg2.prepareImage(this.field1296, this);
        this.method1342();
        arg2.prepareImage(this.field1296, this);
        this.method1418();
    }

    @ObfuscatedName("bl.z(Ljava/awt/Graphics;III)V")
    public final void method1338(Graphics arg0, int arg1, int arg2) {
        this.method1342();
        arg0.drawImage(this.field1296, arg1, arg2, this);
    }

    @ObfuscatedName("bl.y(Ljava/awt/Graphics;IIIII)V")
    public final void method1339(Graphics arg0, int arg1, int arg2, int arg3, int arg4) {
        this.method1343(arg1, arg2, arg3, arg4);
        Shape var6 = arg0.getClip();
        arg0.clipRect(arg1, arg2, arg3, arg4);
        arg0.drawImage(this.field1296, 0, 0, this);
        arg0.setClip(var6);
    }

    public synchronized void addConsumer(ImageConsumer arg0) {
        this.field1283 = arg0;
        arg0.setDimensions(this.field1294, this.field1295);
        arg0.setProperties((Hashtable) null);
        arg0.setColorModel(this.field1279);
        arg0.setHints(14);
    }

    public synchronized boolean isConsumer(ImageConsumer arg0) {
        return this.field1283 == arg0;
    }

    public synchronized void removeConsumer(ImageConsumer arg0) {
        if (this.field1283 == arg0) {
            this.field1283 = null;
        }
    }

    public void startProduction(ImageConsumer arg0) {
        this.addConsumer(arg0);
    }

    public void requestTopDownLeftRightResend(ImageConsumer arg0) {
    }

    @ObfuscatedName("bl.h(I)V")
    public synchronized void method1342() {
        if (this.field1283 != null) {
            this.field1283.setPixels(0, 0, this.field1294, this.field1295, this.field1279, this.field1299, 0, this.field1294);
            this.field1283.imageComplete(2);
        }
    }

    @ObfuscatedName("bl.r(IIIIB)V")
    public synchronized void method1343(int arg0, int arg1, int arg2, int arg3) {
        if (this.field1283 != null) {
            this.field1283.setPixels(arg0, arg1, arg2, arg3, this.field1279, this.field1299, this.field1294 * arg1 + arg0, this.field1294);
            this.field1283.imageComplete(2);
        }
    }

    public boolean imageUpdate(Image arg0, int arg1, int arg2, int arg3, int arg4, int arg5) {
        return true;
    }
}
