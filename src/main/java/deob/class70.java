package deob;

import java.awt.Component;
import javax.sound.sampled.AudioFormat;
import javax.sound.sampled.AudioSystem;
import javax.sound.sampled.LineUnavailableException;
import javax.sound.sampled.SourceDataLine;
import javax.sound.sampled.DataLine.Info;

@ObfuscatedName("bt")
public class class70 extends class57 {

    @ObfuscatedName("bt.y")
    public AudioFormat field1284;

    @ObfuscatedName("bt.d")
    public SourceDataLine field1285;

    @ObfuscatedName("bt.ai")
    public int field1286;

    @ObfuscatedName("bt.ab")
    public byte[] field1287;

    @ObfuscatedName("bt.y(Ljava/awt/Component;)V")
    public void method1101(Component arg0) {
        this.field1284 = new AudioFormat((float) Statics.field2644, 16, Statics.field1178 ? 2 : 1, true, false);
        this.field1287 = new byte[0x100 << (Statics.field1178 ? 2 : 1)];
    }

    @ObfuscatedName("bt.d(I)V")
    public void method1106(int arg0) throws LineUnavailableException {
        try {
            Info var2 = new Info(SourceDataLine.class, this.field1284, arg0 << (Statics.field1178 ? 2 : 1));
            this.field1285 = (SourceDataLine) AudioSystem.getLine(var2);
            this.field1285.open();
            this.field1285.start();
            this.field1286 = arg0;
        } catch (LineUnavailableException var11) {
            if (class126.method577(arg0) == 1) {
                this.field1285 = null;
                throw var11;
            }
            int var4 = arg0 - 1;
            int var5 = var4 | var4 >>> 1;
            int var6 = var5 | var5 >>> 2;
            int var7 = var6 | var6 >>> 4;
            int var8 = var7 | var7 >>> 8;
            int var9 = var8 | var8 >>> 16;
            int var10 = var9 + 1;
            this.method1106(var10);
        }
    }

    @ObfuscatedName("bt.g()I")
    public int method1110() {
        return this.field1286 - (this.field1285.available() >> (Statics.field1178 ? 2 : 1));
    }

    @ObfuscatedName("bt.w()V")
    public void method1090() {
        int var1 = 256;
        if (Statics.field1178) {
            var1 <<= 0x1;
        }
        for (int var2 = 0; var2 < var1; var2++) {
            int var3 = this.field1196[var2];
            if ((var3 + 8388608 & 0xFF000000) != 0) {
                var3 = 0x7FFFFF ^ var3 >> 31;
            }
            this.field1287[var2 * 2] = (byte) (var3 >> 8);
            this.field1287[var2 * 2 + 1] = (byte) (var3 >> 16);
        }
        this.field1285.write(this.field1287, 0, var1 << 1);
    }

    @ObfuscatedName("bt.e()V")
    public void method1093() {
        if (this.field1285 != null) {
            this.field1285.close();
            this.field1285 = null;
        }
    }

    @ObfuscatedName("bt.c()V")
    public void method1107() {
        this.field1285.flush();
    }
}
