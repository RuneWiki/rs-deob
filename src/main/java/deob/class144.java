package deob;

import java.io.DataInputStream;
import java.io.IOException;
import java.net.HttpURLConnection;
import java.net.URL;
import java.net.URLConnection;
import java.util.LinkedList;
import java.util.Queue;

@ObfuscatedName("ez")
public class class144 implements Runnable {

    @ObfuscatedName("ez.q")
    public final Thread field1950 = new Thread(this);

    @ObfuscatedName("ez.w")
    public volatile boolean field1947;

    @ObfuscatedName("ez.e")
    public Queue field1946 = new LinkedList();

    public class144() {
        this.field1950.setPriority(1);
        this.field1950.start();
    }

    public void run() {
        while (!this.field1947) {
            try {
                class145 var2;
                synchronized (this) {
                    var2 = (class145) this.field1946.poll();
                    if (var2 == null) {
                        try {
                            this.wait();
                        } catch (InterruptedException var17) {
                        }
                        continue;
                    }
                }
                DataInputStream var5 = null;
                URLConnection var6 = null;
                try {
                    var6 = var2.field1953.openConnection();
                    var6.setConnectTimeout(5000);
                    var6.setReadTimeout(5000);
                    var6.setUseCaches(false);
                    var6.setRequestProperty("Connection", "close");
                    int var7 = var6.getContentLength();
                    if (var7 >= 0) {
                        byte[] var8 = new byte[var7];
                        var5 = new DataInputStream(var6.getInputStream());
                        var5.readFully(var8);
                        var2.field1952 = var8;
                    }
                    var2.field1958 = true;
                } catch (IOException var18) {
                    var2.field1958 = true;
                } finally {
                    if (var5 != null) {
                        var5.close();
                    }
                    if (var6 != null && var6 instanceof HttpURLConnection) {
                        ((HttpURLConnection) var6).disconnect();
                    }
                }
            } catch (Exception var21) {
                class342.method4173((String) null, var21);
            }
        }
    }

    @ObfuscatedName("ez.q(Ljava/net/URL;B)Lex;")
    public class145 method3015(URL arg0) {
        class145 var2 = new class145(arg0);
        synchronized (this) {
            this.field1946.add(var2);
            this.notify();
            return var2;
        }
    }

    @ObfuscatedName("ez.w(B)V")
    public void method3016() {
        this.field1947 = true;
        try {
            synchronized (this) {
                this.notify();
            }
            this.field1950.join();
        } catch (InterruptedException var5) {
        }
    }
}
