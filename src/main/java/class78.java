import org.openrs2.deob.annotation.OriginalClass;
import org.openrs2.deob.annotation.OriginalMember;

@OriginalClass("client!iga")
public class class78 extends class424 {

    @OriginalMember(owner = "client!iga", name = "q", descriptor = "Lra;")
    public static class361 field1054 = new class361(42, 3);

    @OriginalMember(owner = "client!iga", name = "C", descriptor = "I")
    public static int field1066 = 1339;

    @OriginalMember(owner = "client!iga", name = "o", descriptor = "I")
    public int field1052;

    @OriginalMember(owner = "client!iga", name = "p", descriptor = "I")
    public int field1053;

    @OriginalMember(owner = "client!iga", name = "r", descriptor = "I")
    public int field1055;

    @OriginalMember(owner = "client!iga", name = "t", descriptor = "I")
    public int field1057;

    @OriginalMember(owner = "client!iga", name = "u", descriptor = "I")
    public int field1058;

    @OriginalMember(owner = "client!iga", name = "v", descriptor = "I")
    public int field1059;

    @OriginalMember(owner = "client!iga", name = "w", descriptor = "I")
    public int field1060;

    @OriginalMember(owner = "client!iga", name = "y", descriptor = "I")
    public int field1062;

    @OriginalMember(owner = "client!iga", name = "z", descriptor = "I")
    public static int field1063;

    @OriginalMember(owner = "client!iga", name = "A", descriptor = "I")
    public int field1064;

    @OriginalMember(owner = "client!iga", name = "B", descriptor = "I")
    public int field1065;

    @OriginalMember(owner = "client!iga", name = "D", descriptor = "I")
    public int field1067;

    @OriginalMember(owner = "client!iga", name = "E", descriptor = "I")
    public int field1068;

    @OriginalMember(owner = "client!iga", name = "F", descriptor = "I")
    public int field1069;

    @OriginalMember(owner = "client!iga", name = "G", descriptor = "I")
    public int field1070;

    @OriginalMember(owner = "client!iga", name = "H", descriptor = "I")
    public int field1071;

    @OriginalMember(owner = "client!iga", name = "J", descriptor = "I")
    public int field1073;

    @OriginalMember(owner = "client!iga", name = "K", descriptor = "I")
    public int field1074;

    @OriginalMember(owner = "client!iga", name = "L", descriptor = "I")
    public static int field1075;

    @OriginalMember(owner = "client!iga", name = "s", descriptor = "Lpda;")
    public class137 field1056;

    @OriginalMember(owner = "client!iga", name = "x", descriptor = "Lge;")
    public class15 field1061;

    @OriginalMember(owner = "client!iga", name = "n", descriptor = "Ldaa;")
    public class406 field1051;

    @OriginalMember(owner = "client!iga", name = "I", descriptor = "Lbp;")
    public class430 field1072;

    @OriginalMember(owner = "client!iga", name = "a", descriptor = "(I)V", line = 16)
    public final void method625(int arg0) {
        this.field1061 = null;
        this.field1072 = null;
        this.field1051 = null;
        this.field1056 = null;
        if (arg0 == 42) {
            field1063++;
        }
    }

    @OriginalMember(owner = "client!iga", name = "a", descriptor = "(B)V", line = 42)
    public static void method626(byte arg0) {
        if (arg0 == 45) {
            field1054 = null;
        }
    }
}
