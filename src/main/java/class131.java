import java.util.Calendar;
import org.openrs2.deob.annotation.OriginalClass;
import org.openrs2.deob.annotation.OriginalMember;

@OriginalClass("client!fh")
public class class131 extends class157 {

    @OriginalMember(owner = "client!fh", name = "O", descriptor = "I")
    private int field2501 = 6;

    @OriginalMember(owner = "client!fh", name = "P", descriptor = "Lcc;")
    private static class209 field2502 = class95.method669(117, "Attack");

    @OriginalMember(owner = "client!fh", name = "T", descriptor = "[I")
    public static int[] field2506 = new int[] { 76, 8, 137, 4, 0, 1, 38, 2, 19 };

    @OriginalMember(owner = "client!fh", name = "R", descriptor = "I")
    public static int field2504 = 0;

    @OriginalMember(owner = "client!fh", name = "V", descriptor = "Lcc;")
    public static class209 field2508 = field2502;

    @OriginalMember(owner = "client!fh", name = "Q", descriptor = "Lla;")
    public static class95 field2503 = new class95(64);

    @OriginalMember(owner = "client!fh", name = "Z", descriptor = "Lcc;")
    public static class209 field2512 = class95.method669(107, "<img=1>");

    @OriginalMember(owner = "client!fh", name = "X", descriptor = "Lcc;")
    public static class209 field2510 = class95.method669(127, ":duelfriend:");

    @OriginalMember(owner = "client!fh", name = "W", descriptor = "[Z")
    public static boolean[] field2509 = new boolean[5];

    @OriginalMember(owner = "client!fh", name = "Y", descriptor = "Ljava/util/Calendar;")
    public static Calendar field2511 = Calendar.getInstance();

    @OriginalMember(owner = "client!fh", name = "db", descriptor = "Lcc;")
    private static class209 field2516 = class95.method669(84, "You can(Wt add yourself to your own ignore list)3");

    @OriginalMember(owner = "client!fh", name = "bb", descriptor = "I")
    public static int field2514 = 0;

    @OriginalMember(owner = "client!fh", name = "cb", descriptor = "Lcc;")
    public static class209 field2515 = field2516;

    @OriginalMember(owner = "client!fh", name = "N", descriptor = "I")
    public static int field2500;

    @OriginalMember(owner = "client!fh", name = "S", descriptor = "I")
    public static int field2505;

    @OriginalMember(owner = "client!fh", name = "U", descriptor = "I")
    public static int field2507;

    @OriginalMember(owner = "client!fh", name = "ab", descriptor = "Z")
    public static boolean field2513;

    @OriginalMember(owner = "client!fh", name = "a", descriptor = "(BI)[I")
    public final int[] method4(byte arg0, int arg1) {
        if (arg0 != -98) {
            field2503 = null;
        }
        ++field2505;
        int[] var3 = super.field2879.method1269(arg1, 17885);
        if (super.field2879.field3363) {
            int[] var4 = this.method1097((byte) 75, 0, arg1);
            int[] var5 = this.method1097((byte) 75, 1, arg1);
            int var6 = this.field2501;
            if (~var6 != -2) {
                if (~var6 != -3) {
                    if (var6 != 3) {
                        if (~var6 != -5) {
                            if (var6 != 5) {
                                if (var6 != 6) {
                                    if (~var6 != -8) {
                                        if (~var6 != -9) {
                                            if (~var6 != -10) {
                                                if (var6 != 10) {
                                                    if (var6 != 11) {
                                                        if (var6 == 12) {
                                                            for (int var7 = 0; var7 < class226.field4124; ++var7) {
                                                                int var8 = var5[var7];
                                                                int var9 = var4[var7];
                                                                var3[var7] = var9 - -var8 + -(var8 * var9 >> 11);
                                                            }
                                                        }
                                                    } else {
                                                        for (int var10 = 0; var10 < class226.field4124; ++var10) {
                                                            int var11 = var5[var10];
                                                            int var12 = var4[var10];
                                                            var3[var10] = var12 > var11 ? -var11 + var12 : -var12 + var11;
                                                        }
                                                    }
                                                } else {
                                                    for (int var13 = 0; var13 < class226.field4124; ++var13) {
                                                        int var14 = var4[var13];
                                                        int var15 = var5[var13];
                                                        var3[var13] = var15 >= var14 ? var15 : var14;
                                                    }
                                                }
                                            } else {
                                                for (int var16 = 0; ~class226.field4124 < ~var16; ++var16) {
                                                    int var17 = var5[var16];
                                                    int var18 = var4[var16];
                                                    var3[var16] = ~var17 < ~var18 ? var18 : var17;
                                                }
                                            }
                                        } else {
                                            for (int var19 = 0; ~var19 > ~class226.field4124; ++var19) {
                                                int var20 = var4[var19];
                                                var3[var19] = ~var20 == -1 ? 0 : -((-var5[var19] + 4096 << 12) / var20) + 4096;
                                            }
                                        }
                                    } else {
                                        for (int var21 = 0; var21 < class226.field4124; ++var21) {
                                            int var22 = var4[var21];
                                            var3[var21] = var22 == 4096 ? 4096 : (var5[var21] << 12) / (-var22 + 4096);
                                        }
                                    }
                                } else {
                                    for (int var23 = 0; ~class226.field4124 < ~var23; ++var23) {
                                        int var24 = var5[var23];
                                        var3[var23] = var24 < 2048 ? var4[var23] * var24 >> 11 : -((4096 - var24) * (-var4[var23] + 4096) >> 11) + 4096;
                                    }
                                }
                            } else {
                                for (int var25 = 0; ~class226.field4124 < ~var25; ++var25) {
                                    var3[var25] = -((-var4[var25] + 4096) * (-var5[var25] + 4096) >> 12) + 4096;
                                }
                            }
                        } else {
                            for (int var26 = 0; ~var26 > ~class226.field4124; ++var26) {
                                int var27 = var5[var26];
                                var3[var26] = var27 == 0 ? 4096 : (var4[var26] << 12) / var27;
                            }
                        }
                    } else {
                        for (int var28 = 0; var28 < class226.field4124; ++var28) {
                            var3[var28] = var4[var28] * var5[var28] >> 12;
                        }
                    }
                } else {
                    for (int var29 = 0; ~class226.field4124 < ~var29; ++var29) {
                        var3[var29] = var4[var29] + -var5[var29];
                    }
                }
            } else {
                for (int var30 = 0; ~var30 > ~class226.field4124; ++var30) {
                    var3[var30] = var4[var30] + var5[var30];
                }
            }
        }
        return var3;
    }

    @OriginalMember(owner = "client!fh", name = "a", descriptor = "(II)[[I")
    public final int[][] method250(int arg0, int arg1) {
        if (arg0 != 1) {
            return null;
        } else {
            ++field2507;
            int[][] var3 = super.field2872.method533(arg1, 3);
            if (super.field2872.field1434) {
                int[][] var4 = this.method1093(arg1, 0, arg0 + 63);
                int[][] var5 = this.method1093(arg1, 1, 64);
                int[] var6 = var3[0];
                int[] var7 = var3[1];
                int[] var8 = var3[2];
                int[] var9 = var4[0];
                int[] var10 = var4[1];
                int[] var11 = var4[2];
                int[] var12 = var5[0];
                int[] var13 = var5[1];
                int[] var14 = var5[2];
                int var15 = this.field2501;
                if (var15 != 1) {
                    if (var15 != 2) {
                        if (~var15 != -4) {
                            if (~var15 != -5) {
                                if (~var15 != -6) {
                                    if (var15 != 6) {
                                        if (var15 != 7) {
                                            if (~var15 != -9) {
                                                if (~var15 != -10) {
                                                    if (~var15 != -11) {
                                                        if (var15 != 11) {
                                                            if (~var15 == -13) {
                                                                for (int var16 = 0; var16 < class226.field4124; ++var16) {
                                                                    int var17 = var11[var16];
                                                                    int var18 = var14[var16];
                                                                    int var19 = var12[var16];
                                                                    int var20 = var9[var16];
                                                                    int var21 = var10[var16];
                                                                    int var22 = var13[var16];
                                                                    var6[var16] = var19 + var20 - (var19 * var20 >> 11);
                                                                    var7[var16] = var21 + var22 + -(var21 * var22 >> 11);
                                                                    var8[var16] = var17 + var18 + -(var17 * var18 >> 11);
                                                                }
                                                            }
                                                        } else {
                                                            for (int var23 = 0; ~var23 > ~class226.field4124; ++var23) {
                                                                int var24 = var12[var23];
                                                                int var25 = var9[var23];
                                                                int var26 = var11[var23];
                                                                int var27 = var10[var23];
                                                                int var28 = var13[var23];
                                                                int var29 = var14[var23];
                                                                var6[var23] = ~var24 > ~var25 ? -var24 + var25 : -var25 + var24;
                                                                var7[var23] = ~var27 >= ~var28 ? -var27 + var28 : -var28 + var27;
                                                                var8[var23] = var29 < var26 ? -var29 + var26 : -var26 + var29;
                                                            }
                                                        }
                                                    } else {
                                                        for (int var30 = 0; var30 < class226.field4124; ++var30) {
                                                            int var31 = var11[var30];
                                                            int var32 = var14[var30];
                                                            int var33 = var12[var30];
                                                            int var34 = var10[var30];
                                                            int var35 = var13[var30];
                                                            int var36 = var9[var30];
                                                            var6[var30] = var33 >= var36 ? var33 : var36;
                                                            var7[var30] = ~var35 <= ~var34 ? var35 : var34;
                                                            var8[var30] = var32 >= var31 ? var32 : var31;
                                                        }
                                                    }
                                                } else {
                                                    for (int var37 = 0; var37 < class226.field4124; ++var37) {
                                                        int var38 = var9[var37];
                                                        int var39 = var11[var37];
                                                        int var40 = var14[var37];
                                                        int var41 = var13[var37];
                                                        int var42 = var10[var37];
                                                        int var43 = var12[var37];
                                                        var6[var37] = var38 >= var43 ? var43 : var38;
                                                        var7[var37] = var42 < var41 ? var42 : var41;
                                                        var8[var37] = var39 >= var40 ? var40 : var39;
                                                    }
                                                }
                                            } else {
                                                for (int var44 = 0; ~var44 > ~class226.field4124; ++var44) {
                                                    int var45 = var9[var44];
                                                    int var46 = var10[var44];
                                                    int var47 = var11[var44];
                                                    var6[var44] = ~var45 != -1 ? 4096 - (-var12[var44] + 4096 << 12) / var45 : 0;
                                                    var7[var44] = ~var46 != -1 ? 4096 - (-var13[var44] + 4096 << 12) / var46 : 0;
                                                    var8[var44] = ~var47 == -1 ? 0 : -((-var14[var44] + 4096 << 12) / var47) + 4096;
                                                }
                                            }
                                        } else {
                                            for (int var48 = 0; var48 < class226.field4124; ++var48) {
                                                int var49 = var11[var48];
                                                int var50 = var10[var48];
                                                int var51 = var9[var48];
                                                var6[var48] = var51 == 4096 ? 4096 : (var12[var48] << 12) / (4096 - var51);
                                                var7[var48] = ~var50 == -4097 ? 4096 : (var13[var48] << 12) / (4096 - var50);
                                                var8[var48] = var49 != 4096 ? (var14[var48] << 12) / (-var49 + 4096) : 4096;
                                            }
                                        }
                                    } else {
                                        for (int var52 = 0; ~var52 > ~class226.field4124; ++var52) {
                                            int var53 = var14[var52];
                                            int var54 = var13[var52];
                                            int var55 = var12[var52];
                                            var6[var52] = ~var55 <= -2049 ? -((-var9[var52] + 4096) * (-var55 + 4096) >> 11) + 4096 : var9[var52] * var55 >> 11;
                                            var7[var52] = var54 < 2048 ? var10[var52] * var54 >> 11 : -((4096 - var54) * (-var10[var52] + 4096) >> 11) + 4096;
                                            var8[var52] = ~var53 <= -2049 ? -((4096 - var53) * (-var11[var52] + 4096) >> 11) + 4096 : var11[var52] * var53 >> 11;
                                        }
                                    }
                                } else {
                                    for (int var56 = 0; ~var56 > ~class226.field4124; ++var56) {
                                        var6[var56] = 4096 - ((4096 - var12[var56]) * (-var9[var56] + 4096) >> 12);
                                        var7[var56] = 4096 - ((4096 - var13[var56]) * (-var10[var56] + 4096) >> 12);
                                        var8[var56] = -((-var11[var56] + 4096) * (-var14[var56] + 4096) >> 12) + 4096;
                                    }
                                }
                            } else {
                                for (int var57 = 0; ~class226.field4124 < ~var57; ++var57) {
                                    int var58 = var13[var57];
                                    int var59 = var12[var57];
                                    int var60 = var14[var57];
                                    var6[var57] = var59 == 0 ? 4096 : (var9[var57] << 12) / var59;
                                    var7[var57] = ~var58 != -1 ? (var10[var57] << 12) / var58 : 4096;
                                    var8[var57] = ~var60 != -1 ? (var11[var57] << 12) / var60 : 4096;
                                }
                            }
                        } else {
                            for (int var61 = 0; ~var61 > ~class226.field4124; ++var61) {
                                var6[var61] = var9[var61] * var12[var61] >> 12;
                                var7[var61] = var10[var61] * var13[var61] >> 12;
                                var8[var61] = var11[var61] * var14[var61] >> 12;
                            }
                        }
                    } else {
                        for (int var62 = 0; ~var62 > ~class226.field4124; ++var62) {
                            var6[var62] = var9[var62] + -var12[var62];
                            var7[var62] = var10[var62] + -var13[var62];
                            var8[var62] = var11[var62] + -var14[var62];
                        }
                    }
                } else {
                    for (int var63 = 0; ~var63 > ~class226.field4124; ++var63) {
                        var6[var63] = var9[var63] + var12[var63];
                        var7[var63] = var10[var63] + var13[var63];
                        var8[var63] = var11[var63] - -var14[var63];
                    }
                }
            }
            return var3;
        }
    }

    @OriginalMember(owner = "client!fh", name = "h", descriptor = "(I)V")
    public static void method929(int arg0) {
        field2510 = null;
        field2503 = null;
        field2512 = null;
        field2511 = null;
        field2515 = null;
        field2508 = null;
        if (arg0 >= -25) {
            method929(-85);
        }
        field2516 = null;
        field2506 = null;
        field2502 = null;
        field2509 = null;
    }

    @OriginalMember(owner = "client!fh", name = "a", descriptor = "(IBLtf;)V")
    public final void method1(int arg0, byte arg1, class106 arg2) {
        if (arg1 < -29) {
            if (arg0 != 0) {
                if (~arg0 == -2) {
                    super.field2877 = arg2.method774((byte) 126) == 1;
                }
            } else {
                this.field2501 = arg2.method774((byte) 73);
            }
            ++field2500;
        }
    }

    @OriginalMember(owner = "client!fh", name = "<init>", descriptor = "()V")
    public class131() {
        super(2, false);
    }
}
