import org.openrs2.deob.annotation.OriginalClass;
import org.openrs2.deob.annotation.OriginalMember;

@OriginalClass("client!ow")
public class class751 {

    @OriginalMember(owner = "client!ow", name = "d", descriptor = "I")
    public int field10329 = 0;

    @OriginalMember(owner = "client!ow", name = "p", descriptor = "[Z")
    public boolean[] field10341 = new boolean[256];

    @OriginalMember(owner = "client!ow", name = "k", descriptor = "[[B")
    public byte[][] field10336 = new byte[6][258];

    @OriginalMember(owner = "client!ow", name = "r", descriptor = "[I")
    public int[] field10343 = new int[256];

    @OriginalMember(owner = "client!ow", name = "i", descriptor = "[B")
    public byte[] field10334 = new byte[256];

    @OriginalMember(owner = "client!ow", name = "w", descriptor = "[I")
    public int[] field10348 = new int[257];

    @OriginalMember(owner = "client!ow", name = "t", descriptor = "[B")
    public byte[] field10345 = new byte[18002];

    @OriginalMember(owner = "client!ow", name = "D", descriptor = "[[I")
    public int[][] field10355 = new int[6][258];

    @OriginalMember(owner = "client!ow", name = "s", descriptor = "[[I")
    public int[][] field10344 = new int[6][258];

    @OriginalMember(owner = "client!ow", name = "z", descriptor = "[B")
    public byte[] field10351 = new byte[4096];

    @OriginalMember(owner = "client!ow", name = "h", descriptor = "[B")
    public byte[] field10333 = new byte[18002];

    @OriginalMember(owner = "client!ow", name = "H", descriptor = "[I")
    public int[] field10359 = new int[6];

    @OriginalMember(owner = "client!ow", name = "g", descriptor = "I")
    public int field10332 = 0;

    @OriginalMember(owner = "client!ow", name = "f", descriptor = "[[I")
    public int[][] field10331 = new int[6][258];

    @OriginalMember(owner = "client!ow", name = "n", descriptor = "[I")
    public int[] field10339 = new int[16];

    @OriginalMember(owner = "client!ow", name = "E", descriptor = "[Z")
    public boolean[] field10356 = new boolean[16];

    @OriginalMember(owner = "client!ow", name = "m", descriptor = "B")
    public byte field10338;

    @OriginalMember(owner = "client!ow", name = "a", descriptor = "I")
    public int field10326;

    @OriginalMember(owner = "client!ow", name = "b", descriptor = "I")
    public int field10327;

    @OriginalMember(owner = "client!ow", name = "c", descriptor = "I")
    public static int field10328;

    @OriginalMember(owner = "client!ow", name = "j", descriptor = "I")
    public static int field10335;

    @OriginalMember(owner = "client!ow", name = "l", descriptor = "I")
    public int field10337;

    @OriginalMember(owner = "client!ow", name = "o", descriptor = "I")
    public int field10340;

    @OriginalMember(owner = "client!ow", name = "u", descriptor = "I")
    public int field10346;

    @OriginalMember(owner = "client!ow", name = "v", descriptor = "I")
    public int field10347;

    @OriginalMember(owner = "client!ow", name = "x", descriptor = "I")
    public int field10349;

    @OriginalMember(owner = "client!ow", name = "y", descriptor = "I")
    public int field10350;

    @OriginalMember(owner = "client!ow", name = "A", descriptor = "I")
    public int field10352;

    @OriginalMember(owner = "client!ow", name = "B", descriptor = "I")
    public int field10353;

    @OriginalMember(owner = "client!ow", name = "C", descriptor = "I")
    public int field10354;

    @OriginalMember(owner = "client!ow", name = "F", descriptor = "I")
    public int field10357;

    @OriginalMember(owner = "client!ow", name = "G", descriptor = "I")
    public int field10358;

    @OriginalMember(owner = "client!ow", name = "e", descriptor = "[B")
    public byte[] field10330;

    @OriginalMember(owner = "client!ow", name = "q", descriptor = "[B")
    public byte[] field10342;

    @OriginalMember(owner = "client!ow", name = "a", descriptor = "(ZLjava/lang/String;)Z", line = 8)
    public static final boolean method4169(boolean arg0, String arg1) {
        field10335++;
        if (!arg0) {
            method4169(false, null);
        }
        return class581.field7957.containsKey(arg1);
    }

    @OriginalMember(owner = "client!ow", name = "a", descriptor = "(Lda;)V", line = 70)
    public static final void method4170(class402 arg0) {
        class320.field4451 = arg0;
    }
}
