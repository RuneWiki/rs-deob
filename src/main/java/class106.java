import org.openrs2.deob.annotation.OriginalClass;
import org.openrs2.deob.annotation.OriginalMember;

@OriginalClass("client!bu")
public class class106 {

    @OriginalMember(owner = "client!bu", name = "a", descriptor = "I")
    public static int field1729;

    @OriginalMember(owner = "client!bu", name = "a", descriptor = "(B)V", line = 5)
    public static final void method821(byte arg0) {
        field1729++;
        class502.field7343.method2288(0);
        class17.field275.field4360 = 0;
        class492.field7133 = 0;
        class530.field7804 = null;
        class104.field1710 = 0;
        class40.field601 = null;
        class322.field4426 = null;
        class72.field1037 = null;
        class104.field1705 = 0;
        class508.field7414.field4360 = 0;
        for (int var1 = 0; var1 < 100; var1++) {
            class529.field7736[var1] = null;
        }
        class499.field7304 = 0;
        if (arg0 != 54) {
            method821((byte) 77);
        }
        class399.field5724 = 0;
        class372.field5340 = null;
        class347.field4994 = null;
        class167.field2554 = 0;
        class382.field5486 = 0;
    }
}
