import org.openrs2.deob.annotation.OriginalClass;
import org.openrs2.deob.annotation.OriginalMember;

@OriginalClass("l")
public class Occlude {

    @OriginalMember(owner = "l", name = "a", descriptor = "I")
    public int field143;

    @OriginalMember(owner = "l", name = "b", descriptor = "I")
    public int field144;

    @OriginalMember(owner = "l", name = "c", descriptor = "I")
    public int field145;

    @OriginalMember(owner = "l", name = "d", descriptor = "I")
    public int field146;

    @OriginalMember(owner = "l", name = "e", descriptor = "I")
    public int field147;

    @OriginalMember(owner = "l", name = "f", descriptor = "I")
    public int field148;

    @OriginalMember(owner = "l", name = "g", descriptor = "I")
    public int field149;

    @OriginalMember(owner = "l", name = "h", descriptor = "I")
    public int field150;

    @OriginalMember(owner = "l", name = "i", descriptor = "I")
    public int field151;

    @OriginalMember(owner = "l", name = "j", descriptor = "I")
    public int field152;

    @OriginalMember(owner = "l", name = "k", descriptor = "I")
    public int field153;

    @OriginalMember(owner = "l", name = "l", descriptor = "I")
    public int field154;

    @OriginalMember(owner = "l", name = "m", descriptor = "I")
    public int field155;

    @OriginalMember(owner = "l", name = "n", descriptor = "I")
    public int field156;

    @OriginalMember(owner = "l", name = "o", descriptor = "I")
    public int field157;

    @OriginalMember(owner = "l", name = "p", descriptor = "I")
    public int field158;

    @OriginalMember(owner = "l", name = "q", descriptor = "I")
    public int field159;

    @OriginalMember(owner = "l", name = "r", descriptor = "I")
    public int field160;
}
