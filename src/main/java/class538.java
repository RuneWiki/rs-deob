import org.openrs2.deob.annotation.OriginalClass;
import org.openrs2.deob.annotation.OriginalMember;

@OriginalClass("client!jq")
public class class538 {

    @OriginalMember(owner = "client!jq", name = "k", descriptor = "I")
    public static int field7580 = 2;

    @OriginalMember(owner = "client!jq", name = "h", descriptor = "[F")
    public static float[] field7577 = new float[4];

    @OriginalMember(owner = "client!jq", name = "a", descriptor = "I")
    public static int field7570;

    @OriginalMember(owner = "client!jq", name = "b", descriptor = "I")
    public int field7571;

    @OriginalMember(owner = "client!jq", name = "d", descriptor = "I")
    public int field7573;

    @OriginalMember(owner = "client!jq", name = "f", descriptor = "I")
    public int field7575;

    @OriginalMember(owner = "client!jq", name = "g", descriptor = "I")
    public int field7576;

    @OriginalMember(owner = "client!jq", name = "i", descriptor = "I")
    public int field7578;

    @OriginalMember(owner = "client!jq", name = "j", descriptor = "I")
    public static int field7579;

    @OriginalMember(owner = "client!jq", name = "l", descriptor = "I")
    public int field7581;

    @OriginalMember(owner = "client!jq", name = "n", descriptor = "I")
    public int field7583;

    @OriginalMember(owner = "client!jq", name = "m", descriptor = "Ldia;")
    public static class247 field7582;

    @OriginalMember(owner = "client!jq", name = "c", descriptor = "[B")
    public byte[] field7572;

    @OriginalMember(owner = "client!jq", name = "e", descriptor = "[B")
    public byte[] field7574;

    @OriginalMember(owner = "client!jq", name = "a", descriptor = "(I)V", line = 5)
    public static void method3195(int arg0) {
        field7577 = null;
        if (arg0 < 52) {
            field7580 = -19;
        }
        field7582 = null;
    }
}
