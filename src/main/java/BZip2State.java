import org.openrs2.deob.annotation.OriginalClass;
import org.openrs2.deob.annotation.OriginalMember;

@OriginalClass("client!tb")
public class BZip2State {

    @OriginalMember(owner = "client!tb", name = "a", descriptor = "I")
    public final int field788 = 4096;

    @OriginalMember(owner = "client!tb", name = "b", descriptor = "I")
    public final int field789 = 16;

    @OriginalMember(owner = "client!tb", name = "c", descriptor = "I")
    public final int field790 = 258;

    @OriginalMember(owner = "client!tb", name = "d", descriptor = "I")
    public final int field791 = 23;

    @OriginalMember(owner = "client!tb", name = "e", descriptor = "I")
    public final int field792 = 1;

    @OriginalMember(owner = "client!tb", name = "f", descriptor = "I")
    public final int field793 = 6;

    @OriginalMember(owner = "client!tb", name = "g", descriptor = "I")
    public final int field794 = 50;

    @OriginalMember(owner = "client!tb", name = "h", descriptor = "I")
    public final int field795 = 4;

    @OriginalMember(owner = "client!tb", name = "i", descriptor = "I")
    public final int field796 = 18002;

    @OriginalMember(owner = "client!tb", name = "D", descriptor = "[I")
    public int[] field817 = new int[256];

    @OriginalMember(owner = "client!tb", name = "F", descriptor = "[I")
    public int[] field819 = new int[257];

    @OriginalMember(owner = "client!tb", name = "G", descriptor = "[I")
    public int[] field820 = new int[257];

    @OriginalMember(owner = "client!tb", name = "J", descriptor = "[Z")
    public boolean[] field823 = new boolean[256];

    @OriginalMember(owner = "client!tb", name = "K", descriptor = "[Z")
    public boolean[] field824 = new boolean[16];

    @OriginalMember(owner = "client!tb", name = "L", descriptor = "[B")
    public byte[] field825 = new byte[256];

    @OriginalMember(owner = "client!tb", name = "M", descriptor = "[B")
    public byte[] field826 = new byte[4096];

    @OriginalMember(owner = "client!tb", name = "N", descriptor = "[I")
    public int[] field827 = new int[16];

    @OriginalMember(owner = "client!tb", name = "O", descriptor = "[B")
    public byte[] field828 = new byte[18002];

    @OriginalMember(owner = "client!tb", name = "P", descriptor = "[B")
    public byte[] field829 = new byte[18002];

    @OriginalMember(owner = "client!tb", name = "Q", descriptor = "[[B")
    public byte[][] field830 = new byte[6][258];

    @OriginalMember(owner = "client!tb", name = "R", descriptor = "[[I")
    public int[][] field831 = new int[6][258];

    @OriginalMember(owner = "client!tb", name = "S", descriptor = "[[I")
    public int[][] field832 = new int[6][258];

    @OriginalMember(owner = "client!tb", name = "T", descriptor = "[[I")
    public int[][] field833 = new int[6][258];

    @OriginalMember(owner = "client!tb", name = "U", descriptor = "[I")
    public int[] field834 = new int[6];

    @OriginalMember(owner = "client!tb", name = "t", descriptor = "B")
    public byte field807;

    @OriginalMember(owner = "client!tb", name = "k", descriptor = "I")
    public int field798;

    @OriginalMember(owner = "client!tb", name = "l", descriptor = "I")
    public int field799;

    @OriginalMember(owner = "client!tb", name = "m", descriptor = "I")
    public int field800;

    @OriginalMember(owner = "client!tb", name = "n", descriptor = "I")
    public int field801;

    @OriginalMember(owner = "client!tb", name = "p", descriptor = "I")
    public int field803;

    @OriginalMember(owner = "client!tb", name = "q", descriptor = "I")
    public int field804;

    @OriginalMember(owner = "client!tb", name = "r", descriptor = "I")
    public int field805;

    @OriginalMember(owner = "client!tb", name = "s", descriptor = "I")
    public int field806;

    @OriginalMember(owner = "client!tb", name = "u", descriptor = "I")
    public int field808;

    @OriginalMember(owner = "client!tb", name = "w", descriptor = "I")
    public int field810;

    @OriginalMember(owner = "client!tb", name = "x", descriptor = "I")
    public int field811;

    @OriginalMember(owner = "client!tb", name = "y", descriptor = "I")
    public int field812;

    @OriginalMember(owner = "client!tb", name = "z", descriptor = "I")
    public int field813;

    @OriginalMember(owner = "client!tb", name = "A", descriptor = "I")
    public int field814;

    @OriginalMember(owner = "client!tb", name = "B", descriptor = "I")
    public int field815;

    @OriginalMember(owner = "client!tb", name = "C", descriptor = "I")
    public int field816;

    @OriginalMember(owner = "client!tb", name = "E", descriptor = "I")
    public int field818;

    @OriginalMember(owner = "client!tb", name = "I", descriptor = "I")
    public int field822;

    @OriginalMember(owner = "client!tb", name = "V", descriptor = "I")
    public int field835;

    @OriginalMember(owner = "client!tb", name = "v", descriptor = "Z")
    public boolean field809;

    @OriginalMember(owner = "client!tb", name = "j", descriptor = "[B")
    public byte[] field797;

    @OriginalMember(owner = "client!tb", name = "o", descriptor = "[B")
    public byte[] field802;

    @OriginalMember(owner = "client!tb", name = "H", descriptor = "[I")
    public static int[] field821;
}
