import org.openrs2.deob.annotation.OriginalClass;
import org.openrs2.deob.annotation.OriginalMember;

@OriginalClass("client!kba")
public class class527 {

    @OriginalMember(owner = "client!kba", name = "c", descriptor = "Liu;")
    public static class517 field7329 = new class517(87, 8);

    @OriginalMember(owner = "client!kba", name = "g", descriptor = "Z")
    public static boolean field7333 = false;

    @OriginalMember(owner = "client!kba", name = "a", descriptor = "I")
    public int field7327;

    @OriginalMember(owner = "client!kba", name = "b", descriptor = "I")
    public int field7328;

    @OriginalMember(owner = "client!kba", name = "d", descriptor = "I")
    public int field7330;

    @OriginalMember(owner = "client!kba", name = "e", descriptor = "I")
    public int field7331;

    @OriginalMember(owner = "client!kba", name = "f", descriptor = "I")
    public int field7332;

    @OriginalMember(owner = "client!kba", name = "h", descriptor = "I")
    public static int field7334;

    @OriginalMember(owner = "client!kba", name = "i", descriptor = "I")
    public int field7335;

    @OriginalMember(owner = "client!kba", name = "j", descriptor = "I")
    public static int field7336;

    @OriginalMember(owner = "client!kba", name = "k", descriptor = "I")
    public static int field7337;

    @OriginalMember(owner = "client!kba", name = "a", descriptor = "(I)V", line = 6)
    public static void method2971(int arg0) {
        if (arg0 == 17874) {
            field7329 = null;
        }
    }
}
