import java.awt.Canvas;
import java.awt.Rectangle;
import org.openrs2.deob.annotation.OriginalClass;
import org.openrs2.deob.annotation.OriginalMember;

@OriginalClass("client!r")
public abstract class class165 {

    @OriginalMember(owner = "client!r", name = "m", descriptor = "Lfa;")
    public class212 field2023;

    @OriginalMember(owner = "client!r", name = "h", descriptor = "I")
    public int field2018;

    @OriginalMember(owner = "client!r", name = "f", descriptor = "Ljk;")
    public static class585 field2016 = new class585(33, 2);

    @OriginalMember(owner = "client!r", name = "a", descriptor = "I")
    public static int field2011;

    @OriginalMember(owner = "client!r", name = "b", descriptor = "I")
    public static int field2012;

    @OriginalMember(owner = "client!r", name = "c", descriptor = "I")
    public static int field2013;

    @OriginalMember(owner = "client!r", name = "d", descriptor = "I")
    public static int field2014;

    @OriginalMember(owner = "client!r", name = "e", descriptor = "I")
    public static int field2015;

    @OriginalMember(owner = "client!r", name = "g", descriptor = "I")
    public static int field2017;

    @OriginalMember(owner = "client!r", name = "i", descriptor = "I")
    public static int field2019;

    @OriginalMember(owner = "client!r", name = "j", descriptor = "I")
    public static int field2020;

    @OriginalMember(owner = "client!r", name = "k", descriptor = "I")
    public static int field2021;

    @OriginalMember(owner = "client!r", name = "l", descriptor = "I")
    public static int field2022;

    @OriginalMember(owner = "client!r", name = "n", descriptor = "I")
    public static int field2024;

    @OriginalMember(owner = "client!r", name = "o", descriptor = "Lgba;")
    public static class618 field2025;

    @OriginalMember(owner = "client!r", name = "a", descriptor = "(IIIII)V", line = 14)
    public final void method1038(int arg0, int arg1, int arg2, int arg3, int arg4) {
        this.method320(arg1, arg0, arg3, arg4, arg2);
        field2014++;
    }

    @OriginalMember(owner = "client!r", name = "a", descriptor = "(IIIBI)V", line = 26)
    public final void method1039(int arg0, int arg1, int arg2, byte arg3, int arg4) {
        field2022++;
        this.method290(arg4, arg2, arg0, arg1, 1);
        int var6 = -31 % ((1 - arg3) / 52);
    }

    @OriginalMember(owner = "client!r", name = "a", descriptor = "(ILfa;ILjava/awt/Canvas;ILjn;)Lr;", line = 60)
    public static final synchronized class165 method1040(int arg0, class212 arg1, int arg2, Canvas arg3, int arg4, class668 arg5) {
        field2020++;
        if (arg0 == 0) {
            return class209.method1252(arg3, 125, arg1);
        } else if (arg0 == 2) {
            return class327.method1903(arg3, arg1, (byte) 119);
        } else if (arg0 == 1) {
            return class260.method1583(arg3, -103, arg4, arg1);
        } else if (arg0 == 5) {
            return class675.method3838(arg1, arg4, arg5, arg3, 8833);
        } else if (~arg0 == arg2) {
            return class595.method3446(arg5, arg4, arg1, (byte) -83, arg3);
        } else {
            throw new IllegalArgumentException("UM");
        }
    }

    @OriginalMember(owner = "client!r", name = "a", descriptor = "(II)Log;", line = 124)
    public static final class599 method1041(int arg0, int arg1) {
        field2024++;
        class599[] var2 = class181.method1123(-115);
        for (int var3 = 0; var3 < var2.length; var3++) {
            class599 var4 = var2[var3];
            if (var4.field8644 == arg1) {
                return var4;
            }
        }
        if (arg0 != -20310) {
            field2016 = null;
        }
        return null;
    }

    @OriginalMember(owner = "client!r", name = "d", descriptor = "(IIIIII)V", line = 163)
    public final void method1042(int arg0, int arg1, int arg2, int arg3, int arg4, int arg5) {
        field2012++;
        this.method248(arg0, arg3, arg1, arg5, arg4, arg2);
    }

    @OriginalMember(owner = "client!r", name = "finalize", descriptor = "()V", line = 174)
    protected void finalize() {
        this.method1047(1);
        field2011++;
    }

    @OriginalMember(owner = "client!r", name = "a", descriptor = "(B)V", line = 188)
    public static void method1043(byte arg0) {
        field2016 = null;
        field2025 = null;
        if (arg0 != -65) {
            field2025 = null;
        }
    }

    @OriginalMember(owner = "client!r", name = "a", descriptor = "(Lf;B)V", line = 200)
    public final void method1044(class701 arg0, byte arg1) {
        field2015++;
        if (arg1 != 76) {
            field2025 = null;
        }
        this.method241(arg0, true);
    }

    @OriginalMember(owner = "client!r", name = "a", descriptor = "(BIIII)V", line = 219)
    public final void method1045(byte arg0, int arg1, int arg2, int arg3, int arg4) {
        this.method310(arg1, arg4, arg2, arg3, 1);
        field2013++;
        if (arg0 < 16) {
            this.method307(-78);
        }
    }

    @OriginalMember(owner = "client!r", name = "e", descriptor = "(IIIIII)V", line = 242)
    public final void method1046(int arg0, int arg1, int arg2, int arg3, int arg4, int arg5) {
        this.method278(arg5, arg3, arg4, arg1, arg2, 1);
        if (arg0 < 126) {
            this.method303(77, 39, -90, 16, 42, -70);
        }
        field2019++;
    }

    @OriginalMember(owner = "client!r", name = "f", descriptor = "(I)V", line = 259)
    public final void method1047(int arg0) {
        class455.field6397[this.field2018] = false;
        field2021++;
        this.method286();
        if (arg0 != 1) {
            this.method237();
        }
    }

    @OriginalMember(owner = "client!r", name = "<init>", descriptor = "(Lfa;)V", line = 279)
    public class165(class212 arg0) {
        this.field2023 = arg0;
        int var2 = -1;
        for (int var3 = 0; var3 < 8; var3++) {
            if (!class455.field6397[var3]) {
                var2 = var3;
                class455.field6397[var3] = true;
                break;
            }
        }
        if (var2 == -1) {
            throw new IllegalStateException("NFTI");
        }
        this.field2018 = var2;
    }

    @OriginalMember(owner = "client!r", name = "a", descriptor = "(IIIBII)V", line = 318)
    public final void method1048(int arg0, int arg1, int arg2, byte arg3, int arg4, int arg5) {
        this.method259(arg4, arg2, arg1, arg5, arg0, 1);
        if (arg3 > -30) {
            this.method230(-126);
        }
        field2017++;
    }

    @OriginalMember(owner = "client!r", name = "a", descriptor = "(IIIIZ)Lf;")
    public abstract class701 method231(int arg0, int arg1, int arg2, int arg3, boolean arg4);

    @OriginalMember(owner = "client!r", name = "a", descriptor = "()Z")
    public abstract boolean method269();

    @OriginalMember(owner = "client!r", name = "GA", descriptor = "(II)V")
    public abstract void method297(int arg0, int arg1);

    @OriginalMember(owner = "client!r", name = "a", descriptor = "(Ljava/awt/Canvas;)V")
    public abstract void method294(Canvas arg0);

    @OriginalMember(owner = "client!r", name = "a", descriptor = "(II[[I[[IIII)Li;")
    public abstract class37 method220(int arg0, int arg1, int[][] arg2, int[][] arg3, int arg4, int arg5, int arg6);

    @OriginalMember(owner = "client!r", name = "BA", descriptor = "()I")
    public abstract int method260();

    @OriginalMember(owner = "client!r", name = "b", descriptor = "(Ljava/awt/Canvas;)V")
    public abstract void method275(Canvas arg0);

    @OriginalMember(owner = "client!r", name = "va", descriptor = "(I)V")
    public abstract void method312(int arg0);

    @OriginalMember(owner = "client!r", name = "a", descriptor = "([IIIII)Lf;")
    public abstract class701 method311(int[] arg0, int arg1, int arg2, int arg3, int arg4);

    @OriginalMember(owner = "client!r", name = "a", descriptor = "(IIIIII)Lcaa;")
    public abstract class275 method303(int arg0, int arg1, int arg2, int arg3, int arg4, int arg5);

    @OriginalMember(owner = "client!r", name = "c", descriptor = "()I")
    public abstract int method232();

    @OriginalMember(owner = "client!r", name = "a", descriptor = "(I)V")
    public abstract void method263(int arg0);

    @OriginalMember(owner = "client!r", name = "d", descriptor = "()Z")
    public abstract boolean method215();

    @OriginalMember(owner = "client!r", name = "da", descriptor = "(IIII)V")
    public abstract void method292(int arg0, int arg1, int arg2, int arg3);

    @OriginalMember(owner = "client!r", name = "U", descriptor = "()I")
    public abstract int method287();

    @OriginalMember(owner = "client!r", name = "T", descriptor = "()I")
    public abstract int method289();

    @OriginalMember(owner = "client!r", name = "e", descriptor = "()Z")
    public abstract boolean method211();

    @OriginalMember(owner = "client!r", name = "IA", descriptor = "(IIIIII[BII)V")
    public abstract void method324(int arg0, int arg1, int arg2, int arg3, int arg4, int arg5, byte[] arg6, int arg7, int arg8);

    @OriginalMember(owner = "client!r", name = "XA", descriptor = "(IIIII)V")
    public abstract void method290(int arg0, int arg1, int arg2, int arg3, int arg4);

    @OriginalMember(owner = "client!r", name = "f", descriptor = "()Z")
    public abstract boolean method218();

    @OriginalMember(owner = "client!r", name = "a", descriptor = "(IIIIIII)V")
    public abstract void method326(int arg0, int arg1, int arg2, int arg3, int arg4, int arg5, int arg6);

    @OriginalMember(owner = "client!r", name = "a", descriptor = "(IIIIIF)Lul;")
    public abstract class530 method243(int arg0, int arg1, int arg2, int arg3, int arg4, float arg5);

    @OriginalMember(owner = "client!r", name = "b", descriptor = "(I)V")
    public abstract void method307(int arg0);

    @OriginalMember(owner = "client!r", name = "a", descriptor = "(IIII)V")
    public abstract void method254(int arg0, int arg1, int arg2, int arg3);

    @OriginalMember(owner = "client!r", name = "a", descriptor = "(Lcaa;)V")
    public abstract void method277(class275 arg0);

    @OriginalMember(owner = "client!r", name = "a", descriptor = "(Lgda;[Lpga;Z)Lla;")
    public abstract class419 method258(class50 arg0, class495[] arg1, boolean arg2);

    @OriginalMember(owner = "client!r", name = "g", descriptor = "()V")
    public abstract void method302();

    @OriginalMember(owner = "client!r", name = "h", descriptor = "()Lik;")
    public abstract class191 method314();

    @OriginalMember(owner = "client!r", name = "a", descriptor = "([Ljava/awt/Rectangle;I)V")
    public abstract void method240(Rectangle[] arg0, int arg1) throws class365;

    @OriginalMember(owner = "client!r", name = "CA", descriptor = "(IIII)V")
    public abstract void method229(int arg0, int arg1, int arg2, int arg3);

    @OriginalMember(owner = "client!r", name = "a", descriptor = "(Le;)V")
    public abstract void method265(class493 arg0);

    @OriginalMember(owner = "client!r", name = "i", descriptor = "()Z")
    public abstract boolean method299();

    @OriginalMember(owner = "client!r", name = "YA", descriptor = "(IIII)V")
    public abstract void method261(int arg0, int arg1, int arg2, int arg3);

    @OriginalMember(owner = "client!r", name = "oa", descriptor = "([I)V")
    public abstract void method251(int[] arg0);

    @OriginalMember(owner = "client!r", name = "j", descriptor = "()V")
    public abstract void method283();

    @OriginalMember(owner = "client!r", name = "V", descriptor = "(IIII)V")
    public abstract void method300(int arg0, int arg1, int arg2, int arg3);

    @OriginalMember(owner = "client!r", name = "c", descriptor = "(I)V")
    public abstract void method252(int arg0);

    @OriginalMember(owner = "client!r", name = "AA", descriptor = "(III[I)V")
    public abstract void method329(int arg0, int arg1, int arg2, int[] arg3);

    @OriginalMember(owner = "client!r", name = "a", descriptor = "(FFF)V")
    public abstract void method255(float arg0, float arg1, float arg2);

    @OriginalMember(owner = "client!r", name = "ja", descriptor = "(I)V")
    public abstract void method323(int arg0);

    @OriginalMember(owner = "client!r", name = "b", descriptor = "()I")
    public abstract int method214();

    @OriginalMember(owner = "client!r", name = "k", descriptor = "()Z")
    public abstract boolean method233();

    @OriginalMember(owner = "client!r", name = "c", descriptor = "(Ljava/awt/Canvas;)V")
    public abstract void method256(Canvas arg0);

    @OriginalMember(owner = "client!r", name = "m", descriptor = "(IIIII)V")
    public abstract void method310(int arg0, int arg1, int arg2, int arg3, int arg4);

    @OriginalMember(owner = "client!r", name = "a", descriptor = "(IIIIIILua;II)V")
    public abstract void method212(int arg0, int arg1, int arg2, int arg3, int arg4, int arg5, class616 arg6, int arg7, int arg8);

    @OriginalMember(owner = "client!r", name = "J", descriptor = "(IIIIII)V")
    public abstract void method248(int arg0, int arg1, int arg2, int arg3, int arg4, int arg5);

    @OriginalMember(owner = "client!r", name = "b", descriptor = "(IIIIII)V")
    public abstract void method278(int arg0, int arg1, int arg2, int arg3, int arg4, int arg5);

    @OriginalMember(owner = "client!r", name = "a", descriptor = "(II[I[I)Lua;")
    public abstract class616 method213(int arg0, int arg1, int[] arg2, int[] arg3);

    @OriginalMember(owner = "client!r", name = "a", descriptor = "(IIIIIIIIII)V")
    public abstract void method285(int arg0, int arg1, int arg2, int arg3, int arg4, int arg5, int arg6, int arg7, int arg8, int arg9);

    @OriginalMember(owner = "client!r", name = "L", descriptor = "()V")
    public abstract void method291();

    @OriginalMember(owner = "client!r", name = "a", descriptor = "(Lq;)V")
    public abstract void method262(class394 arg0);

    @OriginalMember(owner = "client!r", name = "a", descriptor = "(I[Lul;)V")
    public abstract void method284(int arg0, class530[] arg1);

    @OriginalMember(owner = "client!r", name = "c", descriptor = "(IIIIII)V")
    public abstract void method259(int arg0, int arg1, int arg2, int arg3, int arg4, int arg5);

    @OriginalMember(owner = "client!r", name = "DA", descriptor = "()I")
    public abstract int method327();

    @OriginalMember(owner = "client!r", name = "ra", descriptor = "(F)V")
    public abstract void method239(float arg0);

    @OriginalMember(owner = "client!r", name = "l", descriptor = "()V")
    public abstract void method227();

    @OriginalMember(owner = "client!r", name = "m", descriptor = "()Z")
    public abstract boolean method318();

    @OriginalMember(owner = "client!r", name = "ya", descriptor = "()V")
    public abstract void method246();

    @OriginalMember(owner = "client!r", name = "a", descriptor = "(Lcaa;Lcaa;FLcaa;)Lcaa;")
    public abstract class275 method313(class275 arg0, class275 arg1, float arg2, class275 arg3);

    @OriginalMember(owner = "client!r", name = "n", descriptor = "()Lq;")
    public abstract class394 method315();

    @OriginalMember(owner = "client!r", name = "o", descriptor = "()Z")
    public abstract boolean method224();

    @OriginalMember(owner = "client!r", name = "p", descriptor = "()V")
    public abstract void method321();

    @OriginalMember(owner = "client!r", name = "b", descriptor = "(II)I")
    public abstract int method306(int arg0, int arg1);

    @OriginalMember(owner = "client!r", name = "a", descriptor = "([I)V")
    public abstract void method319(int[] arg0);

    @OriginalMember(owner = "client!r", name = "d", descriptor = "(Ljava/awt/Canvas;)V")
    public abstract void method257(Canvas arg0);

    @OriginalMember(owner = "client!r", name = "q", descriptor = "()V")
    public abstract void method293() throws class365;

    @OriginalMember(owner = "client!r", name = "a", descriptor = "(Lkk;IIII)Lda;")
    public abstract class55 method274(class173 arg0, int arg1, int arg2, int arg3, int arg4);

    @OriginalMember(owner = "client!r", name = "a", descriptor = "(Lso;)V")
    public abstract void method228(class436 arg0);

    @OriginalMember(owner = "client!r", name = "a", descriptor = "(Lpga;Z)Lf;")
    public abstract class701 method216(class495 arg0, boolean arg1);

    @OriginalMember(owner = "client!r", name = "r", descriptor = "()Z")
    public abstract boolean method237();

    @OriginalMember(owner = "client!r", name = "s", descriptor = "()Lq;")
    public abstract class394 method238();

    @OriginalMember(owner = "client!r", name = "h", descriptor = "(IIII)[I")
    public abstract int[] method325(int arg0, int arg1, int arg2, int arg3);

    @OriginalMember(owner = "client!r", name = "ZA", descriptor = "(Z)V")
    public abstract void method309(boolean arg0);

    @OriginalMember(owner = "client!r", name = "t", descriptor = "()Z")
    public abstract boolean method247();

    @OriginalMember(owner = "client!r", name = "c", descriptor = "(II)I")
    public abstract int method271(int arg0, int arg1);

    @OriginalMember(owner = "client!r", name = "a", descriptor = "(Lf;Z)V")
    public abstract void method241(class701 arg0, boolean arg1);

    @OriginalMember(owner = "client!r", name = "u", descriptor = "()Lq;")
    public abstract class394 method305();

    @OriginalMember(owner = "client!r", name = "a", descriptor = "(Lso;I)V")
    public abstract void method223(class436 arg0, int arg1);

    @OriginalMember(owner = "client!r", name = "d", descriptor = "(I)Le;")
    public abstract class493 method210(int arg0);

    @OriginalMember(owner = "client!r", name = "w", descriptor = "()Z")
    public abstract boolean method225();

    @OriginalMember(owner = "client!r", name = "JA", descriptor = "()V")
    public abstract void method249();

    @OriginalMember(owner = "client!r", name = "x", descriptor = "()V")
    public abstract void method286();

    @OriginalMember(owner = "client!r", name = "MA", descriptor = "(III[I)V")
    public abstract void method322(int arg0, int arg1, int arg2, int[] arg3);

    @OriginalMember(owner = "client!r", name = "la", descriptor = "(IIII)V")
    public abstract void method301(int arg0, int arg1, int arg2, int arg3);

    @OriginalMember(owner = "client!r", name = "e", descriptor = "(I)V")
    public abstract void method230(int arg0);

    @OriginalMember(owner = "client!r", name = "y", descriptor = "()V")
    public abstract void method328();

    @OriginalMember(owner = "client!r", name = "FA", descriptor = "(ILua;II)V")
    public abstract void method245(int arg0, class616 arg1, int arg2, int arg3);

    @OriginalMember(owner = "client!r", name = "VA", descriptor = "(IFFFFF)V")
    public abstract void method236(int arg0, float arg1, float arg2, float arg3, float arg4, float arg5);

    @OriginalMember(owner = "client!r", name = "z", descriptor = "()Z")
    public abstract boolean method317();

    @OriginalMember(owner = "client!r", name = "a", descriptor = "(IIZ)Lf;")
    public abstract class701 method304(int arg0, int arg1, boolean arg2);

    @OriginalMember(owner = "client!r", name = "e", descriptor = "(IIIII)V")
    public abstract void method320(int arg0, int arg1, int arg2, int arg3, int arg4);

    @OriginalMember(owner = "client!r", name = "a", descriptor = "(Z)V")
    public abstract void method295(boolean arg0);

    @OriginalMember(owner = "client!r", name = "A", descriptor = "()Z")
    public abstract boolean method226();

    @OriginalMember(owner = "client!r", name = "pa", descriptor = "(III)V")
    public abstract void method273(int arg0, int arg1, int arg2);

    @OriginalMember(owner = "client!r", name = "g", descriptor = "(I)V")
    public abstract void method222(int arg0);

    @OriginalMember(owner = "client!r", name = "B", descriptor = "()V")
    public abstract void method250();

    @OriginalMember(owner = "client!r", name = "C", descriptor = "()Z")
    public abstract boolean method316();

    @OriginalMember(owner = "client!r", name = "b", descriptor = "(Z)V")
    public abstract void method296(boolean arg0);

    @OriginalMember(owner = "client!r", name = "O", descriptor = "(IIIIII)Z")
    public abstract boolean method217(int arg0, int arg1, int arg2, int arg3, int arg4, int arg5);

    @OriginalMember(owner = "client!r", name = "v", descriptor = "()[I")
    public abstract int[] method264();
}
