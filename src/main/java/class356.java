import org.openrs2.deob.annotation.OriginalClass;
import org.openrs2.deob.annotation.OriginalMember;

@OriginalClass("client!tc")
public class class356 extends class656 {

    @OriginalMember(owner = "client!tc", name = "k", descriptor = "Ljava/lang/String;")
    public String field4997;

    @OriginalMember(owner = "client!tc", name = "j", descriptor = "I")
    public static int field4996 = 0;

    @OriginalMember(owner = "client!tc", name = "i", descriptor = "I")
    public static int field4995;

    @OriginalMember(owner = "client!tc", name = "<init>", descriptor = "()V", line = 12)
    public class356() {
    }

    @OriginalMember(owner = "client!tc", name = "<init>", descriptor = "(Ljava/lang/String;I)V", line = 14)
    public class356(String arg0, int arg1) {
        this.field4997 = arg0;
    }
}
