import java.io.IOException;
import java.io.RandomAccessFile;
import org.openrs2.deob.annotation.OriginalClass;
import org.openrs2.deob.annotation.OriginalMember;

@OriginalClass("vb")
public class FileStream {

    @OriginalMember(owner = "vb", name = "a", descriptor = "I")
    private int field843 = -47594;

    @OriginalMember(owner = "vb", name = "b", descriptor = "Z")
    private boolean field844 = false;

    @OriginalMember(owner = "vb", name = "g", descriptor = "I")
    public int field849 = 65000;

    @OriginalMember(owner = "vb", name = "f", descriptor = "I")
    public int field848;

    @OriginalMember(owner = "vb", name = "d", descriptor = "Ljava/io/RandomAccessFile;")
    public RandomAccessFile field846;

    @OriginalMember(owner = "vb", name = "e", descriptor = "Ljava/io/RandomAccessFile;")
    public RandomAccessFile field847;

    @OriginalMember(owner = "vb", name = "c", descriptor = "[B")
    public static byte[] field845 = new byte[520];

    @OriginalMember(owner = "vb", name = "<init>", descriptor = "(IIILjava/io/RandomAccessFile;Ljava/io/RandomAccessFile;)V")
    public FileStream(int arg0, int arg1, int arg2, RandomAccessFile arg3, RandomAccessFile arg4) {
        this.field848 = arg1;
        this.field846 = arg4;
        this.field847 = arg3;
        if (arg0 <= 0) {
            this.field843 = -5;
        }
        this.field849 = arg2;
    }

    @OriginalMember(owner = "vb", name = "a", descriptor = "(II)[B")
    public synchronized byte[] method291(int arg0, int arg1) {
        if (arg0 != 8) {
            throw new NullPointerException();
        }
        try {
            this.method294(arg1 * 6, this.field847, 0);
            int var4;
            for (int var3 = 0; var3 < 6; var3 += var4) {
                var4 = this.field847.read(field845, var3, 6 - var3);
                if (var4 == -1) {
                    return null;
                }
            }
            int var5 = (field845[2] & 0xFF) + ((field845[0] & 0xFF) << 16) + ((field845[1] & 0xFF) << 8);
            int var6 = (field845[5] & 0xFF) + ((field845[3] & 0xFF) << 16) + ((field845[4] & 0xFF) << 8);
            if (var5 < 0 || var5 > this.field849) {
                return null;
            } else if (var6 > 0 && (long) var6 <= this.field846.length() / 520L) {
                byte[] var7 = new byte[var5];
                int var8 = 0;
                int var9 = 0;
                while (var8 < var5) {
                    if (var6 == 0) {
                        return null;
                    }
                    this.method294(var6 * 520, this.field846, 0);
                    int var10 = 0;
                    int var11 = var5 - var8;
                    if (var11 > 512) {
                        var11 = 512;
                    }
                    while (var10 < var11 + 8) {
                        int var12 = this.field846.read(field845, var10, var11 + 8 - var10);
                        if (var12 == -1) {
                            return null;
                        }
                        var10 += var12;
                    }
                    int var13 = ((field845[0] & 0xFF) << 8) + (field845[1] & 0xFF);
                    int var14 = ((field845[2] & 0xFF) << 8) + (field845[3] & 0xFF);
                    int var15 = (field845[6] & 0xFF) + ((field845[4] & 0xFF) << 16) + ((field845[5] & 0xFF) << 8);
                    int var16 = field845[7] & 0xFF;
                    if (arg1 == var13 && var9 == var14 && this.field848 == var16) {
                        if (var15 >= 0 && (long) var15 <= this.field846.length() / 520L) {
                            for (int var17 = 0; var17 < var11; var17++) {
                                var7[var8++] = field845[var17 + 8];
                            }
                            var6 = var15;
                            var9++;
                            continue;
                        }
                        return null;
                    }
                    return null;
                }
                return var7;
            } else {
                return null;
            }
        } catch (IOException var18) {
            return null;
        }
    }

    @OriginalMember(owner = "vb", name = "a", descriptor = "(II[BI)Z")
    public synchronized boolean method292(int arg0, int arg1, byte[] arg2, int arg3) {
        if (arg1 <= 0) {
            this.field844 = !this.field844;
        }
        boolean var5 = this.method293(true, arg2, arg3, true, arg0);
        if (!var5) {
            var5 = this.method293(false, arg2, arg3, true, arg0);
        }
        return var5;
    }

    @OriginalMember(owner = "vb", name = "a", descriptor = "(Z[BIZI)Z")
    private synchronized boolean method293(boolean arg0, byte[] arg1, int arg2, boolean arg3, int arg4) {
        if (!arg3) {
            throw new NullPointerException();
        }
        try {
            int var8;
            if (arg0) {
                this.method294(arg4 * 6, this.field847, 0);
                int var7;
                for (int var6 = 0; var6 < 6; var6 += var7) {
                    var7 = this.field847.read(field845, var6, 6 - var6);
                    if (var7 == -1) {
                        return false;
                    }
                }
                var8 = (field845[5] & 0xFF) + ((field845[3] & 0xFF) << 16) + ((field845[4] & 0xFF) << 8);
                if (var8 <= 0 || (long) var8 > this.field846.length() / 520L) {
                    return false;
                }
            } else {
                var8 = (int) ((this.field846.length() + 519L) / 520L);
                if (var8 == 0) {
                    var8 = 1;
                }
            }
            field845[0] = (byte) (arg2 >> 16);
            field845[1] = (byte) (arg2 >> 8);
            field845[2] = (byte) arg2;
            field845[3] = (byte) (var8 >> 16);
            field845[4] = (byte) (var8 >> 8);
            field845[5] = (byte) var8;
            this.method294(arg4 * 6, this.field847, 0);
            this.field847.write(field845, 0, 6);
            int var9 = 0;
            int var10 = 0;
            while (var9 < arg2) {
                int var11 = 0;
                if (arg0) {
                    this.method294(var8 * 520, this.field846, 0);
                    int var12;
                    int var13;
                    for (var12 = 0; var12 < 8; var12 += var13) {
                        var13 = this.field846.read(field845, var12, 8 - var12);
                        if (var13 == -1) {
                            break;
                        }
                    }
                    if (var12 == 8) {
                        label110: {
                            int var14 = ((field845[0] & 0xFF) << 8) + (field845[1] & 0xFF);
                            int var15 = ((field845[2] & 0xFF) << 8) + (field845[3] & 0xFF);
                            var11 = (field845[6] & 0xFF) + ((field845[4] & 0xFF) << 16) + ((field845[5] & 0xFF) << 8);
                            int var16 = field845[7] & 0xFF;
                            if (arg4 == var14 && var10 == var15 && this.field848 == var16) {
                                if (var11 >= 0 && (long) var11 <= this.field846.length() / 520L) {
                                    break label110;
                                }
                                return false;
                            }
                            return false;
                        }
                    }
                }
                if (var11 == 0) {
                    arg0 = false;
                    var11 = (int) ((this.field846.length() + 519L) / 520L);
                    if (var11 == 0) {
                        var11++;
                    }
                    if (var8 == var11) {
                        var11++;
                    }
                }
                if (arg2 - var9 <= 512) {
                    var11 = 0;
                }
                field845[0] = (byte) (arg4 >> 8);
                field845[1] = (byte) arg4;
                field845[2] = (byte) (var10 >> 8);
                field845[3] = (byte) var10;
                field845[4] = (byte) (var11 >> 16);
                field845[5] = (byte) (var11 >> 8);
                field845[6] = (byte) var11;
                field845[7] = (byte) this.field848;
                this.method294(var8 * 520, this.field846, 0);
                this.field846.write(field845, 0, 8);
                int var17 = arg2 - var9;
                if (var17 > 512) {
                    var17 = 512;
                }
                this.field846.write(arg1, var9, var17);
                var9 += var17;
                var8 = var11;
                var10++;
            }
            return true;
        } catch (IOException var18) {
            return false;
        }
    }

    @OriginalMember(owner = "vb", name = "a", descriptor = "(ILjava/io/RandomAccessFile;I)V")
    public synchronized void method294(int arg0, RandomAccessFile arg1, int arg2) throws IOException {
        if (arg2 != 0) {
            return;
        }
        if (arg0 < 0 || arg0 > 62914560) {
            System.out.println("Badseek - pos:" + arg0 + " len:" + arg1.length());
            arg0 = 62914560;
            try {
                Thread.sleep(1000L);
            } catch (Exception var4) {
            }
        }
        arg1.seek((long) arg0);
    }
}
