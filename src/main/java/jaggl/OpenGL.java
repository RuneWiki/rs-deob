...
public class OpenGL {
    ...
    private static Hashtable field6431 = new Hashtable();
    ...
    private Thread field6429;

    @OriginalMember(owner = "client!jaggl/OpenGL", name = "b", descriptor = "Ljava/util/Hashtable;")
    private Hashtable field6430;

    @OriginalMember(owner = "client!jaggl/OpenGL", name = "getCurrent", descriptor = "()Ljaggl/OpenGL;", line = 219)
    public static OpenGL getCurrent() {
        return (OpenGL) field6431.get(Thread.currentThread());
    }

    @OriginalMember(owner = "client!jaggl/OpenGL", name = "isExtensionAvailable", descriptor = "(Ljava/lang/String;)Z", line = 922)
    public boolean isExtensionAvailable(String arg0) {
        if (this.field6430 == null) {
            this.field6430 = new Hashtable();
            ...
            while (true) {
                ...
                if (var4 == -1) {
                    ...
                    if (var6.length() != 0) {
                        this.field6430.put(var6, var6);
                    }
                    break;
                }
                String var5 = var2.substring(var3, var4).trim();
                if (var5.length() != 0) {
                    this.field6430.put(var5, var5);
                }
                var3 = var4 + 1;
            }
        }
        return this.field6430.containsKey(arg0);
    }

    @OriginalMember(owner = "client!jaggl/OpenGL", name = "attach", descriptor = "()Z", line = 1058)
    public synchronized boolean attach() {
        Thread var1 = Thread.currentThread();
        if (!this.attachPeer()) {
            return false;
        }
        OpenGL var2 = (OpenGL) field6431.put(var1, this);
        if (var2 != null) {
            var2.field6429 = null;
        }
        this.field6429 = var1;
    ...
    @OriginalMember(owner = "client!jaggl/OpenGL", name = "detach", descriptor = "()Z", line = 2451)
    public synchronized boolean detach() {
        if (this.field6429 == Thread.currentThread()) {
            this.detachPeer();
            field6431.remove(this.field6429);
            this.field6429 = null;
            return true;
        } else {
            return false;
