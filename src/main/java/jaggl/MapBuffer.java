package jaggl;

import jaclib.memory.NativeBuffer;
import org.openrs2.deob.annotation.OriginalClass;
import org.openrs2.deob.annotation.OriginalMember;

@OriginalClass("client!jaggl/MapBuffer")
public class MapBuffer extends NativeBuffer {

    @OriginalMember(owner = "client!jaggl/MapBuffer", name = "c", descriptor = "I")
    private int field237;

    @OriginalMember(owner = "client!jaggl/MapBuffer", name = "a", descriptor = "([BIII)V", line = 4)
    public final void method120(byte[] arg0, int arg1, int arg2, int arg3) {
        if (this.field237 == 0) {
            throw new RuntimeException();
        }
        super.method120(arg0, arg1, arg2, arg3);
    }

    @OriginalMember(owner = "client!jaggl/MapBuffer", name = "a", descriptor = "(III)Z", line = 14)
    public final boolean method121(int arg0, int arg1, int arg2) {
        if (this.field237 != 0) {
            return false;
        }
        long var4 = OpenGL.glMapBufferARB(arg0, arg2);
        if (var4 == 0L) {
            return false;
        } else {
            this.method2336(var4, arg1);
            this.field237 = arg0;
            return true;
        }
    }

    @OriginalMember(owner = "client!jaggl/MapBuffer", name = "a", descriptor = "()Z", line = 34)
    public final boolean method122() {
        boolean var1 = true;
        if (this.field237 != 0) {
            var1 = OpenGL.glUnmapBufferARB(this.field237);
            this.method2336(0L, 0);
            this.field237 = 0;
        }
        return var1;
    }

    @OriginalMember(owner = "client!jaggl/MapBuffer", name = "b", descriptor = "()Z", line = 51)
    public final boolean method123() {
        return this.field237 != 0;
    }
}
