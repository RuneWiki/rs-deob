import org.openrs2.deob.annotation.OriginalClass;
import org.openrs2.deob.annotation.OriginalMember;

@OriginalClass("client!nc")
public class SeqType {

    @OriginalMember(owner = "client!nc", name = "b", descriptor = "I")
    private int field1142 = 38;

    @OriginalMember(owner = "client!nc", name = "i", descriptor = "I")
    public int field1149 = -1;

    @OriginalMember(owner = "client!nc", name = "k", descriptor = "Z")
    public boolean field1151 = false;

    @OriginalMember(owner = "client!nc", name = "l", descriptor = "I")
    public int field1152 = 5;

    @OriginalMember(owner = "client!nc", name = "m", descriptor = "I")
    public int field1153 = -1;

    @OriginalMember(owner = "client!nc", name = "n", descriptor = "I")
    public int field1154 = -1;

    @OriginalMember(owner = "client!nc", name = "o", descriptor = "I")
    public int field1155 = 99;

    @OriginalMember(owner = "client!nc", name = "p", descriptor = "I")
    public int field1156 = -1;

    @OriginalMember(owner = "client!nc", name = "q", descriptor = "I")
    public int field1157 = -1;

    @OriginalMember(owner = "client!nc", name = "c", descriptor = "I")
    public static int field1143;

    @OriginalMember(owner = "client!nc", name = "e", descriptor = "I")
    public int field1145;

    @OriginalMember(owner = "client!nc", name = "r", descriptor = "I")
    public int field1158;

    @OriginalMember(owner = "client!nc", name = "s", descriptor = "I")
    public int field1159;

    @OriginalMember(owner = "client!nc", name = "a", descriptor = "Z")
    private static boolean field1141;

    @OriginalMember(owner = "client!nc", name = "t", descriptor = "Z")
    public static boolean field1160;

    @OriginalMember(owner = "client!nc", name = "f", descriptor = "[I")
    public int[] field1146;

    @OriginalMember(owner = "client!nc", name = "g", descriptor = "[I")
    public int[] field1147;

    @OriginalMember(owner = "client!nc", name = "h", descriptor = "[I")
    private int[] field1148;

    @OriginalMember(owner = "client!nc", name = "j", descriptor = "[I")
    public int[] field1150;

    @OriginalMember(owner = "client!nc", name = "d", descriptor = "[Lnc;")
    public static SeqType[] field1144;

    @OriginalMember(owner = "client!nc", name = "a", descriptor = "(Lxb;I)V")
    public static void method381(Jagfile arg0, int arg1) {
        Packet var2 = new Packet(arg0.method299("seq.dat", null), field1141);
        field1143 = var2.method231();
        if (field1144 == null) {
            field1144 = new SeqType[field1143];
        }
        for (int var3 = 0; var3 < field1143; var3++) {
            if (field1144[var3] == null) {
                field1144[var3] = new SeqType();
            }
            field1144[var3].method383(false, var2);
        }
        if (arg1 != 5) {
            for (int var4 = 1; var4 > 0; var4++) {
            }
        }
    }

    @OriginalMember(owner = "client!nc", name = "a", descriptor = "(II)I")
    public int method382(int arg0, int arg1) {
        while (arg1 >= 0) {
            this.field1142 = 231;
        }
        int var3 = this.field1148[arg0];
        if (var3 == 0) {
            AnimFrame var4 = AnimFrame.method47((byte) 4, this.field1146[arg0]);
            if (var4 != null) {
                var3 = this.field1148[arg0] = var4.field155;
            }
        }
        if (var3 == 0) {
            var3 = 1;
        }
        return var3;
    }

    @OriginalMember(owner = "client!nc", name = "a", descriptor = "(ZLlb;)V")
    public void method383(boolean arg0, Packet arg1) {
        if (arg0) {
            throw new NullPointerException();
        }
        while (true) {
            while (true) {
                int var3 = arg1.method229();
                if (var3 == 0) {
                    if (this.field1145 == 0) {
                        this.field1145 = 1;
                        this.field1146 = new int[1];
                        this.field1146[0] = -1;
                        this.field1147 = new int[1];
                        this.field1147[0] = -1;
                        this.field1148 = new int[1];
                        this.field1148[0] = -1;
                    }
                    if (this.field1156 == -1) {
                        if (this.field1150 == null) {
                            this.field1156 = 0;
                        } else {
                            this.field1156 = 2;
                        }
                    }
                    if (this.field1157 == -1) {
                        if (this.field1150 != null) {
                            this.field1157 = 2;
                            return;
                        }
                        this.field1157 = 0;
                        return;
                    }
                    return;
                }
                if (var3 == 1) {
                    this.field1145 = arg1.method229();
                    this.field1146 = new int[this.field1145];
                    this.field1147 = new int[this.field1145];
                    this.field1148 = new int[this.field1145];
                    for (int var4 = 0; var4 < this.field1145; var4++) {
                        this.field1146[var4] = arg1.method231();
                        this.field1147[var4] = arg1.method231();
                        if (this.field1147[var4] == 65535) {
                            this.field1147[var4] = -1;
                        }
                        this.field1148[var4] = arg1.method231();
                    }
                } else if (var3 == 2) {
                    this.field1149 = arg1.method231();
                } else if (var3 == 3) {
                    int var5 = arg1.method229();
                    this.field1150 = new int[var5 + 1];
                    for (int var6 = 0; var6 < var5; var6++) {
                        this.field1150[var6] = arg1.method229();
                    }
                    this.field1150[var5] = 9999999;
                } else if (var3 == 4) {
                    this.field1151 = true;
                } else if (var3 == 5) {
                    this.field1152 = arg1.method229();
                } else if (var3 == 6) {
                    this.field1153 = arg1.method231();
                } else if (var3 == 7) {
                    this.field1154 = arg1.method231();
                } else if (var3 == 8) {
                    this.field1155 = arg1.method229();
                } else if (var3 == 9) {
                    this.field1156 = arg1.method229();
                } else if (var3 == 10) {
                    this.field1157 = arg1.method229();
                } else if (var3 == 11) {
                    this.field1158 = arg1.method229();
                } else if (var3 == 12) {
                    this.field1159 = arg1.method234();
                } else {
                    System.out.println("Error unrecognised seq config code: " + var3);
                }
            }
        }
    }
}
