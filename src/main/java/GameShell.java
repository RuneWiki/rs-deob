import java.applet.Applet;
import java.awt.Color;
import java.awt.Font;
import java.awt.FontMetrics;
import java.awt.Graphics;
import java.awt.event.FocusEvent;
import java.awt.event.FocusListener;
import java.awt.event.KeyEvent;
import java.awt.event.KeyListener;
import java.awt.event.MouseEvent;
import java.awt.event.MouseListener;
import java.awt.event.MouseMotionListener;
import java.awt.event.WindowEvent;
import java.awt.event.WindowListener;
import org.openrs2.deob.annotation.OriginalClass;
import org.openrs2.deob.annotation.OriginalMember;

@OriginalClass("client!a")
public class GameShell extends Applet implements Runnable, MouseListener, MouseMotionListener, KeyListener, FocusListener, WindowListener {

    @OriginalMember(owner = "client!a", name = "a", descriptor = "Z")
    private boolean field1 = false;

    @OriginalMember(owner = "client!a", name = "b", descriptor = "Z")
    private boolean field2 = false;

    @OriginalMember(owner = "client!a", name = "d", descriptor = "Z")
    private boolean field4 = false;

    @OriginalMember(owner = "client!a", name = "e", descriptor = "B")
    private byte field5 = 3;

    @OriginalMember(owner = "client!a", name = "f", descriptor = "I")
    private int field6 = 27808;

    @OriginalMember(owner = "client!a", name = "h", descriptor = "I")
    private int field8 = 20;

    @OriginalMember(owner = "client!a", name = "i", descriptor = "I")
    public int field9 = 1;

    @OriginalMember(owner = "client!a", name = "j", descriptor = "[J")
    private long[] field10 = new long[10];

    @OriginalMember(owner = "client!a", name = "p", descriptor = "[Lhb;")
    public Pix24[] field16 = new Pix24[6];

    @OriginalMember(owner = "client!a", name = "r", descriptor = "Z")
    public boolean field18 = true;

    @OriginalMember(owner = "client!a", name = "z", descriptor = "[I")
    public int[] field26 = new int[128];

    @OriginalMember(owner = "client!a", name = "A", descriptor = "[I")
    private int[] field27 = new int[128];

    @OriginalMember(owner = "client!a", name = "k", descriptor = "I")
    public int field11;

    @OriginalMember(owner = "client!a", name = "l", descriptor = "I")
    public int field12;

    @OriginalMember(owner = "client!a", name = "m", descriptor = "I")
    public int field13;

    @OriginalMember(owner = "client!a", name = "s", descriptor = "I")
    public int field19;

    @OriginalMember(owner = "client!a", name = "t", descriptor = "I")
    public int field20;

    @OriginalMember(owner = "client!a", name = "u", descriptor = "I")
    public int field21;

    @OriginalMember(owner = "client!a", name = "v", descriptor = "I")
    public int field22;

    @OriginalMember(owner = "client!a", name = "w", descriptor = "I")
    public int field23;

    @OriginalMember(owner = "client!a", name = "x", descriptor = "I")
    public int field24;

    @OriginalMember(owner = "client!a", name = "y", descriptor = "I")
    public int field25;

    @OriginalMember(owner = "client!a", name = "B", descriptor = "I")
    private int field28;

    @OriginalMember(owner = "client!a", name = "C", descriptor = "I")
    private int field29;

    @OriginalMember(owner = "client!a", name = "c", descriptor = "I")
    private int field3;

    @OriginalMember(owner = "client!a", name = "D", descriptor = "I")
    public static int field30;

    @OriginalMember(owner = "client!a", name = "g", descriptor = "I")
    private int field7;

    @OriginalMember(owner = "client!a", name = "o", descriptor = "Lqb;")
    public PixMap field15;

    @OriginalMember(owner = "client!a", name = "q", descriptor = "Lb;")
    public ViewBox field17;

    @OriginalMember(owner = "client!a", name = "n", descriptor = "Ljava/awt/Graphics;")
    public Graphics field14;

    @OriginalMember(owner = "client!a", name = "a", descriptor = "(III)V")
    public final void method1(int arg0, int arg1, int arg2) {
        if (arg2 != 0) {
            this.field3 = 411;
        }
        this.field12 = arg1;
        this.field13 = arg0;
        this.field17 = new ViewBox(this.field13, 35731, this, this.field12);
        this.field14 = this.method11(this.field5).getGraphics();
        this.field15 = new PixMap(this.method11(this.field5), this.field12, 299, this.field13);
        this.method12(this, 1);
    }

    @OriginalMember(owner = "client!a", name = "a", descriptor = "(IZI)V")
    public final void method2(int arg0, boolean arg1, int arg2) {
        this.field12 = arg2;
        this.field13 = arg0;
        this.field14 = this.method11(this.field5).getGraphics();
        this.field15 = new PixMap(this.method11(this.field5), this.field12, 299, this.field13);
        if (arg1) {
            this.field3 = -370;
        }
        this.method12(this, 1);
    }

    @OriginalMember(owner = "client!a", name = "run", descriptor = "()V")
    public void run() {
        this.method11(this.field5).addMouseListener(this);
        this.method11(this.field5).addMouseMotionListener(this);
        this.method11(this.field5).addKeyListener(this);
        this.method11(this.field5).addFocusListener(this);
        if (this.field17 != null) {
            this.field17.addWindowListener(this);
        }
        this.method13(true, "Loading...", 0);
        this.method6();
        int var1 = 0;
        int var2 = 256;
        int var3 = 1;
        int var4 = 0;
        for (int var5 = 0; var5 < 10; var5++) {
            this.field10[var5] = System.currentTimeMillis();
        }
        long var6 = System.currentTimeMillis();
        while (this.field7 >= 0) {
            if (this.field7 > 0) {
                this.field7--;
                if (this.field7 == 0) {
                    this.method3(-652);
                    return;
                }
            }
            int var8 = var2;
            int var9 = var3;
            var2 = 300;
            var3 = 1;
            long var10 = System.currentTimeMillis();
            if (this.field10[var1] == 0L) {
                var2 = var8;
                var3 = var9;
            } else if (var10 > this.field10[var1]) {
                var2 = (int) ((long) (this.field8 * 2560) / (var10 - this.field10[var1]));
            }
            if (var2 < 25) {
                var2 = 25;
            }
            if (var2 > 256) {
                var2 = 256;
                var3 = (int) ((long) this.field8 - (var10 - this.field10[var1]) / 10L);
            }
            this.field10[var1] = var10;
            var1 = (var1 + 1) % 10;
            if (var3 > 1) {
                for (int var12 = 0; var12 < 10; var12++) {
                    if (this.field10[var12] != 0L) {
                        this.field10[var12] += var3;
                    }
                }
            }
            if (var3 < this.field9) {
                var3 = this.field9;
            }
            try {
                Thread.sleep((long) var3);
            } catch (InterruptedException var13) {
            }
            while (var4 < 256) {
                this.method7(437);
                this.field23 = 0;
                this.field28 = this.field29;
                var4 += var2;
            }
            var4 &= 0xFF;
            if (this.field8 > 0) {
                this.field11 = var2 * 1000 / (this.field8 * 256);
            }
            this.method9(false);
        }
        if (this.field7 == -1) {
            this.method3(-652);
        }
    }

    @OriginalMember(owner = "client!a", name = "a", descriptor = "(I)V")
    public final void method3(int arg0) {
        while (arg0 >= 0) {
            this.field4 = !this.field4;
        }
        this.field7 = -2;
        this.method8((byte) -28);
        try {
            Thread.sleep(1000L);
        } catch (Exception var3) {
        }
        try {
            System.exit(0);
        } catch (Throwable var2) {
        }
    }

    @OriginalMember(owner = "client!a", name = "a", descriptor = "(II)V")
    public final void method4(int arg0, int arg1) {
        this.field8 = 1000 / arg1;
        if (arg0 <= 0) {
            this.field1 = !this.field1;
        }
    }

    @OriginalMember(owner = "client!a", name = "start", descriptor = "()V")
    public final void start() {
        if (this.field7 >= 0) {
            this.field7 = 0;
        }
    }

    @OriginalMember(owner = "client!a", name = "stop", descriptor = "()V")
    public final void stop() {
        if (this.field7 >= 0) {
            this.field7 = 4000 / this.field8;
        }
    }

    @OriginalMember(owner = "client!a", name = "destroy", descriptor = "()V")
    public final void destroy() {
        this.field7 = -1;
        try {
            Thread.sleep(5000L);
        } catch (Exception var1) {
        }
        if (this.field7 == -1) {
            this.method3(-652);
        }
    }

    @OriginalMember(owner = "client!a", name = "update", descriptor = "(Ljava/awt/Graphics;)V")
    public final void update(Graphics arg0) {
        if (this.field14 == null) {
            this.field14 = arg0;
        }
        this.field18 = true;
        this.method10(3);
    }

    @OriginalMember(owner = "client!a", name = "paint", descriptor = "(Ljava/awt/Graphics;)V")
    public final void paint(Graphics arg0) {
        if (this.field14 == null) {
            this.field14 = arg0;
        }
        this.field18 = true;
        this.method10(3);
    }

    @OriginalMember(owner = "client!a", name = "mousePressed", descriptor = "(Ljava/awt/event/MouseEvent;)V")
    public final void mousePressed(MouseEvent arg0) {
        int var2 = arg0.getX();
        int var3 = arg0.getY();
        if (this.field17 != null) {
            var2 -= 4;
            var3 -= 22;
        }
        this.field19 = 0;
        this.field24 = var2;
        this.field25 = var3;
        if (arg0.isMetaDown()) {
            this.field23 = 2;
            this.field20 = 2;
        } else {
            this.field23 = 1;
            this.field20 = 1;
        }
        if (InputTracking.field76) {
            InputTracking.method39(var2, arg0.isMetaDown() ? 1 : 0, var3, (byte) 4);
        }
    }

    @OriginalMember(owner = "client!a", name = "mouseReleased", descriptor = "(Ljava/awt/event/MouseEvent;)V")
    public final void mouseReleased(MouseEvent arg0) {
        this.field19 = 0;
        this.field20 = 0;
        if (InputTracking.field76) {
            InputTracking.method40(arg0.isMetaDown() ? 1 : 0, 0);
        }
    }

    @OriginalMember(owner = "client!a", name = "mouseClicked", descriptor = "(Ljava/awt/event/MouseEvent;)V")
    public final void mouseClicked(MouseEvent arg0) {
    }

    @OriginalMember(owner = "client!a", name = "mouseEntered", descriptor = "(Ljava/awt/event/MouseEvent;)V")
    public final void mouseEntered(MouseEvent arg0) {
        if (InputTracking.field76) {
            InputTracking.method46(-657);
        }
    }

    @OriginalMember(owner = "client!a", name = "mouseExited", descriptor = "(Ljava/awt/event/MouseEvent;)V")
    public final void mouseExited(MouseEvent arg0) {
        if (InputTracking.field76) {
            InputTracking.method47(false);
        }
    }

    @OriginalMember(owner = "client!a", name = "mouseDragged", descriptor = "(Ljava/awt/event/MouseEvent;)V")
    public final void mouseDragged(MouseEvent arg0) {
        int var2 = arg0.getX();
        int var3 = arg0.getY();
        if (this.field17 != null) {
            var2 -= 4;
            var3 -= 22;
        }
        this.field19 = 0;
        this.field21 = var2;
        this.field22 = var3;
        if (InputTracking.field76) {
            InputTracking.method41(var3, true, var2);
        }
    }

    @OriginalMember(owner = "client!a", name = "mouseMoved", descriptor = "(Ljava/awt/event/MouseEvent;)V")
    public final void mouseMoved(MouseEvent arg0) {
        int var2 = arg0.getX();
        int var3 = arg0.getY();
        if (this.field17 != null) {
            var2 -= 4;
            var3 -= 22;
        }
        this.field19 = 0;
        this.field21 = var2;
        this.field22 = var3;
        if (InputTracking.field76) {
            InputTracking.method41(var3, true, var2);
        }
    }

    @OriginalMember(owner = "client!a", name = "keyPressed", descriptor = "(Ljava/awt/event/KeyEvent;)V")
    public final void keyPressed(KeyEvent arg0) {
        this.field19 = 0;
        int var2 = arg0.getKeyCode();
        int var3 = arg0.getKeyChar();
        if (var3 < 30) {
            var3 = 0;
        }
        if (var2 == 37) {
            var3 = 1;
        }
        if (var2 == 39) {
            var3 = 2;
        }
        if (var2 == 38) {
            var3 = 3;
        }
        if (var2 == 40) {
            var3 = 4;
        }
        if (var2 == 17) {
            var3 = 5;
        }
        if (var2 == 8) {
            var3 = 8;
        }
        if (var2 == 127) {
            var3 = 8;
        }
        if (var2 == 9) {
            var3 = 9;
        }
        if (var2 == 10) {
            var3 = 10;
        }
        if (var2 >= 112 && var2 <= 123) {
            var3 = var2 + 1008 - 112;
        }
        if (var2 == 36) {
            var3 = 1000;
        }
        if (var2 == 35) {
            var3 = 1001;
        }
        if (var2 == 33) {
            var3 = 1002;
        }
        if (var2 == 34) {
            var3 = 1003;
        }
        if (var3 > 0 && var3 < 128) {
            this.field26[var3] = 1;
        }
        if (var3 > 4) {
            this.field27[this.field29] = var3;
            this.field29 = this.field29 + 1 & 0x7F;
        }
        if (InputTracking.field76) {
            InputTracking.method42(var3, true);
        }
    }

    @OriginalMember(owner = "client!a", name = "keyReleased", descriptor = "(Ljava/awt/event/KeyEvent;)V")
    public final void keyReleased(KeyEvent arg0) {
        this.field19 = 0;
        int var2 = arg0.getKeyCode();
        char var3 = arg0.getKeyChar();
        if (var3 < '\u001E') {
            var3 = '\u0000';
        }
        if (var2 == 37) {
            var3 = '\u0001';
        }
        if (var2 == 39) {
            var3 = '\u0002';
        }
        if (var2 == 38) {
            var3 = '\u0003';
        }
        if (var2 == 40) {
            var3 = '\u0004';
        }
        if (var2 == 17) {
            var3 = '\u0005';
        }
        if (var2 == 8) {
            var3 = '\b';
        }
        if (var2 == 127) {
            var3 = '\b';
        }
        if (var2 == 9) {
            var3 = '\t';
        }
        if (var2 == 10) {
            var3 = '\n';
        }
        if (var3 > '\u0000' && var3 < '\u0080') {
            this.field26[var3] = 0;
        }
        if (InputTracking.field76) {
            InputTracking.method43(var3, 1);
        }
    }

    @OriginalMember(owner = "client!a", name = "keyTyped", descriptor = "(Ljava/awt/event/KeyEvent;)V")
    public final void keyTyped(KeyEvent arg0) {
    }

    @OriginalMember(owner = "client!a", name = "focusGained", descriptor = "(Ljava/awt/event/FocusEvent;)V")
    public final void focusGained(FocusEvent arg0) {
        this.field18 = true;
        this.method10(3);
        if (InputTracking.field76) {
            InputTracking.method44(-936);
        }
    }

    @OriginalMember(owner = "client!a", name = "focusLost", descriptor = "(Ljava/awt/event/FocusEvent;)V")
    public final void focusLost(FocusEvent arg0) {
        if (InputTracking.field76) {
            InputTracking.method45(0);
        }
    }

    @OriginalMember(owner = "client!a", name = "a", descriptor = "(Z)I")
    public final int method5(boolean arg0) {
        int var2 = -1;
        if (arg0) {
            return 2;
        }
        if (this.field29 != this.field28) {
            var2 = this.field27[this.field28];
            this.field28 = this.field28 + 1 & 0x7F;
        }
        return var2;
    }

    @OriginalMember(owner = "client!a", name = "windowActivated", descriptor = "(Ljava/awt/event/WindowEvent;)V")
    public final void windowActivated(WindowEvent arg0) {
    }

    @OriginalMember(owner = "client!a", name = "windowClosed", descriptor = "(Ljava/awt/event/WindowEvent;)V")
    public final void windowClosed(WindowEvent arg0) {
    }

    @OriginalMember(owner = "client!a", name = "windowClosing", descriptor = "(Ljava/awt/event/WindowEvent;)V")
    public final void windowClosing(WindowEvent arg0) {
        this.destroy();
    }

    @OriginalMember(owner = "client!a", name = "windowDeactivated", descriptor = "(Ljava/awt/event/WindowEvent;)V")
    public final void windowDeactivated(WindowEvent arg0) {
    }

    @OriginalMember(owner = "client!a", name = "windowDeiconified", descriptor = "(Ljava/awt/event/WindowEvent;)V")
    public final void windowDeiconified(WindowEvent arg0) {
    }

    @OriginalMember(owner = "client!a", name = "windowIconified", descriptor = "(Ljava/awt/event/WindowEvent;)V")
    public final void windowIconified(WindowEvent arg0) {
    }

    @OriginalMember(owner = "client!a", name = "windowOpened", descriptor = "(Ljava/awt/event/WindowEvent;)V")
    public final void windowOpened(WindowEvent arg0) {
    }

    @OriginalMember(owner = "client!a", name = "a", descriptor = "()V")
    public void method6() {
    }

    @OriginalMember(owner = "client!a", name = "b", descriptor = "(I)V")
    public void method7(int arg0) {
        if (arg0 <= 0) {
            this.field2 = !this.field2;
        }
    }

    @OriginalMember(owner = "client!a", name = "a", descriptor = "(B)V")
    public void method8(byte arg0) {
        if (arg0 != -28) {
            this.field6 = -407;
        }
    }

    @OriginalMember(owner = "client!a", name = "b", descriptor = "(Z)V")
    public void method9(boolean arg0) {
        if (arg0) {
            this.field4 = !this.field4;
        }
    }

    @OriginalMember(owner = "client!a", name = "c", descriptor = "(I)V")
    public void method10(int arg0) {
        if (arg0 < 3 || arg0 > 3) {
            this.field2 = !this.field2;
        }
    }

    @OriginalMember(owner = "client!a", name = "b", descriptor = "(B)Ljava/awt/Component;")
    public java.awt.Component method11(byte arg0) {
        if (arg0 != 3) {
            throw new NullPointerException();
        }
        return this.field17 == null ? this : this.field17;
    }

    @OriginalMember(owner = "client!a", name = "a", descriptor = "(Ljava/lang/Runnable;I)V")
    public void method12(Runnable arg0, int arg1) {
        Thread var3 = new Thread(arg0);
        var3.start();
        var3.setPriority(arg1);
    }

    @OriginalMember(owner = "client!a", name = "a", descriptor = "(ZLjava/lang/String;I)V")
    public void method13(boolean arg0, String arg1, int arg2) {
        while (this.field14 == null) {
            this.field14 = this.method11(this.field5).getGraphics();
            try {
                this.method11(this.field5).repaint();
            } catch (Exception var10) {
            }
            try {
                Thread.sleep(1000L);
            } catch (Exception var9) {
            }
        }
        Font var4 = new Font("Helvetica", 1, 13);
        FontMetrics var5 = this.method11(this.field5).getFontMetrics(var4);
        Font var6 = new Font("Helvetica", 0, 13);
        this.method11(this.field5).getFontMetrics(var6);
        if (this.field18) {
            this.field14.setColor(Color.black);
            this.field14.fillRect(0, 0, this.field12, this.field13);
            this.field18 = false;
        }
        Color var7 = new Color(140, 17, 17);
        int var8 = this.field13 / 2 - 18;
        this.field14.setColor(var7);
        this.field14.drawRect(this.field12 / 2 - 152, var8, 304, 34);
        this.field14.fillRect(this.field12 / 2 - 150, var8 + 2, arg2 * 3, 30);
        this.field14.setColor(Color.black);
        this.field14.fillRect(arg2 * 3 + this.field12 / 2 - 150, var8 + 2, 300 - arg2 * 3, 30);
        this.field14.setFont(var4);
        this.field14.setColor(Color.white);
        if (arg0) {
            this.field14.drawString(arg1, (this.field12 - var5.stringWidth(arg1)) / 2, var8 + 22);
        }
    }
}
