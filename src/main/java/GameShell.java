import java.applet.Applet;
import java.awt.Color;
import java.awt.Font;
import java.awt.FontMetrics;
import java.awt.Graphics;
import java.awt.event.FocusEvent;
import java.awt.event.FocusListener;
import java.awt.event.KeyEvent;
import java.awt.event.KeyListener;
import java.awt.event.MouseEvent;
import java.awt.event.MouseListener;
import java.awt.event.MouseMotionListener;
import java.awt.event.WindowEvent;
import java.awt.event.WindowListener;
import org.openrs2.deob.annotation.OriginalClass;
import org.openrs2.deob.annotation.OriginalMember;

@OriginalClass("client!a")
public class GameShell extends Applet implements Runnable, MouseListener, MouseMotionListener, KeyListener, FocusListener, WindowListener {

    @OriginalMember(owner = "client!a", name = "a", descriptor = "I")
    private int field1 = 3073;

    @OriginalMember(owner = "client!a", name = "b", descriptor = "Z")
    private boolean field2 = false;

    @OriginalMember(owner = "client!a", name = "c", descriptor = "Z")
    private boolean field3 = true;

    @OriginalMember(owner = "client!a", name = "d", descriptor = "B")
    private byte field4 = -63;

    @OriginalMember(owner = "client!a", name = "e", descriptor = "Z")
    private boolean field5 = false;

    @OriginalMember(owner = "client!a", name = "g", descriptor = "I")
    private int field7 = 20;

    @OriginalMember(owner = "client!a", name = "h", descriptor = "I")
    public int field8 = 1;

    @OriginalMember(owner = "client!a", name = "i", descriptor = "[J")
    private long[] field9 = new long[10];

    @OriginalMember(owner = "client!a", name = "o", descriptor = "[Lhb;")
    public Pix24[] field15 = new Pix24[6];

    @OriginalMember(owner = "client!a", name = "q", descriptor = "Z")
    public boolean field17 = true;

    @OriginalMember(owner = "client!a", name = "y", descriptor = "[I")
    public int[] field25 = new int[128];

    @OriginalMember(owner = "client!a", name = "z", descriptor = "[I")
    private int[] field26 = new int[128];

    @OriginalMember(owner = "client!a", name = "j", descriptor = "I")
    public int field10;

    @OriginalMember(owner = "client!a", name = "k", descriptor = "I")
    public int field11;

    @OriginalMember(owner = "client!a", name = "l", descriptor = "I")
    public int field12;

    @OriginalMember(owner = "client!a", name = "r", descriptor = "I")
    public int field18;

    @OriginalMember(owner = "client!a", name = "s", descriptor = "I")
    public int field19;

    @OriginalMember(owner = "client!a", name = "t", descriptor = "I")
    public int field20;

    @OriginalMember(owner = "client!a", name = "u", descriptor = "I")
    public int field21;

    @OriginalMember(owner = "client!a", name = "v", descriptor = "I")
    public int field22;

    @OriginalMember(owner = "client!a", name = "w", descriptor = "I")
    public int field23;

    @OriginalMember(owner = "client!a", name = "x", descriptor = "I")
    public int field24;

    @OriginalMember(owner = "client!a", name = "A", descriptor = "I")
    private int field27;

    @OriginalMember(owner = "client!a", name = "B", descriptor = "I")
    private int field28;

    @OriginalMember(owner = "client!a", name = "C", descriptor = "I")
    public static int field29;

    @OriginalMember(owner = "client!a", name = "f", descriptor = "I")
    private int field6;

    @OriginalMember(owner = "client!a", name = "n", descriptor = "Lqb;")
    public PixMap field14;

    @OriginalMember(owner = "client!a", name = "p", descriptor = "Lb;")
    public ViewBox field16;

    @OriginalMember(owner = "client!a", name = "m", descriptor = "Ljava/awt/Graphics;")
    public Graphics field13;

    @OriginalMember(owner = "client!a", name = "a", descriptor = "(III)V")
    public final void method1(int arg0, int arg1, int arg2) {
        this.field11 = arg1;
        this.field12 = arg2;
        this.field16 = new ViewBox(this.field11, this.field12, this, true);
        this.field13 = this.method11(-36374).getGraphics();
        if (arg0 == 0) {
            this.field14 = new PixMap(this.field12, (byte) -58, this.method11(-36374), this.field11);
            this.method12(this, 1);
        }
    }

    @OriginalMember(owner = "client!a", name = "a", descriptor = "(ZII)V")
    public final void method2(boolean arg0, int arg1, int arg2) {
        if (!arg0) {
            return;
        }
        this.field11 = arg1;
        this.field12 = arg2;
        this.field13 = this.method11(-36374).getGraphics();
        this.field14 = new PixMap(this.field12, (byte) -58, this.method11(-36374), this.field11);
        this.method12(this, 1);
    }

    @OriginalMember(owner = "client!a", name = "run", descriptor = "()V")
    public void run() {
        System.out.println("Registering event listeners");
        this.method11(-36374).addMouseListener(this);
        this.method11(-36374).addMouseMotionListener(this);
        this.method11(-36374).addKeyListener(this);
        this.method11(-36374).addFocusListener(this);
        if (this.field16 != null) {
            this.field16.addWindowListener(this);
        }
        this.method13((byte) -75, 0, "Loading...");
        this.method6();
        int var1 = 0;
        int var2 = 256;
        int var3 = 1;
        int var4 = 0;
        for (int var5 = 0; var5 < 10; var5++) {
            this.field9[var5] = System.currentTimeMillis();
        }
        long var6 = System.currentTimeMillis();
        while (this.field6 >= 0) {
            if (this.field6 > 0) {
                this.field6--;
                if (this.field6 == 0) {
                    this.method3(-59);
                    return;
                }
            }
            int var8 = var2;
            int var9 = var3;
            var2 = 300;
            var3 = 1;
            long var10 = System.currentTimeMillis();
            if (this.field9[var1] == 0L) {
                var2 = var8;
                var3 = var9;
            } else if (var10 > this.field9[var1]) {
                var2 = (int) ((long) (this.field7 * 2560) / (var10 - this.field9[var1]));
            }
            if (var2 < 25) {
                var2 = 25;
            }
            if (var2 > 256) {
                var2 = 256;
                var3 = (int) ((long) this.field7 - (var10 - this.field9[var1]) / 10L);
            }
            this.field9[var1] = var10;
            var1 = (var1 + 1) % 10;
            if (var3 > 1) {
                for (int var12 = 0; var12 < 10; var12++) {
                    if (this.field9[var12] != 0L) {
                        this.field9[var12] += var3;
                    }
                }
            }
            if (var3 < this.field8) {
                var3 = this.field8;
            }
            try {
                Thread.sleep((long) var3);
            } catch (InterruptedException var13) {
            }
            while (var4 < 256) {
                this.method7((byte) 3);
                this.field22 = 0;
                this.field27 = this.field28;
                var4 += var2;
            }
            var4 &= 0xFF;
            if (this.field7 > 0) {
                this.field10 = var2 * 1000 / (this.field7 * 256);
            }
            this.method9(-14379);
        }
        if (this.field6 == -1) {
            this.method3(-59);
        }
    }

    @OriginalMember(owner = "client!a", name = "a", descriptor = "(I)V")
    public final void method3(int arg0) {
        this.field6 = -2;
        System.out.println("Closing program");
        this.method8(false);
        if (arg0 >= 0) {
            this.field1 = 282;
        }
        try {
            Thread.sleep(1000L);
        } catch (Exception var3) {
        }
        try {
            System.exit(0);
        } catch (Throwable var2) {
        }
    }

    @OriginalMember(owner = "client!a", name = "a", descriptor = "(BI)V")
    public final void method4(byte arg0, int arg1) {
        if (arg0 == 7) {
            boolean var3 = false;
            this.field7 = 1000 / arg1;
        }
    }

    @OriginalMember(owner = "client!a", name = "start", descriptor = "()V")
    public final void start() {
        if (this.field6 >= 0) {
            this.field6 = 0;
        }
    }

    @OriginalMember(owner = "client!a", name = "stop", descriptor = "()V")
    public final void stop() {
        if (this.field6 >= 0) {
            this.field6 = 4000 / this.field7;
        }
    }

    @OriginalMember(owner = "client!a", name = "destroy", descriptor = "()V")
    public final void destroy() {
        this.field6 = -1;
        try {
            Thread.sleep(5000L);
        } catch (Exception var1) {
        }
        if (this.field6 == -1) {
            System.out.println("5 seconds expired, forcing kill");
            this.method3(-59);
        }
    }

    @OriginalMember(owner = "client!a", name = "update", descriptor = "(Ljava/awt/Graphics;)V")
    public final void update(Graphics arg0) {
        if (this.field13 == null) {
            this.field13 = arg0;
        }
        this.field17 = true;
        this.method10(false);
    }

    @OriginalMember(owner = "client!a", name = "paint", descriptor = "(Ljava/awt/Graphics;)V")
    public final void paint(Graphics arg0) {
        if (this.field13 == null) {
            this.field13 = arg0;
        }
        this.field17 = true;
        this.method10(false);
    }

    @OriginalMember(owner = "client!a", name = "mousePressed", descriptor = "(Ljava/awt/event/MouseEvent;)V")
    public final void mousePressed(MouseEvent arg0) {
        int var2 = arg0.getX();
        int var3 = arg0.getY();
        if (this.field16 != null) {
            var2 -= 4;
            var3 -= 22;
        }
        this.field18 = 0;
        this.field23 = var2;
        this.field24 = var3;
        if (arg0.isMetaDown()) {
            this.field22 = 2;
            this.field19 = 2;
        } else {
            this.field22 = 1;
            this.field19 = 1;
        }
        if (InputTracking.field72) {
            InputTracking.method36(var3, (byte) -58, var2);
        }
    }

    @OriginalMember(owner = "client!a", name = "mouseReleased", descriptor = "(Ljava/awt/event/MouseEvent;)V")
    public final void mouseReleased(MouseEvent arg0) {
        this.field18 = 0;
        this.field19 = 0;
        if (InputTracking.field72) {
            InputTracking.method37(this.field3);
        }
    }

    @OriginalMember(owner = "client!a", name = "mouseClicked", descriptor = "(Ljava/awt/event/MouseEvent;)V")
    public final void mouseClicked(MouseEvent arg0) {
    }

    @OriginalMember(owner = "client!a", name = "mouseEntered", descriptor = "(Ljava/awt/event/MouseEvent;)V")
    public final void mouseEntered(MouseEvent arg0) {
        if (InputTracking.field72) {
            InputTracking.method43(-611);
        }
    }

    @OriginalMember(owner = "client!a", name = "mouseExited", descriptor = "(Ljava/awt/event/MouseEvent;)V")
    public final void mouseExited(MouseEvent arg0) {
        if (InputTracking.field72) {
            InputTracking.method44((byte) 66);
        }
    }

    @OriginalMember(owner = "client!a", name = "mouseDragged", descriptor = "(Ljava/awt/event/MouseEvent;)V")
    public final void mouseDragged(MouseEvent arg0) {
        int var2 = arg0.getX();
        int var3 = arg0.getY();
        if (this.field16 != null) {
            var2 -= 4;
            var3 -= 22;
        }
        this.field18 = 0;
        this.field20 = var2;
        this.field21 = var3;
        if (InputTracking.field72) {
            InputTracking.method38(true, var3, var2);
        }
    }

    @OriginalMember(owner = "client!a", name = "mouseMoved", descriptor = "(Ljava/awt/event/MouseEvent;)V")
    public final void mouseMoved(MouseEvent arg0) {
        int var2 = arg0.getX();
        int var3 = arg0.getY();
        if (this.field16 != null) {
            var2 -= 4;
            var3 -= 22;
        }
        this.field18 = 0;
        this.field20 = var2;
        this.field21 = var3;
        if (InputTracking.field72) {
            InputTracking.method38(true, var3, var2);
        }
    }

    @OriginalMember(owner = "client!a", name = "keyPressed", descriptor = "(Ljava/awt/event/KeyEvent;)V")
    public final void keyPressed(KeyEvent arg0) {
        this.field18 = 0;
        int var2 = arg0.getKeyCode();
        int var3 = arg0.getKeyChar();
        if (var3 < 30) {
            var3 = 0;
        }
        if (var2 == 37) {
            var3 = 1;
        }
        if (var2 == 39) {
            var3 = 2;
        }
        if (var2 == 38) {
            var3 = 3;
        }
        if (var2 == 40) {
            var3 = 4;
        }
        if (var2 == 17) {
            var3 = 5;
        }
        if (var2 == 8) {
            var3 = 8;
        }
        if (var2 == 127) {
            var3 = 8;
        }
        if (var2 == 9) {
            var3 = 9;
        }
        if (var2 == 10) {
            var3 = 10;
        }
        if (var2 >= 112 && var2 <= 123) {
            var3 = var2 + 1008 - 112;
        }
        if (var2 == 36) {
            var3 = 1000;
        }
        if (var2 == 35) {
            var3 = 1001;
        }
        if (var2 == 33) {
            var3 = 1002;
        }
        if (var2 == 34) {
            var3 = 1003;
        }
        if (var3 > 0 && var3 < 128) {
            this.field25[var3] = 1;
        }
        if (var3 > 4) {
            this.field26[this.field28] = var3;
            this.field28 = this.field28 + 1 & 0x7F;
        }
        if (InputTracking.field72) {
            InputTracking.method39(var3, true);
        }
        if (Linkable.field289) {
        }
    }

    @OriginalMember(owner = "client!a", name = "keyReleased", descriptor = "(Ljava/awt/event/KeyEvent;)V")
    public final void keyReleased(KeyEvent arg0) {
        this.field18 = 0;
        int var2 = arg0.getKeyCode();
        char var3 = arg0.getKeyChar();
        if (var3 < '\u001E') {
            var3 = '\u0000';
        }
        if (var2 == 37) {
            var3 = '\u0001';
        }
        if (var2 == 39) {
            var3 = '\u0002';
        }
        if (var2 == 38) {
            var3 = '\u0003';
        }
        if (var2 == 40) {
            var3 = '\u0004';
        }
        if (var2 == 17) {
            var3 = '\u0005';
        }
        if (var2 == 8) {
            var3 = '\b';
        }
        if (var2 == 127) {
            var3 = '\b';
        }
        if (var2 == 9) {
            var3 = '\t';
        }
        if (var2 == 10) {
            var3 = '\n';
        }
        if (var3 > '\u0000' && var3 < '\u0080') {
            this.field25[var3] = 0;
        }
        if (InputTracking.field72) {
            InputTracking.method40(this.field4, var3);
        }
    }

    @OriginalMember(owner = "client!a", name = "keyTyped", descriptor = "(Ljava/awt/event/KeyEvent;)V")
    public final void keyTyped(KeyEvent arg0) {
    }

    @OriginalMember(owner = "client!a", name = "focusGained", descriptor = "(Ljava/awt/event/FocusEvent;)V")
    public final void focusGained(FocusEvent arg0) {
        this.field17 = true;
        this.method10(false);
        if (InputTracking.field72) {
            InputTracking.method41(this.field5);
        }
    }

    @OriginalMember(owner = "client!a", name = "focusLost", descriptor = "(Ljava/awt/event/FocusEvent;)V")
    public final void focusLost(FocusEvent arg0) {
        if (InputTracking.field72) {
            InputTracking.method42((byte) 9);
        }
    }

    @OriginalMember(owner = "client!a", name = "b", descriptor = "(I)I")
    public final int method5(int arg0) {
        if (arg0 != 6542) {
            this.field1 = 230;
        }
        int var2 = -1;
        if (this.field28 != this.field27) {
            var2 = this.field26[this.field27];
            this.field27 = this.field27 + 1 & 0x7F;
        }
        return var2;
    }

    @OriginalMember(owner = "client!a", name = "windowActivated", descriptor = "(Ljava/awt/event/WindowEvent;)V")
    public final void windowActivated(WindowEvent arg0) {
    }

    @OriginalMember(owner = "client!a", name = "windowClosed", descriptor = "(Ljava/awt/event/WindowEvent;)V")
    public final void windowClosed(WindowEvent arg0) {
    }

    @OriginalMember(owner = "client!a", name = "windowClosing", descriptor = "(Ljava/awt/event/WindowEvent;)V")
    public final void windowClosing(WindowEvent arg0) {
        this.destroy();
    }

    @OriginalMember(owner = "client!a", name = "windowDeactivated", descriptor = "(Ljava/awt/event/WindowEvent;)V")
    public final void windowDeactivated(WindowEvent arg0) {
    }

    @OriginalMember(owner = "client!a", name = "windowDeiconified", descriptor = "(Ljava/awt/event/WindowEvent;)V")
    public final void windowDeiconified(WindowEvent arg0) {
    }

    @OriginalMember(owner = "client!a", name = "windowIconified", descriptor = "(Ljava/awt/event/WindowEvent;)V")
    public final void windowIconified(WindowEvent arg0) {
    }

    @OriginalMember(owner = "client!a", name = "windowOpened", descriptor = "(Ljava/awt/event/WindowEvent;)V")
    public final void windowOpened(WindowEvent arg0) {
    }

    @OriginalMember(owner = "client!a", name = "a", descriptor = "()V")
    public void method6() {
    }

    @OriginalMember(owner = "client!a", name = "a", descriptor = "(B)V")
    public void method7(byte arg0) {
        if (arg0 != 3) {
            for (int var2 = 1; var2 > 0; var2++) {
            }
        }
    }

    @OriginalMember(owner = "client!a", name = "a", descriptor = "(Z)V")
    public void method8(boolean arg0) {
        if (arg0) {
            this.field5 = !this.field5;
        }
    }

    @OriginalMember(owner = "client!a", name = "c", descriptor = "(I)V")
    public void method9(int arg0) {
        if (arg0 != -14379) {
            this.field3 = !this.field3;
        }
    }

    @OriginalMember(owner = "client!a", name = "b", descriptor = "(Z)V")
    public void method10(boolean arg0) {
        if (arg0) {
            for (int var2 = 1; var2 > 0; var2++) {
            }
        }
    }

    @OriginalMember(owner = "client!a", name = "d", descriptor = "(I)Ljava/awt/Component;")
    public java.awt.Component method11(int arg0) {
        if (arg0 != -36374) {
            this.field1 = -259;
        }
        return this.field16 == null ? this : this.field16;
    }

    @OriginalMember(owner = "client!a", name = "a", descriptor = "(Ljava/lang/Runnable;I)V")
    public void method12(Runnable arg0, int arg1) {
        Thread var3 = new Thread(arg0);
        var3.start();
        var3.setPriority(arg1);
    }

    @OriginalMember(owner = "client!a", name = "a", descriptor = "(BILjava/lang/String;)V")
    public void method13(byte arg0, int arg1, String arg2) {
        while (this.field13 == null) {
            this.field13 = this.method11(-36374).getGraphics();
            try {
                this.method11(-36374).repaint();
            } catch (Exception var10) {
            }
            try {
                Thread.sleep(1000L);
            } catch (Exception var9) {
            }
        }
        Font var4 = new Font("Helvetica", 1, 13);
        FontMetrics var5 = this.method11(-36374).getFontMetrics(var4);
        Font var6 = new Font("Helvetica", 0, 13);
        this.method11(-36374).getFontMetrics(var6);
        if (this.field17) {
            this.field13.setColor(Color.black);
            this.field13.fillRect(0, 0, this.field11, this.field12);
            this.field17 = false;
        }
        Color var7 = new Color(140, 17, 17);
        int var8 = this.field12 / 2 - 18;
        this.field13.setColor(var7);
        this.field13.drawRect(this.field11 / 2 - 152, var8, 304, 34);
        this.field13.fillRect(this.field11 / 2 - 150, var8 + 2, arg1 * 3, 30);
        this.field13.setColor(Color.black);
        if (arg0 == -75) {
            this.field13.fillRect(arg1 * 3 + this.field11 / 2 - 150, var8 + 2, 300 - arg1 * 3, 30);
            this.field13.setFont(var4);
            this.field13.setColor(Color.white);
            this.field13.drawString(arg2, (this.field11 - var5.stringWidth(arg2)) / 2, var8 + 22);
        }
    }
}
