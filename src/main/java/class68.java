import java.nio.ByteBuffer;
import javax.media.opengl.GL;
import org.openrs2.deob.annotation.OriginalClass;
import org.openrs2.deob.annotation.OriginalMember;

@OriginalClass("client!ri")
public class class68 extends class203 {

    @OriginalMember(owner = "client!ri", name = "V", descriptor = "I")
    public int field999 = -1;

    @OriginalMember(owner = "client!ri", name = "X", descriptor = "I")
    public int field1001 = 0;

    @OriginalMember(owner = "client!ri", name = "Y", descriptor = "I")
    private int field1002 = -1;

    @OriginalMember(owner = "client!ri", name = "ab", descriptor = "I")
    private int field1004 = 0;

    @OriginalMember(owner = "client!ri", name = "W", descriptor = "I")
    private int field1000;

    @OriginalMember(owner = "client!ri", name = "Z", descriptor = "I")
    public int field1003;

    @OriginalMember(owner = "client!ri", name = "D", descriptor = "I")
    public int field998;

    @OriginalMember(owner = "client!ri", name = "b", descriptor = "(IIII)V", line = 6)
    public final void method426(int arg0, int arg1, int arg2, int arg3) {
        class217.method1477();
        GL var5 = class217.field3551;
        class217.method1457(this.field999);
        this.method430(1);
        float var6 = (float) this.field3281 / (float) this.field998;
        float var7 = (float) this.field3284 / (float) this.field1003;
        float var8 = (float) arg2 * var6;
        float var9 = (float) arg3 * var7;
        int var10 = this.field3282 + arg0;
        int var11 = this.field3281 * arg2 + var10;
        int var12 = class217.field3531 - arg1 - this.field3280;
        int var13 = var12 - this.field3284 * arg3;
        var5.glBegin(6);
        var5.glTexCoord2f(var8, 0.0F);
        var5.glVertex2f((float) var11, (float) var12);
        var5.glTexCoord2f(0.0F, 0.0F);
        var5.glVertex2f((float) var10, (float) var12);
        var5.glTexCoord2f(0.0F, var9);
        var5.glVertex2f((float) var10, (float) var13);
        var5.glTexCoord2f(var8, var9);
        var5.glVertex2f((float) var11, (float) var13);
        var5.glEnd();
    }

    @OriginalMember(owner = "client!ri", name = "b", descriptor = "(IIIII)V", line = 37)
    public final void method427(int arg0, int arg1, int arg2, int arg3, int arg4) {
        if (arg2 <= 0 || arg3 <= 0) {
            return;
        }
        class217.method1455();
        int var6 = this.field3281;
        int var7 = this.field3284;
        int var8 = 0;
        int var9 = 0;
        int var10 = this.field3283;
        int var11 = this.field3278;
        int var12 = (var10 << 16) / arg2;
        int var13 = (var11 << 16) / arg3;
        if (this.field3282 > 0) {
            int var14 = ((this.field3282 << 16) + var12 - 1) / var12;
            arg0 += var14;
            var8 += var12 * var14 - (this.field3282 << 16);
        }
        if (this.field3280 > 0) {
            int var15 = ((this.field3280 << 16) + var13 - 1) / var13;
            arg1 += var15;
            var9 += var13 * var15 - (this.field3280 << 16);
        }
        if (var6 < var10) {
            arg2 = ((var6 << 16) + var12 - var8 - 1) / var12;
        }
        if (var7 < var11) {
            arg3 = ((var7 << 16) + var13 - var9 - 1) / var13;
        }
        GL var16 = class217.field3551;
        class217.method1457(this.field999);
        this.method430(1);
        float var17 = (float) arg0;
        float var18 = (float) arg2 + var17;
        float var19 = (float) (class217.field3531 - arg1);
        float var20 = var19 - (float) arg3;
        float var21 = (float) this.field3281 / (float) this.field998;
        float var22 = (float) this.field3284 / (float) this.field1003;
        float var23 = (float) arg4 / 256.0F;
        var16.glBegin(6);
        var16.glColor4f(1.0F, 1.0F, 1.0F, var23);
        var16.glTexCoord2f(var21, 0.0F);
        var16.glVertex2f(var18, var19);
        var16.glTexCoord2f(0.0F, 0.0F);
        var16.glVertex2f(var17, var19);
        var16.glTexCoord2f(0.0F, var22);
        var16.glVertex2f(var17, var20);
        var16.glTexCoord2f(var21, var22);
        var16.glVertex2f(var18, var20);
        var16.glEnd();
    }

    @OriginalMember(owner = "client!ri", name = "a", descriptor = "(IIII)V", line = 111)
    public final void method428(int arg0, int arg1, int arg2, int arg3) {
        if (arg2 <= 0 || arg3 <= 0) {
            return;
        }
        class217.method1477();
        int var5 = this.field3281;
        int var6 = this.field3284;
        int var7 = 0;
        int var8 = 0;
        int var9 = this.field3283;
        int var10 = this.field3278;
        int var11 = (var9 << 16) / arg2;
        int var12 = (var10 << 16) / arg3;
        if (this.field3282 > 0) {
            int var13 = ((this.field3282 << 16) + var11 - 1) / var11;
            arg0 += var13;
            var7 += var11 * var13 - (this.field3282 << 16);
        }
        if (this.field3280 > 0) {
            int var14 = ((this.field3280 << 16) + var12 - 1) / var12;
            arg1 += var14;
            var8 += var12 * var14 - (this.field3280 << 16);
        }
        if (var5 < var9) {
            arg2 = ((var5 << 16) + var11 - var7 - 1) / var11;
        }
        if (var6 < var10) {
            arg3 = ((var6 << 16) + var12 - var8 - 1) / var12;
        }
        GL var15 = class217.field3551;
        class217.method1457(this.field999);
        this.method430(2);
        float var16 = (float) arg0;
        float var17 = (float) arg2 + var16;
        float var18 = (float) (class217.field3531 - arg1);
        float var19 = var18 - (float) arg3;
        float var20 = (float) this.field3281 / (float) this.field998;
        float var21 = (float) this.field3284 / (float) this.field1003;
        var15.glBegin(6);
        var15.glTexCoord2f(var20, 0.0F);
        var15.glVertex2f(var17, var18);
        var15.glTexCoord2f(0.0F, 0.0F);
        var15.glVertex2f(var16, var18);
        var15.glTexCoord2f(0.0F, var21);
        var15.glVertex2f(var16, var19);
        var15.glTexCoord2f(var20, var21);
        var15.glVertex2f(var17, var19);
        var15.glEnd();
    }

    @OriginalMember(owner = "client!ri", name = "finalize", descriptor = "()V", line = 181)
    protected final void finalize() throws Throwable {
        if (this.field999 != -1) {
            class207.method1398(this.field999, this.field1001, this.field1000);
            this.field999 = -1;
            this.field1001 = 0;
        }
        if (this.field1002 != -1) {
            class207.method1396(this.field1002, this.field1000);
            this.field1002 = -1;
        }
        super.finalize();
    }

    @OriginalMember(owner = "client!ri", name = "c", descriptor = "(II)V", line = 197)
    public final void method429(int arg0, int arg1) {
        class217.method1477();
        int var3 = this.field3282 + arg0;
        int var4 = this.field3280 + arg1;
        GL var5 = class217.field3551;
        class217.method1457(this.field999);
        this.method430(1);
        var5.glTranslatef((float) var3, (float) (class217.field3531 - var4), 0.0F);
        var5.glCallList(this.field1002);
        var5.glLoadIdentity();
    }

    @OriginalMember(owner = "client!ri", name = "f", descriptor = "(I)V", line = 210)
    private final void method430(int arg0) {
        if (this.field1004 == arg0) {
            return;
        }
        this.field1004 = arg0;
        GL var2 = class217.field3551;
        if (arg0 == 2) {
            var2.glTexParameteri(3553, 10241, 9729);
            var2.glTexParameteri(3553, 10240, 9729);
        } else {
            var2.glTexParameteri(3553, 10241, 9728);
            var2.glTexParameteri(3553, 10240, 9728);
        }
    }

    @OriginalMember(owner = "client!ri", name = "a", descriptor = "(IILri;)V", line = 229)
    public final void method431(int arg0, int arg1, class68 arg2) {
        if (arg2 == null) {
            return;
        }
        class217.method1477();
        class217.method1457(arg2.field999);
        arg2.method430(1);
        GL var4 = class217.field3551;
        class217.method1457(this.field999);
        this.method430(1);
        var4.glActiveTexture(33985);
        var4.glEnable(3553);
        var4.glBindTexture(3553, arg2.field999);
        var4.glTexEnvi(8960, 34161, 7681);
        var4.glTexEnvi(8960, 34176, 34168);
        float var5 = (float) (arg0 - class13.field181) / (float) arg2.field998;
        float var6 = (float) (arg1 - class13.field185) / (float) arg2.field1003;
        float var7 = (float) (this.field3281 + arg0 - class13.field181) / (float) arg2.field998;
        float var8 = (float) (this.field3284 + arg1 - class13.field185) / (float) arg2.field1003;
        int var9 = this.field3282 + arg0;
        int var10 = this.field3280 + arg1;
        var4.glBegin(6);
        var4.glColor4f(1.0F, 1.0F, 1.0F, 1.0F);
        float var11 = (float) this.field3281 / (float) this.field998;
        float var12 = (float) this.field3284 / (float) this.field1003;
        var4.glMultiTexCoord2f(33985, var7, var6);
        var4.glTexCoord2f(var11, 0.0F);
        var4.glVertex2f((float) (this.field3281 + var9), (float) (class217.field3531 - var10));
        var4.glMultiTexCoord2f(33985, var5, var6);
        var4.glTexCoord2f(0.0F, 0.0F);
        var4.glVertex2f((float) var9, (float) (class217.field3531 - var10));
        var4.glMultiTexCoord2f(33985, var5, var8);
        var4.glTexCoord2f(0.0F, var12);
        var4.glVertex2f((float) var9, (float) (class217.field3531 - (this.field3284 + var10)));
        var4.glMultiTexCoord2f(33985, var7, var8);
        var4.glTexCoord2f(var11, var12);
        var4.glVertex2f((float) (this.field3281 + var9), (float) (class217.field3531 - (this.field3284 + var10)));
        var4.glEnd();
        var4.glTexEnvi(8960, 34161, 8448);
        var4.glTexEnvi(8960, 34176, 5890);
        var4.glDisable(3553);
        var4.glActiveTexture(33984);
    }

    @OriginalMember(owner = "client!ri", name = "b", descriptor = "(IIIIII)V", line = 279)
    public final void method432(int arg0, int arg1, int arg2, int arg3, int arg4, int arg5) {
        class217.method1477();
        GL var7 = class217.field3551;
        class217.method1457(this.field999);
        this.method430(2);
        int var8 = arg0 - (this.field3282 << 4);
        int var9 = arg1 - (this.field3280 << 4);
        var7.glTranslatef((float) arg2 / 16.0F, (float) class217.field3531 - (float) arg3 / 16.0F, 0.0F);
        var7.glRotatef((float) (-arg4) * 0.005493164F, 0.0F, 0.0F, 1.0F);
        if (arg5 != 4096) {
            var7.glScalef((float) arg5 / 4096.0F, (float) arg5 / 4096.0F, 0.0F);
        }
        var7.glTranslatef((float) (-var8) / 16.0F, (float) var9 / 16.0F, 0.0F);
        var7.glCallList(this.field1002);
        var7.glLoadIdentity();
    }

    @OriginalMember(owner = "client!ri", name = "a", descriptor = "([I)V", line = 297)
    public void method433(int[] arg0) {
        this.field998 = class270.method1847(this.field3281, 96);
        this.field1003 = class270.method1847(this.field3284, 96);
        byte[] var2 = new byte[this.field998 * this.field1003 * 4];
        int var3 = 0;
        int var4 = 0;
        int var5 = (this.field998 - this.field3281) * 4;
        for (int var6 = 0; var6 < this.field3284; var6++) {
            for (int var7 = 0; var7 < this.field3281; var7++) {
                int var8 = arg0[var4++];
                if (var8 == 0) {
                    var3 += 4;
                } else {
                    var2[var3++] = (byte) (var8 >> 16);
                    var2[var3++] = (byte) (var8 >> 8);
                    var2[var3++] = (byte) var8;
                    var2[var3++] = -1;
                }
            }
            var3 += var5;
        }
        ByteBuffer var9 = ByteBuffer.wrap(var2);
        GL var10 = class217.field3551;
        if (this.field999 == -1) {
            int[] var11 = new int[1];
            var10.glGenTextures(1, var11, 0);
            this.field999 = var11[0];
            this.field1000 = class207.field3383;
        }
        class217.method1457(this.field999);
        var10.glTexImage2D(3553, 0, 6408, this.field998, this.field1003, 0, 6408, 5121, var9);
        class207.field3382 += var9.limit() - this.field1001;
        this.field1001 = var9.limit();
    }

    @OriginalMember(owner = "client!ri", name = "a", descriptor = "(IIIIII)V", line = 356)
    public final void method434(int arg0, int arg1, int arg2, int arg3, int arg4, int arg5) {
        class217.method1477();
        GL var7 = class217.field3551;
        class217.method1457(this.field999);
        this.method430(1);
        int var8 = arg0 - (this.field3282 << 4);
        int var9 = arg1 - (this.field3280 << 4);
        var7.glTranslatef((float) arg2 / 16.0F, (float) class217.field3531 - (float) arg3 / 16.0F, 0.0F);
        var7.glRotatef((float) arg4 * 0.005493164F, 0.0F, 0.0F, 1.0F);
        if (arg5 != 4096) {
            var7.glScalef((float) arg5 / 4096.0F, (float) arg5 / 4096.0F, 0.0F);
        }
        var7.glTranslatef((float) (-var8) / 16.0F, (float) var9 / 16.0F, 0.0F);
        var7.glCallList(this.field1002);
        var7.glLoadIdentity();
    }

    @OriginalMember(owner = "client!ri", name = "a", descriptor = "(II)V", line = 374)
    public final void method435(int arg0, int arg1) {
        class217.method1477();
        int var3 = this.field3282 + arg0;
        int var4 = this.field3280 + arg1;
        GL var5 = class217.field3551;
        class217.method1457(this.field999);
        this.method430(1);
        var5.glTranslatef((float) var3, (float) (class217.field3531 - var4), 0.0F);
        var5.glCallList(this.field1002);
        var5.glLoadIdentity();
    }

    @OriginalMember(owner = "client!ri", name = "d", descriptor = "(II)V", line = 386)
    public final void method436(int arg0, int arg1) {
        class217.method1477();
        int var3 = this.field3282 + arg0;
        int var4 = this.field3280 + arg1;
        GL var5 = class217.field3551;
        class217.method1457(this.field999);
        this.method430(1);
        var5.glTranslatef((float) var3, (float) (class217.field3531 - var4), 0.0F);
        float var6 = (float) this.field3281 / (float) this.field998;
        float var7 = (float) this.field3284 / (float) this.field1003;
        var5.glBegin(6);
        var5.glTexCoord2f(0.0F, 0.0F);
        var5.glVertex2f((float) this.field3281, 0.0F);
        var5.glTexCoord2f(var6, 0.0F);
        var5.glVertex2f(0.0F, 0.0F);
        var5.glTexCoord2f(var6, var7);
        var5.glVertex2f(0.0F, (float) (-this.field3284));
        var5.glTexCoord2f(0.0F, var7);
        var5.glVertex2f((float) this.field3281, (float) (-this.field3284));
        var5.glEnd();
        var5.glLoadIdentity();
    }

    @OriginalMember(owner = "client!ri", name = "<init>", descriptor = "(IIIIII[I)V", line = 572)
    public class68(int arg0, int arg1, int arg2, int arg3, int arg4, int arg5, int[] arg6) {
        this.field3283 = arg0;
        this.field3278 = arg1;
        this.field3282 = arg2;
        this.field3280 = arg3;
        this.field3281 = arg4;
        this.field3284 = arg5;
        this.method433(arg6);
        this.method437();
    }

    @OriginalMember(owner = "client!ri", name = "<init>", descriptor = "(Ldc;)V", line = 582)
    public class68(class109 arg0) {
        this.field3283 = arg0.field3283;
        this.field3278 = arg0.field3278;
        this.field3282 = arg0.field3282;
        this.field3280 = arg0.field3280;
        this.field3281 = arg0.field3281;
        this.field3284 = arg0.field3284;
        this.method433(arg0.field1552);
        this.method437();
    }

    @OriginalMember(owner = "client!ri", name = "a", descriptor = "()V", line = 416)
    private final void method437() {
        float var1 = (float) this.field3281 / (float) this.field998;
        float var2 = (float) this.field3284 / (float) this.field1003;
        GL var3 = class217.field3551;
        if (this.field1002 == -1) {
            this.field1002 = var3.glGenLists(1);
            this.field1000 = class207.field3383;
        }
        var3.glNewList(this.field1002, 4864);
        var3.glBegin(6);
        var3.glTexCoord2f(var1, 0.0F);
        var3.glVertex2f((float) this.field3281, 0.0F);
        var3.glTexCoord2f(0.0F, 0.0F);
        var3.glVertex2f(0.0F, 0.0F);
        var3.glTexCoord2f(0.0F, var2);
        var3.glVertex2f(0.0F, (float) (-this.field3284));
        var3.glTexCoord2f(var1, var2);
        var3.glVertex2f((float) this.field3281, (float) (-this.field3284));
        var3.glEnd();
        var3.glEndList();
    }

    @OriginalMember(owner = "client!ri", name = "a", descriptor = "(III)V", line = 441)
    public final void method438(int arg0, int arg1, int arg2) {
        class217.method1455();
        int var4 = this.field3282 + arg0;
        int var5 = this.field3280 + arg1;
        GL var6 = class217.field3551;
        class217.method1457(this.field999);
        this.method430(1);
        var6.glColor4f(1.0F, 1.0F, 1.0F, (float) arg2 / 256.0F);
        var6.glTranslatef((float) var4, (float) (class217.field3531 - var5), 0.0F);
        var6.glCallList(this.field1002);
        var6.glLoadIdentity();
    }

    @OriginalMember(owner = "client!ri", name = "c", descriptor = "(IIIII)V", line = 457)
    public final void method439(int arg0, int arg1, int arg2, int arg3, int arg4) {
        class217.method1455();
        GL var6 = class217.field3551;
        class217.method1457(this.field999);
        this.method430(1);
        float var7 = (float) this.field3281 / (float) this.field998;
        float var8 = (float) this.field3284 / (float) this.field1003;
        float var9 = (float) arg3 * var7;
        float var10 = (float) arg4 * var8;
        int var11 = this.field3282 + arg0;
        int var12 = this.field3281 * arg3 + var11;
        int var13 = class217.field3531 - arg1 - this.field3280;
        int var14 = var13 - this.field3284 * arg4;
        float var15 = (float) arg2 / 256.0F;
        var6.glBegin(6);
        var6.glColor4f(1.0F, 1.0F, 1.0F, var15);
        var6.glTexCoord2f(var9, 0.0F);
        var6.glVertex2f((float) var12, (float) var13);
        var6.glTexCoord2f(0.0F, 0.0F);
        var6.glVertex2f((float) var11, (float) var13);
        var6.glTexCoord2f(0.0F, var10);
        var6.glVertex2f((float) var11, (float) var14);
        var6.glTexCoord2f(var9, var10);
        var6.glVertex2f((float) var12, (float) var14);
        var6.glEnd();
    }

    @OriginalMember(owner = "client!ri", name = "a", descriptor = "(IIIIIIIILri;)V", line = 495)
    public final void method440(int arg0, int arg1, int arg2, int arg3, int arg4, int arg5, int arg6, int arg7, class68 arg8) {
        if (arg8 == null) {
            return;
        }
        class217.method1477();
        class217.method1457(arg8.field999);
        arg8.method430(1);
        GL var10 = class217.field3551;
        class217.method1457(this.field999);
        this.method430(1);
        var10.glActiveTexture(33985);
        var10.glEnable(3553);
        var10.glBindTexture(3553, arg8.field999);
        var10.glTexEnvi(8960, 34161, 7681);
        var10.glTexEnvi(8960, 34176, 34168);
        int var11 = -arg2 / 2;
        int var12 = -arg3 / 2;
        int var13 = -var11;
        int var14 = -var12;
        int var15 = (int) (Math.sin((double) arg6 / 326.11D) * 65536.0D);
        int var16 = (int) (Math.cos((double) arg6 / 326.11D) * 65536.0D);
        int var17 = arg7 * var15 >> 8;
        int var18 = arg7 * var16 >> 8;
        int var19 = (arg4 << 16) + var11 * var18 + var12 * var17;
        int var20 = (arg5 << 16) + (var12 * var18 - var11 * var17);
        int var21 = (arg4 << 16) + var12 * var17 + var13 * var18;
        int var22 = (arg5 << 16) + (var12 * var18 - var13 * var17);
        int var23 = (arg4 << 16) + var11 * var18 + var14 * var17;
        int var24 = (arg5 << 16) + (var14 * var18 - var11 * var17);
        int var25 = (arg4 << 16) + var13 * var18 + var14 * var17;
        int var26 = (arg5 << 16) + (var14 * var18 - var13 * var17);
        float var27 = (float) arg8.field3281 / (float) arg8.field998;
        float var28 = (float) arg8.field3284 / (float) arg8.field1003;
        var10.glBegin(6);
        var10.glColor4f(1.0F, 1.0F, 1.0F, 1.0F);
        float var29 = (float) this.field998 * 65536.0F;
        float var30 = (float) (this.field1003 * 65536);
        var10.glMultiTexCoord2f(33985, var27, 0.0F);
        var10.glTexCoord2f((float) var21 / var29, (float) var22 / var30);
        var10.glVertex2f((float) (arg0 + arg2), (float) (class217.field3531 - arg1));
        var10.glMultiTexCoord2f(33985, 0.0F, 0.0F);
        var10.glTexCoord2f((float) var19 / var29, (float) var20 / var30);
        var10.glVertex2f((float) arg0, (float) (class217.field3531 - arg1));
        var10.glMultiTexCoord2f(33985, 0.0F, var28);
        var10.glTexCoord2f((float) var23 / var29, (float) var24 / var30);
        var10.glVertex2f((float) arg0, (float) (class217.field3531 - (arg1 + arg3)));
        var10.glMultiTexCoord2f(33985, var27, var28);
        var10.glTexCoord2f((float) var25 / var29, (float) var26 / var30);
        var10.glVertex2f((float) (arg0 + arg2), (float) (class217.field3531 - (arg1 + arg3)));
        var10.glEnd();
        var10.glTexEnvi(8960, 34161, 8448);
        var10.glTexEnvi(8960, 34176, 5890);
        var10.glDisable(3553);
        var10.glActiveTexture(33984);
    }
}
