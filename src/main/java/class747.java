import org.openrs2.deob.annotation.OriginalClass;
import org.openrs2.deob.annotation.OriginalMember;

@OriginalClass("client!dw")
public class class747 {

    @OriginalMember(owner = "client!dw", name = "a", descriptor = "I")
    public static int field10380 = 0;

    @OriginalMember(owner = "client!dw", name = "c", descriptor = "Z")
    public static boolean field10382 = false;

    @OriginalMember(owner = "client!dw", name = "b", descriptor = "[I")
    public static int[] field10381 = new int[5];

    @OriginalMember(owner = "client!dw", name = "a", descriptor = "(I)V", line = 8)
    public static void method4164(int arg0) {
        field10381 = null;
        int var1 = -51 / ((75 - arg0) / 37);
    }
}
