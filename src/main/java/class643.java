import org.openrs2.deob.annotation.OriginalClass;
import org.openrs2.deob.annotation.OriginalMember;

@OriginalClass("client!waa")
public class class643 extends class270 {

    @OriginalMember(owner = "client!waa", name = "m", descriptor = "I")
    public int field9086;

    @OriginalMember(owner = "client!waa", name = "n", descriptor = "I")
    public int field9087;

    @OriginalMember(owner = "client!waa", name = "o", descriptor = "I")
    public int field9088;

    @OriginalMember(owner = "client!waa", name = "p", descriptor = "I")
    public int field9089;

    @OriginalMember(owner = "client!waa", name = "q", descriptor = "I")
    public static int field9090;

    @OriginalMember(owner = "client!waa", name = "r", descriptor = "I")
    public int field9091;

    @OriginalMember(owner = "client!waa", name = "t", descriptor = "I")
    public int field9093;

    @OriginalMember(owner = "client!waa", name = "v", descriptor = "I")
    public int field9095;

    @OriginalMember(owner = "client!waa", name = "x", descriptor = "I")
    public int field9097;

    @OriginalMember(owner = "client!waa", name = "y", descriptor = "I")
    public int field9098;

    @OriginalMember(owner = "client!waa", name = "A", descriptor = "I")
    public int field9100;

    @OriginalMember(owner = "client!waa", name = "B", descriptor = "I")
    public static int field9101;

    @OriginalMember(owner = "client!waa", name = "C", descriptor = "I")
    public int field9102;

    @OriginalMember(owner = "client!waa", name = "D", descriptor = "I")
    public int field9103;

    @OriginalMember(owner = "client!waa", name = "E", descriptor = "I")
    public int field9104;

    @OriginalMember(owner = "client!waa", name = "F", descriptor = "I")
    public int field9105;

    @OriginalMember(owner = "client!waa", name = "G", descriptor = "I")
    public int field9106;

    @OriginalMember(owner = "client!waa", name = "H", descriptor = "I")
    public int field9107;

    @OriginalMember(owner = "client!waa", name = "I", descriptor = "I")
    public int field9108;

    @OriginalMember(owner = "client!waa", name = "w", descriptor = "Lgg;")
    public class159 field9096;

    @OriginalMember(owner = "client!waa", name = "z", descriptor = "Lwb;")
    public class298 field9099;

    @OriginalMember(owner = "client!waa", name = "u", descriptor = "Lnr;")
    public class54 field9094;

    @OriginalMember(owner = "client!waa", name = "s", descriptor = "Lu;")
    public class67 field9092;

    @OriginalMember(owner = "client!waa", name = "a", descriptor = "(I)V", line = 12)
    public final void method3609(int arg0) {
        field9090++;
        this.field9099 = null;
        if (arg0 == 20235) {
            this.field9096 = null;
            this.field9092 = null;
            this.field9094 = null;
        }
    }
}
