import org.openrs2.deob.annotation.OriginalClass;
import org.openrs2.deob.annotation.OriginalMember;

@OriginalClass("client!pc")
public class SpotAnimType {

    @OriginalMember(owner = "client!pc", name = "f", descriptor = "I")
    public int field1169 = -1;

    @OriginalMember(owner = "client!pc", name = "h", descriptor = "[I")
    public int[] field1171 = new int[6];

    @OriginalMember(owner = "client!pc", name = "i", descriptor = "[I")
    public int[] field1172 = new int[6];

    @OriginalMember(owner = "client!pc", name = "j", descriptor = "I")
    public int field1173 = 128;

    @OriginalMember(owner = "client!pc", name = "k", descriptor = "I")
    public int field1174 = 128;

    @OriginalMember(owner = "client!pc", name = "o", descriptor = "Ls;")
    public static LruCache field1178 = new LruCache(30, true);

    @OriginalMember(owner = "client!pc", name = "b", descriptor = "I")
    public static int field1165;

    @OriginalMember(owner = "client!pc", name = "d", descriptor = "I")
    public int field1167;

    @OriginalMember(owner = "client!pc", name = "e", descriptor = "I")
    public int field1168;

    @OriginalMember(owner = "client!pc", name = "l", descriptor = "I")
    public int field1175;

    @OriginalMember(owner = "client!pc", name = "m", descriptor = "I")
    public int field1176;

    @OriginalMember(owner = "client!pc", name = "n", descriptor = "I")
    public int field1177;

    @OriginalMember(owner = "client!pc", name = "g", descriptor = "Lnc;")
    public SeqType field1170;

    @OriginalMember(owner = "client!pc", name = "a", descriptor = "Z")
    private static boolean field1164;

    @OriginalMember(owner = "client!pc", name = "c", descriptor = "[Lpc;")
    public static SpotAnimType[] field1166;

    @OriginalMember(owner = "client!pc", name = "a", descriptor = "(Lxb;I)V")
    public static void method384(Jagfile arg0, int arg1) {
        Packet var2 = new Packet(arg0.method299("spotanim.dat", null), field1164);
        field1165 = var2.method231();
        if (field1166 == null) {
            field1166 = new SpotAnimType[field1165];
        }
        for (int var3 = 0; var3 < field1165; var3++) {
            if (field1166[var3] == null) {
                field1166[var3] = new SpotAnimType();
            }
            field1166[var3].field1167 = var3;
            field1166[var3].method385(false, var2);
        }
        if (arg1 != 5) {
            for (int var4 = 1; var4 > 0; var4++) {
            }
        }
    }

    @OriginalMember(owner = "client!pc", name = "a", descriptor = "(ZLlb;)V")
    public void method385(boolean arg0, Packet arg1) {
        if (arg0) {
            for (int var3 = 1; var3 > 0; var3++) {
            }
        }
        while (true) {
            while (true) {
                int var4 = arg1.method229();
                if (var4 == 0) {
                    return;
                }
                if (var4 == 1) {
                    this.field1168 = arg1.method231();
                } else if (var4 == 2) {
                    this.field1169 = arg1.method231();
                    if (SeqType.field1144 != null) {
                        this.field1170 = SeqType.field1144[this.field1169];
                    }
                } else if (var4 == 4) {
                    this.field1173 = arg1.method231();
                } else if (var4 == 5) {
                    this.field1174 = arg1.method231();
                } else if (var4 == 6) {
                    this.field1175 = arg1.method231();
                } else if (var4 == 7) {
                    this.field1176 = arg1.method229();
                } else if (var4 == 8) {
                    this.field1177 = arg1.method229();
                } else if (var4 >= 40 && var4 < 50) {
                    this.field1171[var4 - 40] = arg1.method231();
                } else if (var4 >= 50 && var4 < 60) {
                    this.field1172[var4 - 50] = arg1.method231();
                } else {
                    System.out.println("Error unrecognised spotanim config code: " + var4);
                }
            }
        }
    }

    @OriginalMember(owner = "client!pc", name = "a", descriptor = "()Leb;")
    public Model method386() {
        Model var1 = (Model) field1178.method101((long) this.field1167);
        if (var1 != null) {
            return var1;
        }
        Model var2 = Model.method126((byte) 4, this.field1168);
        if (var2 == null) {
            return null;
        }
        for (int var3 = 0; var3 < 6; var3++) {
            if (this.field1171[0] != 0) {
                var2.method140(this.field1171[var3], this.field1172[var3]);
            }
        }
        field1178.method102((long) this.field1167, -533, var2);
        return var2;
    }
}
