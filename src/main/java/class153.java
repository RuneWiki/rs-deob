import java.nio.ByteBuffer;
import javax.media.opengl.GL;
import org.openrs2.deob.annotation.OriginalClass;
import org.openrs2.deob.annotation.OriginalMember;

@OriginalClass("client!oc")
public class class153 extends class196 {

    @OriginalMember(owner = "client!oc", name = "<init>", descriptor = "(Leg;)V", line = 5)
    public class153(class317 arg0) {
        super(arg0);
    }

    @OriginalMember(owner = "client!oc", name = "<init>", descriptor = "(IIIIII[I)V", line = 8)
    public class153(int arg0, int arg1, int arg2, int arg3, int arg4, int arg5, int[] arg6) {
        super(arg0, arg1, arg2, arg3, arg4, arg5, arg6);
    }

    @OriginalMember(owner = "client!oc", name = "a", descriptor = "([I)V", line = 11)
    public final void method1147(int[] arg0) {
        this.field3022 = class319.method2255(1, this.field1412);
        this.field3024 = class319.method2255(1, this.field1413);
        byte[] var2 = new byte[this.field3024 * this.field3022 * 4];
        int var3 = 0;
        int var4 = 0;
        int var5 = (this.field3022 - this.field1412) * 4;
        for (int var6 = 0; var6 < this.field1413; var6++) {
            for (int var7 = 0; var7 < this.field1412; var7++) {
                int var8 = arg0[var4++];
                if (var8 == 0) {
                    var3 += 4;
                } else {
                    var2[var3++] = (byte) (var8 >> 16);
                    var2[var3++] = (byte) (var8 >> 8);
                    var2[var3++] = (byte) var8;
                    var2[var3++] = (byte) (var8 >> 24);
                }
            }
            var3 += var5;
        }
        ByteBuffer var9 = ByteBuffer.wrap(var2);
        GL var10 = class162.field2654;
        if (this.field3026 == -1) {
            int[] var11 = new int[1];
            var10.glGenTextures(1, var11, 0);
            this.field3026 = var11[0];
        }
        class162.method1219(this.field3026);
        var10.glTexImage2D(3553, 0, 6408, this.field3022, this.field3024, 0, 6408, 5121, var9);
        class177.field2821 += var9.limit() - this.field3027;
        this.field3027 = var9.limit();
    }
}
