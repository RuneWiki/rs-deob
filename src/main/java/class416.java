import org.openrs2.deob.annotation.OriginalClass;
import org.openrs2.deob.annotation.OriginalMember;

@OriginalClass("client!mt")
public class class416 extends class302 {
   @OriginalMember(
      owner = "client!mt",
      name = "M",
      descriptor = "I"
   )
   private int field6363 = 10;
   @OriginalMember(
      owner = "client!mt",
      name = "V",
      descriptor = "I"
   )
   private int field6369 = 2048;
   @OriginalMember(
      owner = "client!mt",
      name = "K",
      descriptor = "I"
   )
   private int field6372 = 0;
   @OriginalMember(
      owner = "client!mt",
      name = "H",
      descriptor = "[Ljava/lang/String;"
   )
   private static final String[] field6377 = new String[]{method3185(method3184("\"\u0010+Ki")), method3185(method3184("\"\u0010+_i")), method3185(method3184("\"\u0010+Yi")), method3185(method3184("\"\u0010+^i")), method3185(method3184("\"\u0010+Hi")), method3185(method3184("\"\u0010+[i")), method3185(method3184("!\u0011iv")), method3185(method3184("4J+4<")), method3185(method3184("\"\u0010+\\i")), method3185(method3184("\"\u0010+]i"))};
   @OriginalMember(
      owner = "client!mt",
      name = "bb",
      descriptor = "[[S"
   )
   private static short[][] field6358 = new short[][]{new short[0], new short[0], new short[0], new short[0], new short[0]};
   @OriginalMember(
      owner = "client!mt",
      name = "I",
      descriptor = "Lhv;"
   )
   public static class544 field6361 = null;
   @OriginalMember(
      owner = "client!mt",
      name = "W",
      descriptor = "[[S"
   )
   private static short[][] field6374 = new short[][]{{6798, 107, 10283, 16, 4797, 7744, 5799, 4634, -31839, 22433, 2983, -11343, 8, 5281, 10438, 3650, -27322, -21845, 200, 571, 908, 21830, 28946, -15701, -14010}, {8741, 12, -1506, -22374, 7735, 8404, 1701, -27106, 24094, 10153, -8915, 4783, 1341, 16578, -30533, 25239, 8, 5281, 10438, 3650, -27322, -21845, 200, 571, 908, 21830, 28946, -15701, -14010, 7085, 4001, 317, 918, 9517, 9125, 3608, 16450, -32593, 22026, 2700, -22256, 10289, 3877, -27999, 10770, 8982, 3753, 10275, 924, 8637, -26428, 26785, -15202, 8736, 2994, -25710, 928, 6859, -25016, -5449, -14823, -23401, 12, -22456, -27471, -11229, 21673, 57, -24046, -27190, -22877, -30041, 20013, 21596, -31414, 29000, -11209, 6361, -25552, -25450, -14174, -10998, -27357, 3249, 7465, 8913, 13774, 4584, 11728, 10454, 8123, 2619, 13756, -28220, -26323, -23901, -31385, -32342, -29806, -29902, -24128, 933, 1189, 3873, 6057, 8590, 1812, 4895, -5366, 15019, 20793, 7599, 9536, 21778, 14102, 6938, 14648, 5919, 6472, 10128, 17940, 3226, 267, 5421, -27570, -25177, -26319, -28217, -28132, -23792, -23249, -23001, -19033, 5427, 6340, 5673, 2458, 3625, 8646, 5186, 10382, 6826, 7442, 158, 2454, 9655, -27496, -27578, -31687, 11146, 18712, 10785, 21902, 10566, 9500, 11819, 11144, -15965, -24170, -32488, -29277, 29097, -9567, -18794, -27116, 921, 6028, 2974, -5230, 6550, 15244, 14, 49, 3994, 2332, 6038, -31701, 520, 41, 9402, -4825, 28978, -25278, 30876, 13475, -28395, -24010, -14027, -22454, -18158, 20503, -27618, -22258, -1776, -31468, 9616, 7333, 650, 784, 796, 813, -616, -3194, -8152, 1686, 29074, 22422, -32245, 29084, 22411, -22000, 3982, 7062, 10382, 3230, 8076, 20, 1936, 10, -22473, 4892, 5805, 5904, 10, 48, 1936, 5640, 8472, -25529}, {25238, 8742, 12, -1506, -22374, 7735, 8404, 1701, -27106, 24094, 10153, -8915, 4783, 1341, 16578, -30533, 8, 5281, 10438, 3650, -27322, -21845, 200, 571, 908, 21830, 28946, -15701, -14010, 7085, 4001, 317, 918, 9517, 9125, 3608, 16450, -32593, 22026, 2700, -22256, 10289, 3877, -27999, 10770, 8982, 3753, 10275, 924, 8637, -26428, 26785, -15202, 8736, 2994, -25710, 928, 6859, -25016, -5449, -14823, -23401, 12, -22456, -27471, -11229, 21673, 57, -24046, -27190, -22877, -30041, 20013, 21596, -31414, 29000, -11209, 6361, -25552, -25450, -14174, -10998, -27357, 3249, 7465, 8913, 13774, 4584, 11728, 10454, 8123, 2619, 13756, -28220, -26323, -23901, -31385, -32342, -29806, -29902, -24128, 933, 1189, 3873, 6057, 8590, 1812, 4895, -5366, 15019, 20793, 7599, 9536, 21778, 14102, 6938, 14648, 5919, 6472, 10128, 17940, 3226, 267, 5421, -27570, -25177, -26319, -28217, -28132, -23792, -23249, -23001, -19033, 5427, 6340, 5673, 2458, 3625, 8646, 5186, 10382, 6826, 7442, 158, 2454, 9655, -27496, -27578, -31687, 11146, 18712, 10785, 21902, 10566, 9500, 11819, 11144, -15965, -24170, -32488, -29277, 29097, -9567, -18794, -27116, 921, 6028, 2974, -5230, 6550, 15244, 14, 49, 3994, 2332, 6038, -31701, 520, 41, 9402, -4825, 28978, -25278, 30876, 13475, -28395, -24010, -14027, -22454, -18158, 20503, -27618, -22258, -1776, -31468, 9616, 7333, 650, 784, 796, 813, -616, -3194, -8152, 1686, 29074, 22422, -32245, 29084, 22411, -22000, 3982, 7062, 10382, 3230, 8076, 20, 1936, 10, -22473, 4892, 5805, 5904, 10, 48, 1936, 5640, 8472, -25529}, {4626, 11146, 6439, 12, 4758, 10270, 7085, 4001, 317, 918, 9517, 9125, 3608, 16450, -32593, 22026, 2700, -22256, 10289, 3877, -27999, 10770, 8982, 3753, 10275, 924, 8637, -26428, 26785, -15202, 8736, 2994, -25710, 928, 6859, -25016, -5449, -14823, -23401, 12, -22456, -27471, -11229, 21673, 57, -24046, -27190, -22877, -30041, 20013, 21596, -31414, 29000, -11209, 6361, -25552, -25450, -14174, -10998, -27357, 3249, 7465, 8913, 13774, 4584, 11728, 10454, 8123, 2619, 13756, -28220, -26323, -23901, -31385, -32342, -29806, -29902, -24128, 933, 1189, 3873, 6057, 8590, 1812, 4895, -5366, 15019, 20793, 7599, 9536, 21778, 14102, 6938, 14648, 5919, 6472, 10128, 17940, 3226, 267, 5421, -27570, -25177, -26319, -28217, -28132, -23792, -23249, -23001, -19033, 5427, 6340, 5673, 2458, 3625, 8646, 5186, 10382, 6826, 7442, 158, 2454, 9655, -27496, -27578, -31687, 11146, 18712, 10785, 21902, 10566, 9500, 11819, 11144, -15965, -24170, -32488, -29277, 29097, -9567, -18794, -27116, 921, 6028, 2974, -5230, 6550, 15244, 14, 49, 3994, 2332, 6038, -31701, 520, 41, 9402, -4825, 28978, -25278, 30876, 13475, -28395, -24010, -14027, -22454, -18158, 20503, -27618, -22258, -1776, -31468, 9616, 7333, 650, 784, 796, 813, -616, -3194, -8152, 1686, 29074, 22422, -32245, 29084, 22411, -22000, 3982, 7062, 10382, 3230, 8076, 20, 1936, 10, -22473, 4892, 5805, 5904, 10, 48, 1936, 5640, 8472, -25529}, {4550, 4537, 5681, 5673, 5790, 6806, 8076, 4574, 4444, 4328, 7052, 4490}};
   @OriginalMember(
      owner = "client!mt",
      name = "T",
      descriptor = "Z"
   )
   public static boolean field6373 = false;
   @OriginalMember(
      owner = "client!mt",
      name = "ab",
      descriptor = "[[S"
   )
   private static short[][] field6367 = new short[][]{{6554, 115, 10304, 28, 5702, 7756, 5681, 4510, -31835, 22437, 2859, -11339, 16, 5157, 10446, 3658, -27314, -21965, 472, 580, 784, 21966, 28950, -15697, -14002}, {9104, 10275, 7595, 3610, 7975, 8526, 918, -26734, 24466, 10145, -6882, 5027, 1457, 16565, -30545, 25486, 24, 5392, 10429, 3673, -27335, -21957, 192, 687, 412, 21821, 28835, -15460, -14019, 7072, 3990, 309, 910, 9509, 9117, 3600, 16442, -32601, 22020, 2694, -22264, 10281, 3869, -28007, 10764, 8974, 3745, 10269, 914, 8629, -26438, 26779, -15208, 8728, 2982, -25718, 918, 6842, -25030, -5462, -14833, -22386, 8, -22468, -27479, -11237, 21664, 45, -24054, -27207, -22887, -30051, 20003, 21584, -31426, 28990, -11219, 6353, -25560, -25458, -14182, -11006, -27365, 3241, 7457, 8899, 13760, 4565, 11711, 10436, 8103, 2604, 13740, -28232, -26333, -23910, -31403, -32352, -29812, -29909, -24137, 925, 1181, 3865, 6049, 8582, 1804, 4887, -5372, 15011, 20785, 7591, 9528, 21770, 14094, 6930, 14635, 5911, 6463, 10124, 17935, 3218, 262, 5413, -27579, -25189, -26331, -28236, -28144, -23804, -23261, -23013, -19045, 5419, 6332, 5665, 2450, 3617, 8638, 5178, 10374, 6819, 7434, 150, 2446, 9647, -27504, -27586, -31695, 11142, 18704, 10775, 21898, 10554, 9492, 11811, 11140, -15978, -24180, -32498, -29288, 29084, -9579, -18799, -27122, 913, 6024, 2966, -5238, 6542, 15240, 10, 39, 3986, 2324, 6030, -31709, 512, 33, 9394, -4833, 28967, -25290, 30866, 13466, -28403, -24022, -14042, -22472, -18166, 20493, -27626, -22266, -1784, -31473, 9612, 7324, 646, 778, 786, 800, -624, -3198, -8164, 1678, 29068, 22412, -32253, 29075, 22404, -22006, 3974, 7054, 10376, 3222, 8070, 12, 1930, 2, -22481, 4884, 5797, 5900, 6, 40, 1932, 5638, 8466, -25537}, {25485, 9105, 10275, 7595, 3610, 7975, 8526, 918, -26734, 24466, 10145, -6882, 5027, 1457, 16565, -30545, 24, 5392, 10429, 3673, -27335, -21957, 192, 687, 412, 21821, 28835, -15460, -14019, 7072, 3990, 309, 910, 9509, 9117, 3600, 16442, -32601, 22020, 2694, -22264, 10281, 3869, -28007, 10764, 8974, 3745, 10269, 914, 8629, -26438, 26779, -15208, 8728, 2982, -25718, 918, 6842, -25030, -5462, -14833, -22386, 8, -22468, -27479, -11237, 21664, 45, -24054, -27207, -22887, -30051, 20003, 21584, -31426, 28990, -11219, 6353, -25560, -25458, -14182, -11006, -27365, 3241, 7457, 8899, 13760, 4565, 11711, 10436, 8103, 2604, 13740, -28232, -26333, -23910, -31403, -32352, -29812, -29909, -24137, 925, 1181, 3865, 6049, 8582, 1804, 4887, -5372, 15011, 20785, 7591, 9528, 21770, 14094, 6930, 14635, 5911, 6463, 10124, 17935, 3218, 262, 5413, -27579, -25189, -26331, -28236, -28144, -23804, -23261, -23013, -19045, 5419, 6332, 5665, 2450, 3617, 8638, 5178, 10374, 6819, 7434, 150, 2446, 9647, -27504, -27586, -31695, 11142, 18704, 10775, 21898, 10554, 9492, 11811, 11140, -15978, -24180, -32498, -29288, 29084, -9579, -18799, -27122, 913, 6024, 2966, -5238, 6542, 15240, 10, 39, 3986, 2324, 6030, -31709, 512, 33, 9394, -4833, 28967, -25290, 30866, 13466, -28403, -24022, -14042, -22472, -18166, 20493, -27626, -22266, -1784, -31473, 9612, 7324, 646, 778, 786, 800, -624, -3198, -8164, 1678, 29068, 22412, -32253, 29075, 22404, -22006, 3974, 7054, 10376, 3222, 8070, 12, 1930, 2, -22481, 4884, 5797, 5900, 6, 40, 1932, 5638, 8466, -25537}, {4620, 11140, 6433, 6, 4752, 10264, 7072, 3990, 309, 910, 9509, 9117, 3600, 16442, -32601, 22020, 2694, -22264, 10281, 3869, -28007, 10764, 8974, 3745, 10269, 914, 8629, -26438, 26779, -15208, 8728, 2982, -25718, 918, 6842, -25030, -5462, -14833, -22386, 8, -22468, -27479, -11237, 21664, 45, -24054, -27207, -22887, -30051, 20003, 21584, -31426, 28990, -11219, 6353, -25560, -25458, -14182, -11006, -27365, 3241, 7457, 8899, 13760, 4565, 11711, 10436, 8103, 2604, 13740, -28232, -26333, -23910, -31403, -32352, -29812, -29909, -24137, 925, 1181, 3865, 6049, 8582, 1804, 4887, -5372, 15011, 20785, 7591, 9528, 21770, 14094, 6930, 14635, 5911, 6463, 10124, 17935, 3218, 262, 5413, -27579, -25189, -26331, -28236, -28144, -23804, -23261, -23013, -19045, 5419, 6332, 5665, 2450, 3617, 8638, 5178, 10374, 6819, 7434, 150, 2446, 9647, -27504, -27586, -31695, 11142, 18704, 10775, 21898, 10554, 9492, 11811, 11140, -15978, -24180, -32498, -29288, 29084, -9579, -18799, -27122, 913, 6024, 2966, -5238, 6542, 15240, 10, 39, 3986, 2324, 6030, -31709, 512, 33, 9394, -4833, 28967, -25290, 30866, 13466, -28403, -24022, -14042, -22472, -18166, 20493, -27626, -22266, -1784, -31473, 9612, 7324, 646, 778, 786, 800, -624, -3198, -8164, 1678, 29068, 22412, -32253, 29075, 22404, -22006, 3974, 7054, 10376, 3222, 8070, 12, 1930, 2, -22481, 4884, 5797, 5900, 6, 40, 1932, 5638, 8466, -25537}, {4540, 4529, 5674, 5667, 5785, 6802, 8072, 4562, 4437, 4317, 5896, 4488}};
   @OriginalMember(
      owner = "client!mt",
      name = "Y",
      descriptor = "[[[S"
   )
   public static short[][][] field6364 = new short[][][]{field6374, field6367, field6358};
   @OriginalMember(
      owner = "client!mt",
      name = "L",
      descriptor = "I"
   )
   public static int field6359;
   @OriginalMember(
      owner = "client!mt",
      name = "N",
      descriptor = "I"
   )
   public static int field6362;
   @OriginalMember(
      owner = "client!mt",
      name = "cb",
      descriptor = "I"
   )
   public static int field6365;
   @OriginalMember(
      owner = "client!mt",
      name = "Z",
      descriptor = "I"
   )
   public static int field6366;
   @OriginalMember(
      owner = "client!mt",
      name = "P",
      descriptor = "I"
   )
   public static int field6370;
   @OriginalMember(
      owner = "client!mt",
      name = "X",
      descriptor = "I"
   )
   public static int field6371;
   @OriginalMember(
      owner = "client!mt",
      name = "S",
      descriptor = "I"
   )
   public static int field6375;
   @OriginalMember(
      owner = "client!mt",
      name = "R",
      descriptor = "I"
   )
   public static int field6376;
   @OriginalMember(
      owner = "client!mt",
      name = "Q",
      descriptor = "[I"
   )
   private int[] field6360;
   @OriginalMember(
      owner = "client!mt",
      name = "U",
      descriptor = "[I"
   )
   private int[] field6368;

   @OriginalMember(
      owner = "client!mt",
      name = "b",
      descriptor = "(B)V"
   )
   public final void method708(byte arg0) {
      try {
         ++field6359;
         this.method3183(6508);
         if (arg0 >= -87) {
            field6358 = null;
         }
      } catch (RuntimeException var3) {
         throw class670.method4877(var3, field6377[3] + arg0 + ')');
      }
   }

   @OriginalMember(
      owner = "client!mt",
      name = "h",
      descriptor = "(I)V"
   )
   public static final void method3179(int arg0) {
      try {
         class459.field6944.method540(class472.field7060, class261.field3566.field9439.method1555(93) != 1 ? -1 : class560.field8376 - -256 << 2, 0);
         ++field6376;
         if (arg0 != -17110) {
            field6373 = false;
         }
      } catch (RuntimeException var2) {
         throw class670.method4877(var2, field6377[4] + arg0 + ')');
      }
   }

   @OriginalMember(
      owner = "client!mt",
      name = "a",
      descriptor = "([Lat;I)V"
   )
   public static final void method3180(class396[] param0, int param1) {
      // $FF: Couldn't be decompiled
   }

   @OriginalMember(
      owner = "client!mt",
      name = "c",
      descriptor = "(III)Z"
   )
   public static final boolean method3181(int arg0, int arg1, int arg2) {
      try {
         if (arg2 != 8) {
            method3179(-90);
         }

         ++field6375;
         return class423.method3214(32768, arg1, arg0) | ~(2048 & arg0) != -1 || class672.method4895(-18045, arg1, arg0);
      } catch (RuntimeException var4) {
         throw class670.method4877(var4, field6377[2] + arg0 + ',' + arg1 + ',' + arg2 + ')');
      }
   }

   @OriginalMember(
      owner = "client!mt",
      name = "g",
      descriptor = "(I)V"
   )
   public static void method3182(int arg0) {
      try {
         field6367 = null;
         field6374 = null;
         field6358 = null;
         field6361 = null;
         field6364 = null;
         if (arg0 != -18166) {
            method3182(-108);
         }
      } catch (RuntimeException var2) {
         throw class670.method4877(var2, field6377[0] + arg0 + ')');
      }
   }

   @OriginalMember(
      owner = "client!mt",
      name = "f",
      descriptor = "(I)V"
   )
   private final void method3183(int arg0) {
      int var2 = client.field4530;

      try {
         ++field6370;
         this.field6360 = new int[this.field6363 - -1];
         this.field6368 = new int[this.field6363 - -1];
         int var3 = 0;
         int var4 = 4096 / this.field6363;
         int var5 = this.field6369 * var4 >> 12;
         int var6 = 0;
         if (var2 != 0) {
            this.field6360[var6] = var3;
            this.field6368[var6] = var3 + var5;
            var3 += var4;
            ++var6;
         }

         while(true) {
            while(var6 < this.field6363) {
               this.field6360[var6] = var3;
               this.field6368[var6] = var3 + var5;
               var3 += var4;
               ++var6;
            }

            this.field6360[this.field6363] = 4096;
            if (var2 == 0) {
               if (arg0 != 6508) {
                  method3181(-19, -37, 90);
               }

               this.field6368[this.field6363] = this.field6368[0] + 4096;
               return;
            }

            var3 = arg0 + 6508;
            ++var6;
         }
      } catch (RuntimeException var8) {
         throw class670.method4877(var8, field6377[1] + arg0 + ')');
      }
   }

   @OriginalMember(
      owner = "client!mt",
      name = "<init>",
      descriptor = "()V"
   )
   public class416() {
      super(0, true);
   }

   @OriginalMember(
      owner = "client!mt",
      name = "a",
      descriptor = "(II)[I"
   )
   public final int[] method401(int param1, int param2) {
      // $FF: Couldn't be decompiled
   }

   @OriginalMember(
      owner = "client!mt",
      name = "a",
      descriptor = "(IILjj;)V"
   )
   public final void method244(int arg0, int arg1, class128 arg2) {
      try {
         ++field6362;
         if (arg1 != 0) {
            method3179(-62);
         }

         if (~arg0 != -1) {
            if (arg0 == 1) {
               this.field6369 = arg2.method1038((byte)-126);
               return;
            }

            if (arg0 != 2) {
               return;
            }

            if (client.field4530 == 0) {
               this.field6372 = arg2.method1104(255);
               return;
            }
         }

         this.field6363 = arg2.method1104(255);
      } catch (RuntimeException var6) {
         throw class670.method4877(var6, field6377[8] + arg0 + ',' + arg1 + ',' + (arg2 != null ? field6377[7] : field6377[6]) + ')');
      }
   }

   @OriginalMember(
      owner = "client!mt",
      name = "z",
      descriptor = "(Ljava/lang/String;)[C"
   )
   private static char[] method3184(String arg0) {
      char[] var10000 = arg0.toCharArray();
      if (var10000.length < 2) {
         var10000[0] = (char)(var10000[0] ^ 65);
      }

      return var10000;
   }

   @OriginalMember(
      owner = "client!mt",
      name = "z",
      descriptor = "([C)Ljava/lang/String;"
   )
   private static String method3185(char[] arg0) {
      int var10002 = arg0.length;
      char[] var10001 = arg0;
      int var10000 = var10002;

      for(int var1 = 0; var10000 > var1; ++var1) {
         char var10004 = var10001[var1];
         byte var10005;
         switch(var1 % 5) {
         case 0:
            var10005 = 79;
            break;
         case 1:
            var10005 = 100;
            break;
         case 2:
            var10005 = 5;
            break;
         case 3:
            var10005 = 26;
            break;
         default:
            var10005 = 65;
         }

         var10001[var1] = (char)(var10004 ^ var10005);
      }

      return (new String(var10001)).intern();
   }
}
